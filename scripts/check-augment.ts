import { buildCalendarCrossAugment } from '@/lib/calendar/destinyCalendar'
...
async function main() {
  ...
  for (const d of aug.domains) {
    ...
    if (d.topConfirms.length)
      console.log(
        `    동의: ${d.topConfirms.map((c) => `(${c.intensity}) ${c.meaning}`).join(' / ')}`
      )
    if (d.dualSignals.length)
      console.log(
        `    양면: ${d.dualSignals.map((c) => `(${c.intensity}) ${c.meaning}`).join(' / ')}`
      )
  }
  console.log()
