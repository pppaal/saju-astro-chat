...
async function main() {
  ...
  const refined = cells.map((c) => {
    ...
    return {
      iso: c.datetime.slice(0, 10),
      plateau: cellSurprise(c, rates, 6).total,
      refined: Math.round(score * 100) / 100,
      contribs,
    }
  })
  ...
  for (const r of ranked.slice(0, 10)) {
    ...
    console.log(
      `   ${r.iso}    ${String(r.plateau).padStart(6)}   ${String(r.refined).padStart(7)}${mark}`
    )
  }
  ...
  for (const c of ranked[0].contribs.slice(0, 6))
    console.log(
      `     · ${c.name} (${c.src}, pol ${c.pol > 0 ? '+' : ''}${c.pol}, 피크에서 ${c.dPeak}일, 유효기여 ${c.eff.toFixed(2)})`
    )
        ...
        : r.sjPol === 1
          ...
          : r.sjPol <= -2
            ...
            : r.sjPol === -1
              ...
              : '·'
    console.log(
      `   ${SIJIN[r.idx].padEnd(12)} ${sjShort.padEnd(20)} ${mark.padEnd(4)}  ${r.planet}`
    )
  }
  console.log('\n' + '━'.repeat(72))
}
