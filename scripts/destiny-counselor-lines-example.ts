...
function twelveStage(dayStem: string, branch: string): string {
  const table =
    STEM_YINYANG[dayStem] === 'yang' ? TWELVE_STAGE_YANG[dayStem] : TWELVE_STAGE_YIN[dayStem]
  return table?.[branch] ?? '?'
}
...
async function main() {
  ...
  const pillars: Array<{ stem: string; branch: string; stemSib?: string; branchSib?: string }> = [
    {
      ...
      stemSib: p?.year?.heavenlyStem?.sibsin,
      branchSib: p?.year?.earthlyBranch?.sibsin,
    },
    {
      stem: p?.month?.heavenlyStem?.name ?? '',
      branch: p?.month?.earthlyBranch?.name ?? '',
      stemSib: p?.month?.heavenlyStem?.sibsin,
      branchSib: p?.month?.earthlyBranch?.sibsin,
    },
    {
      stem: p?.day?.heavenlyStem?.name ?? '',
      branch: p?.day?.earthlyBranch?.name ?? '',
      stemSib: '-',
      branchSib: p?.day?.earthlyBranch?.sibsin,
    },
    {
      stem: p?.time?.heavenlyStem?.name ?? '',
      branch: p?.time?.earthlyBranch?.name ?? '',
      stemSib: p?.time?.heavenlyStem?.sibsin,
      branchSib: p?.time?.earthlyBranch?.sibsin,
    },
  ]
  ...
  for (let i = 0; i < 4; i++) {
    push(
      `${PILLAR_LABELS[i]}: ${pillars[i].stem}${pillars[i].branch}  (천간 ${pillars[i].stemSib ?? '-'} / 지지 ${pillars[i].branchSib ?? '-'})`
    )
  }
  push(`일간: ${dayMaster}`)
  push('')
  ...
  for (let i = 0; i < 4; i++) {
    for (let j = i + 1; j < 4; j++) {
      const aS = pillars[i].stem,
        bS = pillars[j].stem
      if (STEM_HAP[aS]?.other === bS) {
        push(
          `${PILLAR_LABELS[i]}천간 ${aS} + ${PILLAR_LABELS[j]}천간 ${bS} — ${aS}${bS}合化${STEM_HAP[aS]!.element} (천간합)`
        )
      }
      if (STEM_CHUNG[aS] === bS) {
        push(`${PILLAR_LABELS[i]}천간 ${aS} ↔ ${PILLAR_LABELS[j]}천간 ${bS} — 천간충`)
      }
      const aB = pillars[i].branch,
        bB = pillars[j].branch
      if (BRANCH_HAP[aB]?.other === bB) {
        push(
          `${PILLAR_LABELS[i]}지 ${aB} + ${PILLAR_LABELS[j]}지 ${bB} — ${aB}${bB}합화${BRANCH_HAP[aB]!.element} (지지합)`
        )
      }
  ...
  if (cur) {
    ...
    const dS = cur.heavenlyStem ?? '',
      dB = cur.earthlyBranch ?? ''
    for (let i = 0; i < 4; i++) {
      if (STEM_HAP[pillars[i].stem]?.other === dS) {
        push(
          `${PILLAR_LABELS[i]}천간 ${pillars[i].stem} + 대운 ${dS} — ${pillars[i].stem}${dS}合 (본명-대운 합)`
        )
      }
      ...
      if (BRANCH_HAP[pillars[i].branch]?.other === dB) {
        push(
          `${PILLAR_LABELS[i]}지 ${pillars[i].branch} + 대운 ${dB} — ${pillars[i].branch}${dB}합`
        )
      }
  ...
  for (const pl of [...chart.planets, chart.ascendant, chart.mc]) {
    ...
    push(
      `${pl.name} in ${enSign} ${pl.degree}°${pl.minute.toString().padStart(2, '0')}'${houseStr}${pl.retrograde ? ' R' : ''}`
    )
  }
  push('')
  ...
  for (const asp of aspects) {
    ...
    push(
      `${asp.from.name} in ${fromSign} ${ASPECT_NAME[asp.type] ?? asp.type} ${asp.to.name} in ${toSign} (Orb: ${orbToDegMin(asp.orb)})`
    )
  }
