/**
 * 운세 캘린더 점수 시스템 대규모 테스트
 * 100명의 무작위 사용자 데이터로 점수 분포 확인
 */
...
console.log('운세 캘린더 점수 시스템 대규모 테스트 - 100명 무작위 검사');
console.log('='.repeat(80));
console.log();
...
console.log('📊 점수 분포:');
console.log(`   평균: ${avgScore}점 (목표: 52~57점)`);
...
for (let grade = 0; grade <= 5; grade++) {
  ...
  const barLength = Math.floor(count / 2);
  const bar = '█'.repeat(barLength);
  console.log(`   Grade ${grade} ${gradeEmojis[grade]} ${gradeLabels[grade].padEnd(8)} │ ${String(count).padStart(3)}명 (${String(percentage).padStart(5)}%) ${bar}`);
}
...
for (let grade = 0; grade <= 5; grade++) {
  const actual = ((gradeDistribution[grade as keyof typeof gradeDistribution] / 100) * 100).toFixed(1);
  ...
  console.log(`   Grade ${grade} ${gradeEmojis[grade]} ${gradeLabels[grade].padEnd(8)} │ 실제: ${String(actual).padStart(5)}% │ 이론: ${String(theoretical).padStart(5)}% │ 차이: ${diffSign}${String(diff).padStart(5)}%`);
}
...
console.log(`   사주 점수: ${avgSaju}점 / 50점 (${((parseFloat(avgSaju) / 50) * 100).toFixed(1)}%)`);
console.log(`   점성술 점수: ${avgAstro}점 / 50점 (${((parseFloat(avgAstro) / 50) * 100).toFixed(1)}%)`);
console.log(`   교차검증 보너스: ${parseFloat(avgCross) >= 0 ? '+' : ''}${avgCross}점`);
...
for (const range of scoreRanges) {
  ...
  console.log(`   ${range.label.padEnd(8)} │ ${String(count).padStart(3)}명 (${String(percentage).padStart(5)}%) ${bar}`);
}
