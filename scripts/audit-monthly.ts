...
async function main() {
  ...
  for (const m of ['2026-04', '2026-05', '2026-06']) {
    ...
    const cell = cells[14]  // 15th — middle of month
    console.log(`\n=== ${m} 중순 monthly signals ===`)
    const monthly = cell.signals.filter((s: any) => s.layer === 'monthly')
    for (const s of monthly) {
      console.log(`  ${s.kind} | ${s.name} | sibsin=${s.evidence?.sibsin} | polarity=${s.polarity}`)
    }
  }
}
