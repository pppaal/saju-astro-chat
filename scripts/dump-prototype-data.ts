...
import { computeBaseRates, cellSurprise, signalImportance, type BaseRateTable } from '@/lib/calendar-engine/derivers/surprise'
...
async function main() {
  ...
  const summary = {
    ...
    dayMaster: natal.saju?.dayMaster?.name, strength: natal.saju?.strength,
    yongsin: [ys?.primary, ys?.secondary].filter(Boolean).join('·'),
    sun: planet('Sun')?.sign, moon: planet('Moon')?.sign,
    asc: chart?.ascendant?.sign, mc: chart?.mc?.sign,
  }
  ...
  const firstCells = await buildCalendar(natal, { start: '2026-01-01', end: '2026-12-31', granularity: 'day' })
  ...
  for (const y of YEARS) {
    const cells = y === 2026 ? firstCells
      : await buildCalendar(natal, { start: `${y}-01-01`, end: `${y}-12-31`, granularity: 'day' })
    for (const c of cells) {
      const sp = cellSurprise(c, rates, 6)
      allDays.push({
        iso: c.datetime, sal: sp.total, fav: c.derivedScore,
        top: sp.top.map((t) => ({ name: t.name, source: t.source, importance: t.importance, polarity: t.polarity })),
  ...
  const decade = YEARS.map((y) => {
    ...
    return { year: y, salience: Math.round(sal), fav, peakDay: peak.iso.slice(0, 10), peakSal: Math.round(peak.sal * 10) / 10 }
  })
  ...
  for (const d of allDays.filter((d) => d.iso.startsWith(String(topYear)))) {
    const k = d.iso.slice(0, 7); (monthMap.get(k) ?? monthMap.set(k, []).get(k)!).push(d)
  }
  const months = [...monthMap.entries()].map(([month, ds]) => ({
    month, salience: Math.round(ds.reduce((a, d) => a + d.sal, 0)),
  ...
  const monthDays = allDays.filter((d) => d.iso.startsWith(topMonth))
    .map((d) => ({
      date: d.iso.slice(0, 10), salience: Math.round(d.sal * 10) / 10, fav: d.fav,
      patterns: d.patterns,
      signals: d.top.slice(0, 5).map((t) => ({
        name: t.name, source: t.source, polarity: t.polarity,
  ...
  // 정점일 (정확각 근접 타이브레이크)
  const winCells = await buildCalendar(natal, { start: `${topMonth}-01`, end: `${topMonth}-28`, granularity: 'day' })
  const SIGMA = 2.5
  const peakDay = winCells.map((c) => {
    const di = dayNum(c.datetime)
    let s = 0
    for (const sig of c.signals) {
      const imp = signalImportance(sig, rates); if (imp <= 0) continue
  ...
  const hourCells = await buildCalendar(
    natal, { start: peakDay, end: peakDay, granularity: 'hour' },
  ...
  for (const c of hourCells.filter((c) => c.datetime.slice(0, 10) === peakDay)) {
    ...
    if (!sijinMap.has(idx) || (sj && !sijinMap.get(idx).ganji)) {
      sijinMap.set(idx, {
        branch: SIJIN[idx], range: RANGES[idx],
        ganji: sj ? String(sj.korean ?? sj.name).replace(' 시진', '') : '',
        polarity: sj?.polarity ?? 0, planet: ph ? (ph.korean ?? ph.name) : '',
      })
    }
  }
  const sijin = [...Array(12)].map((_, i) => sijinMap.get(i) ?? { branch: SIJIN[i], range: RANGES[i], ganji: '', polarity: 0, planet: '' })

  const out = {
    summary,
    lifetime: flow,
    decade, topYear,
    year: { year: topYear, months }, topMonth,
