...
import { computeBaseRates, cellSurprise, type BaseRateTable } from '@/lib/calendar-engine/derivers/surprise'
...
interface DaySummary {
  iso: string; surprise: number; fav: number
...
function scoreYear(cells: CalendarCell[], rates: BaseRateTable): DaySummary[] {
  return cells.map((c) => {
    ...
    return {
      iso: c.datetime, surprise: sp.total, fav: c.derivedScore,
...
async function main() {
  ...
  for (const y of yearAgg)
    console.log(`${y.year}  ${bar(y.intensity, maxY)}  우호 ${String(y.fav).padStart(3)}  ▲ ${d10(y.peak.iso)} (${y.peak.surprise})`)
  const topYear = [...yearAgg].sort((a, b) => b.intensity - a.intensity)[0].year

  // ───── ② 월 (top 해) ─────
  const monthMap = new Map<string, { intensity: number; favSum: number; n: number; peak: DaySummary }>()
  for (const d of allDays.filter((d) => d.iso.startsWith(String(topYear)))) {
    ...
    a.intensity += d.surprise; a.favSum += d.fav; a.n += 1
    if (d.surprise > a.peak.surprise) a.peak = d
    monthMap.set(k, a)
  }
  const months = [...monthMap.entries()].map(([k, a]) => ({ month: k, intensity: a.intensity, fav: Math.round(a.favSum / a.n), peak: a.peak })).sort((a, b) => a.month.localeCompare(b.month))
  const maxM = Math.max(...months.map((m) => m.intensity))
  console.log('\n' + '━'.repeat(72))
  console.log(`② 월 — 가장 강한 해 ${topYear}년의 12개월`)
  console.log('━'.repeat(72))
  for (const m of months)
    console.log(`${m.month}  ${bar(m.intensity, maxM)}  우호 ${String(m.fav).padStart(3)}  ▲ ${d10(m.peak.iso)} (${m.peak.surprise})`)
  const topMonth = [...months].sort((a, b) => b.intensity - a.intensity)[0].month

  // ───── ③ 일 (top 달) ─────
  const dayList = allDays.filter((d) => d.iso.startsWith(topMonth)).sort((a, b) => b.surprise - a.surprise)
  console.log('\n' + '━'.repeat(72))
  ...
  for (const d of dayList.slice(0, 6)) {
    ...
    for (const tp of d.top.slice(0, 4))
      console.log(`     · ${tp.name}  (${tp.source}, 기여 ${tp.importance.toFixed(2)}, pol ${tp.polarity > 0 ? '+' : ''}${tp.polarity})`)
  }
  ...
  const h = hourCells.map((c) => ({ iso: c.datetime, sp: cellSurprise(c, rates, 5), fav: c.derivedScore }))
    .sort((a, b) => a.iso.localeCompare(b.iso))
  if (h.length <= 1) {
    console.log('  (이 날 신호 대부분이 일(日) 단위라 시각이 한 버킷으로 모임 — 시주/행성시 등')
    console.log('   시 전용 신호만 시각을 가른다. 활성 신호:')
    for (const tp of (h[0]?.sp.top ?? [])) console.log(`     · ${tp.name} (${tp.source}, pol ${tp.polarity})`)
  } else {
    for (const x of h) console.log(`  ${x.iso.slice(11, 13)}시  우호 ${String(x.fav).padStart(3)}  ${x.sp.top.slice(0, 2).map((t) => t.name).join(', ')}`)
  }
  console.log('\n' + '━'.repeat(72))
}
