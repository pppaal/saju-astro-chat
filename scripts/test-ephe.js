...
console.warn('테스트를 시작합니다...');
console.warn('현재 위치:', __dirname);

const ephePath = path.join(__dirname, 'public', 'ephe');
console.warn('천체력 파일 경로:', ephePath);

swisseph.swe_set_ephe_path(ephePath);

console.warn('경로 설정 완료. 행성 위치 계산을 시도합니다...');
...
swisseph.swe_calc_ut(julianDay, sunSign, swisseph.SEFLG_SPEED, (err, result) => {
  if (err) {
    ...
    console.warn('***************** 성공 *****************');
    console.warn('천체력 파일을 성공적으로 읽고 계산을 완료했습니다.');
    console.warn('계산 결과:', result);
    console.warn('****************************************');
  }
});