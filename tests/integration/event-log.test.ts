...
describe("Integration: Event Log", () => {
  ...
  describe("Event Statistics", () => {
    ...
    it("counts events by name", async () => {
      const names = ["saju_started", "tarot_started", "saju_started", "saju_started", "compatibility_started"];
  ...
  describe("Funnel Analysis", () => {
    it("tracks conversion funnel", async () => {
      ...
      const landingCount = stepCounts.find(
        (c) => c.eventName === "landing_page_viewed"
      )?._count.id;
      const purchaseCount = stepCounts.find(
        (c) => c.eventName === "subscription_purchased"
      )?._count.id;
