...
describe("Integration: Admin Audit System", () => {
  ...
  describe("Admin Audit Log", () => {
    ...
    it("filters logs by action type", async () => {
      await testPrisma.adminAuditLog.create({
        data: { adminId: "admin_1", action: "user_ban", targetType: "user", targetId: "u1", details: {} },
      });
      await testPrisma.adminAuditLog.create({
        data: { adminId: "admin_1", action: "credit_adjust", targetType: "credits", targetId: "c1", details: {} },
      });
      await testPrisma.adminAuditLog.create({
        data: { adminId: "admin_1", action: "user_ban", targetType: "user", targetId: "u2", details: {} },
      });
  ...
  describe("User Report", () => {
    ...
    it("retrieves pending reports", async () => {
      ...
      await testPrisma.userReport.create({
        data: { reporterId: reporter.id, reportedId: reported1.id, reason: "spam", status: "pending" },
      });
      await testPrisma.userReport.create({
        data: { reporterId: reporter.id, reportedId: reported2.id, reason: "abuse", status: "resolved" },
      });
  ...
  describe("Credit Refund Log", () => {
    ...
    it("retrieves refunds by status", async () => {
      ...
      await testPrisma.creditRefundLog.create({
        data: { userId: user.id, amount: 5, reason: "R1", processedBy: "admin", status: "completed" },
      });
      await testPrisma.creditRefundLog.create({
        data: { userId: user.id, amount: 10, reason: "R2", processedBy: "admin", status: "pending" },
      });
      await testPrisma.creditRefundLog.create({
        data: { userId: user.id, amount: 15, reason: "R3", processedBy: "admin", status: "completed" },
      });
    ...
    it("calculates total refunded amount", async () => {
      ...
      for (const amount of amounts) {
        await testPrisma.creditRefundLog.create({
          data: { userId: user.id, amount, reason: "Test", processedBy: "admin", status: "completed" },
        });
      }
