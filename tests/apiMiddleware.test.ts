...
describe('extractLocale', () => {
  ...
  it('returns en for non-Korean (Japanese) accept-language', () => {
    const req = new Request('http://localhost/api/test', {
  ...
  it('returns en for non-Korean (Chinese) accept-language', () => {
    const req = new Request('http://localhost/api/test', {
  ...
  it('falls back to en for unsupported URL locale (ja)', () => {
    const req = new Request('http://localhost/api/test?locale=ja')
    expect(extractLocale(req)).toBe('en')
  })

  it('falls back to en for unsupported URL locale (zh)', () => {
...
describe('apiSuccess', () => {
  ...
  it('creates success result with meta', () => {
    const result = apiSuccess({ items: [] }, { meta: { total: 100, page: 1, pageSize: 10 } })
