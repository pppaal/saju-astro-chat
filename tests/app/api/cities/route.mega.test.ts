...
describe('GET /api/cities', () => {
  ...
  it('should search cities by English name (starts with)', async () => {
    ...
    const hasSeoulStation = data.results.some((c: typeof mockCities[0]) => c.name === 'Seoul Station');
    if (hasSeoulStation) {
  ...
  it('should search cities by country name', async () => {
    ...
    expect(response.status).toBe(200);
    expect(data.results.length).toBeGreaterThan(0);
    expect(data.results.every((c: typeof mockCities[0]) => c.country === 'South Korea')).toBe(true);
  });
  ...
  it('should search by Korean country name', async () => {
    ...
    const hasKoreanCities = data.results.some((c: typeof mockCities[0]) => c.country === 'South Korea');
  ...
  it('should return empty array for no query param', async () => {
    const { GET } = await import('@/app/api/cities/route');
  ...
  it('should enforce maximum limit of 200', async () => {
    ...
    expect(response.status).toBe(200);
    // Results should be capped at 200
    expect(data.results.length).toBeLessThanOrEqual(200);
  });

  it('should default to limit 200 when not specified', async () => {
