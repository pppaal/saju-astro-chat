...
describe('Calendar API Route - /api/calendar', () => {
  ...
  describe('Date Calculations for Different Years', () => {
    ...
    it('should calculate dates for past year', async () => {
      // year is validated to [currentYear-5, currentYear+5]; use an in-range past year.
      const pastYear = new Date().getFullYear() - 3
      const request = createRequest({ birthDate: '1990-01-15', year: String(pastYear) })

      const response = await GET(request)
      const data = await response.json()

      expect(response.status).toBe(200)
      expect(data.year).toBe(pastYear)
    })
  ...
  describe('Edge Cases', () => {
    ...
    it('should reject an out-of-range extreme past year', async () => {
      // year is validated to [CURRENT_YEAR-5, CURRENT_YEAR+5]; 1901 is rejected.
