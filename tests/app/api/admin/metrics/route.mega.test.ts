...
describe('GET /api/admin/metrics', () => {
  const mockMetricsSnapshot = {
    counters: [
      { name: 'api.request.total', value: 1000, labels: { service: 'tarot' } },
      {
        ...
        value: 10,
        labels: { service: 'tarot', error_category: 'VALIDATION' },
      },
    ...
    gauges: [{ name: 'session.active', value: 25 }],
  }
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle errors gracefully', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith('[Metrics API Error]', expect.any(Error))
    })
