/**
 * /api/tarot/interpret-stream/result — turnId 검증 + ownership 회귀.
 *
...
async function importRoute() {
  return await import('@/app/api/tarot/interpret-stream/result/route')
}

const makeReq = (turnId: string) =>
  new NextRequest(`http://localhost/api/tarot/interpret-stream/result?turnId=${turnId}`)

describe('/api/tarot/interpret-stream/result GET', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('로그인 + turnId 누락 → 400 (기존 에러 shape 유지)', async () => {
    ...
    const { GET } = await importRoute()
    const res = await GET(new NextRequest('http://localhost/api/tarot/interpret-stream/result'))
  ...
  it('로그인 + 자기 캐시 hit → ready=true + content 반환', async () => {
    mockGetServerSession.mockResolvedValue({ user: { id: 'user-A' } })
    mockCacheGet.mockResolvedValue('{"summary":"my tarot reading"}')
    const { GET } = await importRoute()
    const res = await GET(makeReq('turn-1'))
    const body = await res.json()
    expect(res.status).toBe(200)
    expect(body).toEqual({ ready: true, content: '{"summary":"my tarot reading"}' })
