...
// next-auth and prisma mocks moved to centralized mocks (see imports above)
...
describe('POST /api/compatibility', () => {
  ...
  describe('Input validation', () => {
    ...
    it('should reject missing relationToP1 for person 2+', async () => {
      const persons = [createBasicPerson(), createBasicPerson({ relationToP1: undefined })]
      const req = createValidRequest(persons)
      const response = await POST(req)
      const data = await response.json()
    ...
    it('should reject invalid relationToP1', async () => {
      const persons = [createBasicPerson(), createBasicPerson({ relationToP1: 'invalid' as never })]
      const req = createValidRequest(persons)
      const response = await POST(req)
      const data = await response.json()
