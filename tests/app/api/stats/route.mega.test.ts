...
describe('GET /api/stats', () => {
  ...
  it('should use client IP for rate limiting', async () => {
    ...
    expect(mockRateLimit).toHaveBeenCalledWith('api:stats:192.168.1.100', {
      limit: 30,
      windowSeconds: 60,
    })
  })
  ...
  it('should maintain separate rate limits per IP', async () => {
    ...
    expect(mockRateLimit).toHaveBeenCalledWith('api:stats:192.168.1.1', expect.any(Object))
    ...
    expect(mockRateLimit).toHaveBeenCalledWith('api:stats:192.168.1.2', expect.any(Object))
  })
  ...
  it('should use 30 requests per 60 seconds rate limit', async () => {
    ...
    expect(mockRateLimit).toHaveBeenCalledWith(expect.any(String), { limit: 30, windowSeconds: 60 })
  })
