...
describe('API auth/validation guards', () => {
  ...
  it('calendar rejects missing/invalid token', async () => {
    const res = await calendarGet(
      asNextRequest(
        new Request('http://test/api/calendar?birthDate=1990-01-01', {
  ...
  it('calendar validates required params', async () => {
    const res = await calendarGet(
      asNextRequest(
        new Request('http://test/api/calendar?birthDate=', {
  ...
  it('compatibility rejects invalid token', async () => {
    const res = await compatibilityPost(
      asNextRequest(
        new Request('http://test/api/compatibility', {
          method: 'POST',
