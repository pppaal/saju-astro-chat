...
describe('analyzeRelations: 천간합', () => {
  it('detects 甲己合', () => {
    const pillars = createPillars(['甲', '子'], ['己', '丑'], ['丙', '寅'], ['丁', '卯'])
  ...
  it('detects 乙庚合', () => {
    const pillars = createPillars(['乙', '子'], ['庚', '丑'], ['丙', '寅'], ['丁', '卯'])

    const hits = analyzeRelations({ pillars })
    const hapHits = hits.filter((h) => h.kind === '천간합')

    expect(hapHits.some((h) => h.detail?.includes('乙-庚') || h.detail?.includes('庚-乙'))).toBe(
      true
    )
  })
...
describe('analyzeRelations: 천간충', () => {
  it('detects 甲庚충 in mode 4', () => {
    const pillars = createPillars(['甲', '子'], ['庚', '丑'], ['丙', '寅'], ['丁', '卯'])
  ...
  it('detects additional 戊甲충 in mode 5', () => {
    const pillars = createPillars(['戊', '子'], ['甲', '丑'], ['丙', '寅'], ['丁', '卯'])
...
describe('analyzeRelations: 지지육합', () => {
  ...
  it('detects 寅亥合', () => {
    const pillars = createPillars(['甲', '寅'], ['乙', '亥'], ['丙', '辰'], ['丁', '巳'])

    const hits = analyzeRelations({ pillars })
    const yukhapHits = hits.filter((h) => h.kind === '지지육합')

    expect(yukhapHits.some((h) => h.detail?.includes('寅-亥') || h.detail?.includes('亥-寅'))).toBe(
      true
    )
  })
})
...
describe('analyzeRelations: 지지삼합', () => {
  it('detects 申子辰 삼합(수)', () => {
    const pillars = createPillars(['甲', '申'], ['乙', '子'], ['丙', '辰'], ['丁', '卯'])
  ...
  it('detects 寅午戌 삼합(화)', () => {
    const pillars = createPillars(['甲', '寅'], ['乙', '午'], ['丙', '戌'], ['丁', '卯'])
...
describe('analyzeRelations: 지지충', () => {
  it('detects 子午충', () => {
    const pillars = createPillars(['甲', '子'], ['乙', '午'], ['丙', '寅'], ['丁', '卯'])
  ...
  it('detects 寅申충', () => {
    const pillars = createPillars(['甲', '寅'], ['乙', '申'], ['丙', '辰'], ['丁', '卯'])
...
describe('analyzeRelations: 지지형', () => {
  it('detects 寅巳申 삼형', () => {
    const pillars = createPillars(['甲', '寅'], ['乙', '巳'], ['丙', '申'], ['丁', '卯'])
  ...
  it('detects 子卯 무은형', () => {
    const pillars = createPillars(['甲', '子'], ['乙', '卯'], ['丙', '辰'], ['丁', '巳'])
  ...
  it('detects self-punish when option enabled', () => {
    const pillars = createPillars(['甲', '午'], ['乙', '午'], ['丙', '辰'], ['丁', '巳'])
...
describe('analyzeRelations: 지지파', () => {
  it('detects 子酉파', () => {
    const pillars = createPillars(['甲', '子'], ['乙', '酉'], ['丙', '辰'], ['丁', '巳'])
...
describe('analyzeRelations: 지지해', () => {
  it('detects 子未해', () => {
    const pillars = createPillars(['甲', '子'], ['乙', '未'], ['丙', '辰'], ['丁', '巳'])
...
describe('analyzeRelations: 공망', () => {
  ...
  it('respects includeGongmang option', () => {
    const pillars = createPillars(['甲', '戌'], ['乙', '丑'], ['甲', '子'], ['丁', '卯'])
...
describe('analyzeRelations: Options', () => {
  it('respects includeHeavenly option', () => {
    ...
    const withHeavenlyCount = hitsWithHeavenly.filter((h) => h.kind.startsWith('천간')).length
    const withoutHeavenlyCount = hitsWithoutHeavenly.filter((h) => h.kind.startsWith('천간')).length
  ...
  it('respects includeEarthly option', () => {
    ...
    const withEarthlyCount = hitsWithEarthly.filter((h) => h.kind.startsWith('지지')).length
    const withoutEarthlyCount = hitsWithoutEarthly.filter((h) => h.kind.startsWith('지지')).length
...
describe('analyzeRelations: Sorting', () => {
  it('returns sorted results by kind', () => {
    const pillars = createPillars(['甲', '子'], ['己', '午'], ['丙', '寅'], ['丁', '卯'])
