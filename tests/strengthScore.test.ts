...
describe('calculateStrengthScore', () => {
  ...
  it('includes 통근 item in calculation', () => {
    const pillars = createTestPillars(['甲', '寅'], ['乙', '卯'], ['甲', '辰'], ['丁', '巳'])
  ...
  it('respects custom monthBranch parameter', () => {
    const pillars = createTestPillars(['甲', '子'], ['乙', '丑'], ['甲', '辰'], ['丁', '卯'])
  ...
  it('total is clamped between 0 and 100', () => {
    ...
    const strongPillars = createTestPillars(['甲', '寅'], ['甲', '寅'], ['甲', '寅'], ['甲', '寅'])

    const weakPillars = createTestPillars(['庚', '申'], ['庚', '申'], ['甲', '酉'], ['庚', '申'])
...
describe('calculateGeokgukScore', () => {
  ...
  it('decreases purity and stability for 충', () => {
    // 子午충
    const pillars = createTestPillars(['甲', '子'], ['乙', '午'], ['丙', '寅'], ['丁', '卯'])
  ...
  it('detects 寅申충', () => {
    const pillars = createTestPillars(['甲', '寅'], ['乙', '申'], ['丙', '辰'], ['丁', '卯'])
  ...
  it('detects 卯酉충', () => {
    const pillars = createTestPillars(['甲', '卯'], ['乙', '酉'], ['丙', '辰'], ['丁', '巳'])
  ...
  it('detects 寅亥합', () => {
    const pillars = createTestPillars(['甲', '寅'], ['乙', '亥'], ['丙', '辰'], ['丁', '巳'])
...
describe('calculateYongsinFitScore', () => {
  ...
  it('calculates high presence score when yongsin is abundant', () => {
    // 수 용신, 수가 많은 사주
    const pillars = createTestPillars(['壬', '子'], ['癸', '亥'], ['甲', '子'], ['壬', '亥'])
...
describe('calculateComprehensiveScore', () => {
  ...
  it('identifies strengths for strong day master', () => {
    const pillars = createTestPillars(['甲', '寅'], ['甲', '寅'], ['甲', '寅'], ['甲', '寅'])

    const score = calculateComprehensiveScore(pillars)

    // Should have strength-related positive comment
    const hasStrengthPositive = score.strengths.some((s) => s.includes('강') || s.includes('극복'))
    expect(hasStrengthPositive).toBe(true)
  })

  it('identifies weaknesses for weak day master', () => {
    const pillars = createTestPillars(['庚', '申'], ['辛', '酉'], ['甲', '戌'], ['庚', '申'])
  ...
  it('assigns S grade for very high scores', () => {
    ...
    const pillars = createTestPillars(['甲', '寅'], ['丙', '午'], ['戊', '辰'], ['庚', '申'])
