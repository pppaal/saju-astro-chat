...
describe('Logger Module', () => {
  ...
  describe('Logger methods', () => {
    ...
    it('serializes an Error passed inside a context object (no {} loss)', async () => {
      // 회귀 가드: 예전엔 prod formatMessage 가 context 를 그대로 JSON.stringify
      // 해 `{ err: new Error('boom') }` 가 `{"err":{}}` 로 소실됐다(Error 의
      // message/stack 은 non-enumerable). normalizeContext 로 펼쳐 실제 메시지가
      // 로그에 남아야 한다. (test 환경은 isDevelopment=false → prod JSON 분기)
      const { logger } = await import('@/lib/logger')
      const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {})

      logger.error('refund failed', { err: new Error('boom-marker-xyz'), reason: 'test' })

      const output = errorSpy.mock.calls.map((c) => String(c[0])).join('\n')
      expect(output).toContain('boom-marker-xyz')
