...
import { yesNoEndingPatterns, yesNoMidPatterns } from '@/lib/tarot/data/questionClassifierPatterns'
  ...
  describe('Real-world question classification', () => {
    it('correctly identifies yes/no questions', () => {
      ...
      testQuestions.forEach((q) => {
        const isYesNo =
          yesNoEndingPatterns.some((p) => p.test(q)) || yesNoMidPatterns.some((p) => p.test(q))
    ...
    it('correctly rejects pure wh-questions without yes/no patterns', () => {
      const testQuestions = ['어떻게 살아야 하지', '왜 이렇게 힘든거지', '뭐가 문제일까']
