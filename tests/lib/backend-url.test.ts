...
describe('backend-url', () => {
  ...
  describe('getBackendUrl', () => {
    ...
    it('should not warn for HTTPS in production', () => {
      ...
      expect(mockLogger.warn).not.toHaveBeenCalledWith(expect.stringContaining('non-HTTPS'))
    })
  ...
  describe('getBackendUrl (alias check)', () => {
    it('should be callable and return same result', () => {
      process.env.AI_BACKEND_URL = 'https://test.com'
      expect(getBackendUrl()).toBe('https://test.com')
    })

    it('should consistently return same result across calls', () => {
      process.env.AI_BACKEND_URL = 'https://test.com'
      expect(getBackendUrl()).toBe(getBackendUrl())
    })
  })
