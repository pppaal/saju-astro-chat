...
describe('zodValidation MEGA - dateSchema', () => {
  it.each([
    '2024-01-01', '2024-12-31', '2000-02-29', '1900-01-01', '2100-12-31', '2024-06-15',
  ])('should accept valid date: %s', (date) => {
...
describe('zodValidation MEGA - timeSchema', () => {
  it.each([
    '00:00', '23:59', '12:00', '01:30', '14:45',
    '00:00 AM', '12:00 PM', '11:59 PM', '1:30 AM', '9:45 pm',
  ])('should accept valid time: %s', (time) => {
    expect(timeSchema.safeParse(time).success).toBe(true);
  });

  it.each([
    '24:00', '12:60', '1:5', '25:00', '12:00:00', 'invalid', '',
  ])('should reject invalid time: %s', (time) => {
...
describe('zodValidation MEGA - timezoneSchema', () => {
  it.each([
    ...
    'GMT', 'Asia/Tokyo', 'America/Los_Angeles', 'Europe/Paris',
...
describe('zodValidation MEGA - longitudeSchema', () => {
  it.each([-180, -90, 0, 90, 180, 126.9780, 151.2093, -0.1278])(
...
describe('zodValidation MEGA - genderSchema', () => {
  ...
  it.each(['M', 'F', 'MALE', 'FEMALE', 'unknown', ''])(
...
describe('zodValidation MEGA - localeSchema', () => {
  ...
  it.each(['ko-KR', 'en-US', 'invalid', '', 'kr'])(
    'should reject invalid locale: %s',
    (locale) => {
...
describe('zodValidation MEGA - compatibilityRequestSchema', () => {
  ...
  it('should accept valid request', () => {
    const result = compatibilityRequestSchema.safeParse({
      person1: person,
      person2: { ...person, birthDate: '1992-05-20' },
    });
    expect(result.success).toBe(true);
  });

  it('should accept with analysisType', () => {
    const result = compatibilityRequestSchema.safeParse({
      person1: person,
      person2: person,
  ...
  it('should accept all analysis types', () => {
    const types = ['romantic', 'friendship', 'business', 'family'];
    types.forEach(type => {
      const result = compatibilityRequestSchema.safeParse({
        person1: person,
        person2: person,
...
describe('zodValidation MEGA - chatMessageSchema', () => {
...
describe('zodValidation MEGA - paginationSchema', () => {
...
describe('zodValidation MEGA - validateRequestBody', () => {
  it('should validate valid body', async () => {
    ...
    const result = await validateRequestBody(req, chatMessageSchema);
    expect(result.success).toBe(true);
  });
...
describe('zodValidation MEGA - validateQueryParams', () => {
  it('should validate valid params', () => {
    const req = new Request('http://localhost?page=1&limit=20');
    const result = validateQueryParams(req, paginationSchema);
    expect(result.success).toBe(true);
  });

  it('should convert numeric strings', () => {
    const req = new Request('http://localhost?page=5&limit=50');
    const result = validateQueryParams(req, paginationSchema);
    if (result.success) {
      expect(result.data.page).toBe(5);
      expect(result.data.limit).toBe(50);
