...
describe('middleware MEGA - extractLocale', () => {
  it.each([
    ...
    // Non-Korean Accept-Language headers now resolve to 'en' (en/ko-only app)
    ['en', 'ja-JP,ja;q=0.9', 'Accept-Language'],
    ['en', 'zh-CN,zh;q=0.9', 'Accept-Language'],
  ...
  it.each([
    ['ko', '?locale=ko'],
    // Non-Korean URL locales now resolve to 'en' (en/ko-only app)
    ['en', '?locale=ja'],
    ['en', '?locale=zh'],
...
describe('middleware MEGA - initializeApiContext', () => {
  ...
  it('should detect premium users', async () => {
    vi.mocked(getServerSession).mockResolvedValue(createMockSession({ plan: 'premium' }))
  ...
  it('should not consider free users as premium', async () => {
    vi.mocked(getServerSession).mockResolvedValue(createMockSession({ plan: 'free' }))
