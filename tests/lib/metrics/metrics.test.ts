...
describe('Metrics Module', () => {
  ...
  describe('recordCounter', () => {
    ...
    it('should handle labels correctly', () => {
      ...
      const successCounter = snapshot.counters.find((c) => c.labels?.status === 'success')
      const errorCounter = snapshot.counters.find((c) => c.labels?.status === 'error')
  ...
  // 고카디널리티 라벨(per-user/IP 등)이 module-level series 맵을 무한 증가시키지
  // 못하게 하는 백스톱. 기본 cap 5000 을 넘겨 flood 해 검증.
  describe('series cardinality cap (unbounded-growth backstop)', () => {
    it('drops NEW series past the cap instead of growing without bound', () => {
      resetMetrics()
      for (let i = 0; i < 6000; i++) {
        recordCounter('flood.counter', 1, { id: String(i) })
      }
      const snapshot = getMetricsSnapshot()
      // 6000 고유 라벨을 넣어도 cap(5000) 이하로 제한된다.
      expect(snapshot.counters.length).toBeLessThanOrEqual(5000)
      expect(snapshot.counters.length).toBeGreaterThan(0)
    })

    it('keeps updating series that already exist after the cap is hit', () => {
      resetMetrics()
      // 첫 series — cap 아래라 항상 유지된다.
      recordCounter('kept.series', 1, { a: '1' })
      for (let i = 0; i < 6000; i++) {
        recordCounter('flood', 1, { id: String(i) })
      }
      // 기존 series 갱신은 cap 이후에도 반영되어야 한다(드롭은 NEW series 만).
      recordCounter('kept.series', 5, { a: '1' })
      const kept = getMetricsSnapshot().counters.find((c) => c.name === 'kept.series')
