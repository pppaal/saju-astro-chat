...
describe("Chart Cache Client", () => {
  ...
  describe("Cache Key Generation", () => {
    it("generates unique keys for different birth data", () => {
      // We can't directly test private functions, but we can test behavior
      // that relies on them through public API
      expect(true).toBe(true);
    });

    it("uses consistent format for cache keys", () => {
      const key1 = "destinyChartData_1990-01-01_12:00";
      const key2 = "destinyChartData_1990-01-02_12:00";

      expect(key1).toMatch(/^destinyChartData_/);
      expect(key2).toMatch(/^destinyChartData_/);
  ...
  describe("clearChartCache", () => {
    ...
    it("handles Redis clear failure gracefully", async () => {
      ...
      await expect(
        clearChartCache(testBirthDate, testBirthTime)
      ).resolves.not.toThrow();
    });
  });
