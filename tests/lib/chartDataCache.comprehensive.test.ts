...
describe('chartDataCache', () => {
  ...
  describe('saveChartData', () => {
    it('should save chart data to session storage', () => {
      ...
      const chartDataCall = sessionStorageMock.setItem.mock.calls.find((call) =>
        call[0].startsWith('destinyChartData_')
      )
  ...
  describe('loadChartData', () => {
    ...
    it('should load cached data when valid', () => {
      ...
      // 캐시 키는 좌표까지 포함(= birthKey).
      mockSessionStorage[`destinyChartData_${birthKey}`] = JSON.stringify({
        saju: sajuData,
        timestamp: Date.now(),
        birthKey,
      })
  ...
  describe('cleanupExpiredCache', () => {
    ...
    it('should remove malformed entries', () => {
      mockSessionStorage['destinyChartDataKeys'] = JSON.stringify(['destinyChartData_malformed'])
