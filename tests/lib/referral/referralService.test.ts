...
describe('Referral Service with mocked Prisma', () => {
  ...
  describe('linkReferrer', () => {
    ...
    it('successfully links referrer', async () => {
      ...
      // Linking is now an atomic updateMany (where referrerId:null) returning
      // a count; count>0 means this request won the race.
    ...
    it('is idempotent — rejects when user already has a referrer (no double credit)', async () => {
      const { prisma } = await import('@/lib/db/prisma')
      const { linkReferrer } = await import('@/lib/referral/referralService')
      const { addBonusCredits } = await import('@/lib/credits/creditService')
      ...
      // Already linked: the atomic updateMany matches no rows (referrerId is
      // no longer null), so it reports count:0 → 'already_linked'.
