...
describe('generateAIPremiumReport', () => {
  it('generates all 10 sections', async () => {
    ...
    expect((result.evidenceSummary?.topInsights || []).length).toBeGreaterThan(0)
    expect((result.evidenceSummary?.drivers || []).length).toBeGreaterThan(0)
    expect((result.evidenceSummary?.cautions || []).length).toBeGreaterThan(0)
    expect(
