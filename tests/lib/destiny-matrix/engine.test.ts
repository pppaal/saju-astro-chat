  ...
  it("strength points are sorted by score descending", () => {
    ...
    if (result.summary.strengthPoints.length > 1) {
      for (let i = 1; i < result.summary.strengthPoints.length; i++) {
        expect(
          result.summary.strengthPoints[i - 1].cell.interaction.score
        ).toBeGreaterThanOrEqual(result.summary.strengthPoints[i].cell.interaction.score);
      }
    }
  });
  ...
  it("caution points are sorted by score ascending", () => {
    ...
    if (result.summary.cautionPoints.length > 1) {
      for (let i = 1; i < result.summary.cautionPoints.length; i++) {
        expect(
          result.summary.cautionPoints[i - 1].cell.interaction.score
        ).toBeLessThanOrEqual(result.summary.cautionPoints[i].cell.interaction.score);
      }
    }
  });
});
...
describe("getSibsinPlanetInteraction", () => {
  const sibsinList: SibsinKind[] = ["비견", "겁재", "식신", "상관", "편재", "정재", "편관", "정관", "편인", "정인"];
...
describe("getStageHouseInteraction", () => {
  const stages: TwelveStageStandard[] = ["장생", "목욕", "관대", "임관", "왕지", "쇠", "병", "사", "묘", "절", "태", "양"];
