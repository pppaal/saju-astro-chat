...
vi.mock('@/lib/destiny-map/calendar/temporal-scoring', () => ({
  ...
  scoreYongsinFitForDate: vi.fn(() => ({ active: true, element: '수', bonus: 10 })),
  scoreGeokgukFitForDate: vi.fn(() => ({ pattern: '정관격', strength: 'strong' })),
...
describe('Date Analysis Saju', () => {
  describe('analyzeSajuFactors', () => {
    ...
    it('returns complete analysis result structure', () => {
      const result = analyzeSajuFactors(mockSajuProfile as any, new Date(2025, 0, 15), mockGanzhi)
  ...
  describe('analyzeJijanggan', () => {
    ...
    it('returns empty array when relations is undefined', () => {
      const result = analyzeJijanggan({ stem: '甲', branch: '子' }, undefined)
  ...
  describe('analyzeBranchRelations', () => {
    it('returns empty arrays when dayBranch is undefined', () => {
      const result = analyzeBranchRelations(undefined, { stem: '甲', branch: '子' }, '목', {
        generatedBy: '수',
        controlledBy: '토',
      })
    ...
    it('returns empty arrays when relations is undefined', () => {
      const result = analyzeBranchRelations('子', { stem: '甲', branch: '午' }, '목', undefined)
    ...
    describe('삼합 (Samhap/Triple Harmony)', () => {
      it('detects positive samhap when element matches', () => {
        const result = analyzeBranchRelations('寅', { stem: '甲', branch: '卯' }, '목', {
          generatedBy: '수',
          controlledBy: '금',
        })
      ...
      it('detects samhap with generatedBy element', () => {
        const result = analyzeBranchRelations('亥', { stem: '甲', branch: '子' }, '목', {
          generatedBy: '수',
          controlledBy: '금',
        })
      ...
      it('detects negative samhap when controlledBy element', () => {
        const result = analyzeBranchRelations('申', { stem: '甲', branch: '酉' }, '목', {
          generatedBy: '수',
          controlledBy: '금',
        })
    ...
    describe('육합 (Yukhap/Six Harmony)', () => {
      it('detects yukhap between 子 and 丑', () => {
        const result = analyzeBranchRelations('子', { stem: '甲', branch: '丑' }, '수', {
          generatedBy: '금',
          controlledBy: '토',
        })
    ...
    describe('충 (Chung/Clash)', () => {
      it('detects chung between 子 and 午', () => {
        const result = analyzeBranchRelations('子', { stem: '甲', branch: '午' }, '수', {
          generatedBy: '금',
          controlledBy: '토',
        })
    ...
    describe('형 (Xing/Punishment)', () => {
      it('detects xing between 寅 and 巳', () => {
        const result = analyzeBranchRelations('寅', { stem: '甲', branch: '巳' }, '목', {
          generatedBy: '수',
          controlledBy: '금',
        })
    ...
    describe('해 (Hai/Harm)', () => {
      it('detects hai between 子 and 未', () => {
        const result = analyzeBranchRelations('子', { stem: '甲', branch: '未' }, '수', {
          generatedBy: '금',
          controlledBy: '토',
        })
      ...
      it('detects hai between 酉 and 戌', () => {
        const result = analyzeBranchRelations('酉', { stem: '辛', branch: '戌' }, '금', {
          generatedBy: '토',
          controlledBy: '화',
        })
