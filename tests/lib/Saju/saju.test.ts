...
describe('saju - Core Calculation', () => {
  describe('calculateSajuData', () => {
    ...
    it('should handle different genders for daeWoon direction', () => {
      const maleResult = calculateSajuData('1990-05-15', '10:30', 'male', 'solar', 'Asia/Seoul')

      const femaleResult = calculateSajuData('1990-05-15', '10:30', 'female', 'solar', 'Asia/Seoul')

      // 같은 연도에서 남녀의 대운 방향이 다를 수 있음
      expect(
        maleResult.daeWoon.isForward !== femaleResult.daeWoon.isForward ||
          maleResult.daeWoon.isForward === femaleResult.daeWoon.isForward
      ).toBe(true)
    })

    it('should handle different timezones', () => {
      const seoulResult = calculateSajuData('1990-05-15', '10:30', 'male', 'solar', 'Asia/Seoul')

      const tokyoResult = calculateSajuData('1990-05-15', '10:30', 'male', 'solar', 'Asia/Tokyo')
    ...
    it('should throw error for invalid date', () => {
      expect(() =>
        calculateSajuData('invalid-date', '10:30', 'male', 'solar', 'Asia/Seoul')
      ).toThrow()
    })
  })
