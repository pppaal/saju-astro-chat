...
describe('unseAnalysis - 대운/세운 종합 분석', () => {
  const testPillars = createPillars(['甲', '子'], ['丙', '寅'], ['戊', '辰'], ['庚', '午'])
  ...
  describe('analyzeUnseComprehensive', () => {
    ...
    it('analyzes twelve stages correctly', () => {
      ...
      expect(['rising', 'peak', 'declining', 'dormant']).toContain(result.twelveStage.energy)
    })
    ...
    it('detects 육합 interaction', () => {
      // 子丑 육합 테스트
      const pillarsWithZi = createPillars(['甲', '子'], ['丙', '寅'], ['戊', '辰'], ['庚', '午'])
      ...
      const yearInteraction = result.interactions.find((i) => i.target === '년주')
      const hasYukhap = yearInteraction?.interactions.some((int) => int.type === '육합')
      expect(hasYukhap).toBe(true)
    })

    it('detects 충 interaction', () => {
      // 子午 충 테스트
      const pillarsWithZi = createPillars(['甲', '子'], ['丙', '寅'], ['戊', '辰'], ['庚', '申'])
      ...
      const yearInteraction = result.interactions.find((i) => i.target === '년주')
      const hasChung = yearInteraction?.interactions.some((int) => int.type === '충')
  ...
  describe('analyzeDaeunPeriod', () => {
    it('returns DaeunPeriodAnalysis structure', () => {
      const result = analyzeDaeunPeriod('甲', '寅', 5, 0, testPillars, [], [], 1990)
    ...
    it('passes yongsin and kisin to analysis', () => {
      ...
      const result = analyzeDaeunPeriod('甲', '寅', 5, 0, testPillars, yongsin, kisin)
  ...
  describe('analyzeSpecificYear', () => {
    ...
    it('returns YearDetailAnalysis structure', () => {
      const result = analyzeSpecificYear(2024, testPillars, 1990, daeunList, [], [])
    ...
    it('finds matching daeun for the year', () => {
      const result = analyzeSpecificYear(2020, testPillars, 1990, daeunList, [], [])
  ...
  describe('analyzeMultiYearTrend', () => {
    ...
    it('returns MultiYearTrend structure', () => {
      const result = analyzeMultiYearTrend(2020, 2025, testPillars, 1990, daeunList, [], [])
    ...
    it('calculates average score correctly', () => {
      const result = analyzeMultiYearTrend(2020, 2022, testPillars, 1990, daeunList, [], [])

      const manualAvg =
        result.years.reduce((sum, y) => sum + y.combinedScore, 0) / result.years.length
      expect(result.averageScore).toBeCloseTo(manualAvg, 1)
    })

    it('determines overall trend', () => {
      const result = analyzeMultiYearTrend(2020, 2030, testPillars, 1990, daeunList, [], [])

      expect(['ascending', 'descending', 'stable', 'fluctuating']).toContain(result.overallTrend)
    })
  ...
  describe('edge cases', () => {
    ...
    it('handles single year trend analysis', () => {
      ...
      const result = analyzeMultiYearTrend(2024, 2024, testPillars, 1990, daeunList, [], [])
  ...
  describe('twelve stages', () => {
    ...
    it('returns appropriate energy based on stage', () => {
      const pillars = createPillars(['甲', '子'], ['丙', '寅'], ['甲', '辰'], ['庚', '午'])
