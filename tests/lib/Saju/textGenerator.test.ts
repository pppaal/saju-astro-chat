...
describe('textGenerator', () => {
  ...
  describe('generateSibsinText', () => {
    ...
    it('should include theme in keywords', () => {
      ...
      expect(
        result.keywords.some((k) => k.includes('질서') || k.includes('명예') || k === '정관')
      ).toBe(true)
    })
  ...
  describe('generateStrengthAdvice', () => {
    ...
    it('should include strength level in keywords', () => {
      ...
      expect(
        result.keywords.some(
          (k) => strengthLevels.includes(k as (typeof strengthLevels)[number]) || k === '금'
        )
      ).toBe(true)
    })
    ...
    it('should recommend yongsin activities', () => {
      ...
      expect(
        result.details.some((d) => d.includes('연구') || d.includes('무역') || d.includes('물류'))
      ).toBe(true)
    })
  })
  ...
  describe('Style variations', () => {
    ...
    it('should produce different outputs for different styles', () => {
      const results = styles.map((style) => generateElementText('목', { style }))
