...
describe('cellsToYearlyDates (v2-native adapter)', () => {
  it('matches the (all-v2) migration golden grade + displayScore for every date (profile B)', async () => {
    ...
    const goldenDates: Record<string, [number, number, number, number]> =
  ...
  it('produces complete, well-formed date objects', async () => {
    const cells = await buildYearCells()
    const dates = cellsToYearlyDates(cells, { lang: 'ko' })
