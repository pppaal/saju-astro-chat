...
describe('ichingWisdom', () => {
  ...
  describe('generateSituationalAdvice', () => {
    ...
    it('includes changing line interpretation when provided', () => {
      ...
      // tw() returns the key "changingLineNote" in test env without loaded locale
      expect(advice).toContain('changingLineNote')
    })
  ...
  describe('generateDailyWisdom', () => {
    it('generates daily wisdom message', () => {
      ...
      // tw() returns keys as fallback in test env
      expect(wisdom).toContain('dailyWisdomTitle')
      expect(wisdom).toContain('건괘')
      expect(wisdom).toContain('dailyCore')
      expect(wisdom).toContain('dailyAdvice')
    })

    it('includes day-specific theme via dayNames key', () => {
      const monday = new Date(2024, 0, 1) // Monday
      const wisdom = generateDailyWisdom(1, monday)

      // twArray() returns empty array in test env, so dayName will be ""
      // But the wisdom string still contains the template structure
  ...
  describe('analyzeHexagramRelationshipWisdom', () => {
    ...
    it('identifies 천-지 complementary relationship', () => {
      ...
      // tw() returns key "compatibilityComplementary" in test env
      expect(result.compatibility).toContain('compatibilityComplementary')
    })
    ...
    it('handles invalid hexagram', () => {
      ...
      // tw() returns key "cannotAnalyze" in test env
      expect(result.compatibility).toBe('cannotAnalyze')
    })
  })
  ...
  describe('generatePeriodicWisdom', () => {
    it('generates yearly wisdom', () => {
      ...
      // tw() returns keys in test env; fortuneTitle contains "periodYear" key result
      expect(wisdom).toContain('fortuneTitle')
      expect(wisdom.length).toBeGreaterThan(0)
    })
    ...
    it('generates monthly wisdom', () => {
      ...
      // tw() returns key "fortuneTitle" in test env
      expect(wisdom).toContain('fortuneTitle')
    })
    ...
    it('includes opportunities and warnings sections', () => {
      const wisdom = generatePeriodicWisdom(1, 'monthly', 1)

      // tw() returns keys; section headers use periodOpportunities and periodCautions
      expect(wisdom).toContain('periodOpportunities')
      expect(wisdom).toContain('periodCautions')
    })
  ...
  describe('deepWisdomAnalysis', () => {
    ...
    it('personalizes advice for male user', () => {
      ...
      // tw() returns key "genderMale" in test env
      expect(result.personalizedAdvice).toContain('genderMale')
    })

    it('personalizes advice for female user', () => {
      const result = deepWisdomAnalysis(1, { gender: 'F' })

      // tw() returns key "genderFemale" in test env
      expect(result.personalizedAdvice).toContain('genderFemale')
    })

    it('adds birth year advice when provided', () => {
      const result = deepWisdomAnalysis(1, { birthYear: 1990 })

      // tw() returns key like "branchAdvice10" in test env (1990-4 % 12 = 10)
      expect(result.personalizedAdvice).toContain('branchAdvice')
    })

    it('handles invalid hexagram', () => {
      const result = deepWisdomAnalysis(0)

      // tw() returns key "noHexagramInfo" in test env
      expect(result.personalizedAdvice).toContain('noHexagramInfo')
    })
    ...
    it('action plan has multiple steps', () => {
      ...
      // tw() returns key "actionStep1" in test env
      expect(result.actionPlan[0]).toContain('actionStep1')
    })
  })
})
