...
describe('Advanced I-Ching Core', () => {
  ...
  describe('analyzeTrigramInteraction', () => {
    ...
    it('should provide dynamic meaning', () => {
      ...
      // In test env, ta() returns the translation key as fallback
      // For "건건", the specific key "trigramDynamic_건건" is looked up first,
      // then falls back to "trigramDynamicDefault"
      expect(result.dynamicMeaning.length).toBeGreaterThan(0)
    })
  ...
  describe('analyzeYaoPositions', () => {
    ...
    it('should correctly identify position names', () => {
      ...
      // In test env without loaded locale, ta() returns the key as fallback
      const expectedNames = ['yaoName1', 'yaoName2', 'yaoName3', 'yaoName4', 'yaoName5', 'yaoName6']
      for (let i = 0; i < 6; i++) {
  ...
  describe('analyzeChangingLines', () => {
    ...
    it('should handle no changing lines', () => {
      ...
      // In test env without loaded locale, ta() returns the key string
      expect(result.interpretation).toContain('changingNone')
    })
    ...
    it('should handle all lines changing (Qian to Kun)', () => {
      ...
      // In test env, ta() returns the key "changingSixYongJiu"
      expect(result.interpretation).toContain('changingSixYongJiu')
    })
    ...
    it('should handle all lines changing (Kun to Qian)', () => {
      ...
      // In test env, ta() returns the key "changingSixYongYuk"
      expect(result.interpretation).toContain('changingSixYongYuk')
    })

    it('should provide interpretation for different change counts', () => {
      // In test env, ta() returns the key strings as fallback
      const testCases = [
        { lines: [1], contains: 'changingSingle' },
        { lines: [1, 2], contains: 'changingDouble' },
        { lines: [1, 2, 3], contains: 'changingTriple' },
        { lines: [1, 2, 3, 4], contains: 'changingQuadruple' },
        { lines: [1, 2, 3, 4, 5], contains: 'changingQuintuple' },
      ]
  ...
  describe('performComprehensiveHexagramAnalysis', () => {
    ...
    it('should provide overall insight', () => {
      ...
      // In test env, ta() returns the key "overallInsight"
      expect(result.overallInsight).toContain('overallInsight')
    })
  ...
  describe('compareHexagrams', () => {
    it('should identify identical hexagrams', () => {
      ...
      // In test env, ta() returns the key "compareSame"
      expect(result.relationship).toBe('compareSame')
    })

    it('should identify sangban (opposite) relationship', () => {
      const result = compareHexagrams('111111', '000000')

      // In test env, ta() returns the key "compareSangban"
      expect(result.relationship).toBe('compareSangban')
    })
    ...
    it('should identify do (reversed) relationship', () => {
      ...
      // In test env, ta() returns the key "compareDo"
      expect(result.relationship).toBe('compareDo')
    })
    ...
    it('should identify similar hexagrams (5+ matching)', () => {
      ...
      // In test env, ta() returns the key "compareSimilar"
      expect(result.relationship).toBe('compareSimilar')
    ...
    it('should identify opposing hexagrams (1 or fewer matching)', () => {
      ...
      // In test env, ta() returns the key "compareOpposite"
      expect(result.relationship).toBe('compareOpposite')
    })
    ...
    it('should provide common energy description', () => {
      ...
      // In test env, ta() returns the key "compareCommon"
      expect(result.commonEnergy).toContain('compareCommon')
    })
    ...
    it('should provide difference analysis', () => {
      ...
      // In test env, ta() returns the key "compareDifference"
      expect(result.differenceAnalysis).toContain('compareDifference')
    })
  })
