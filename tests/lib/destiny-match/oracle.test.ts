...
} from '@/lib/destiny-match/oracle/tarotDraw';
import {
  getOracleReading,
...
vi.mock('@/lib/prediction/specificDateEngine', async () => {
  const actual =
    await vi.importActual<typeof import('@/lib/prediction/specificDateEngine')>(
...
describe('drawRelationshipSpread', () => {
  ...
  it('returns three positioned cards', () => {
    ...
    expect(spread.cards.map((c) => c.position)).toEqual([
      'present',
      'potential',
      'advice',
    ]);
...
describe('getOracleReading', () => {
  ...
  it('produces identical readings for two callers on the same day', async () => {
    ...
    expect(a.auspicious.dates.map((d) => d.date)).toEqual(
      b.auspicious.dates.map((d) => d.date),
    );
  });
