...
describe('zodiacalReleasing — Fortune ZR + L2/L3/L4 sub-period + Peak/Loosing-of-Bond', () => {
  ...
  describe('annotateZRMarkers — Peak / Loosing-of-Bond on L1', () => {
    it('flags 1/4/7/10th-from-start as peak; 7th as loosing-of-bond', () => {
      ...
      // 7th = Libra → loosing of bond AND peak
      const libra = marked.find((p) => p.sign === 'Libra')!
      expect(libra.isLoosingOfTheBond).toBe(true)
  ...
  describe('L2 sub-period — months scale', () => {
    ...
    it('L2 peak/loosing marked relative to parent L1 sign', () => {
      ...
      expect(aries.isPeak).toBe(true)
      // Whether Libra L2 actually exists within L1 duration depends on month
      // accumulation. Aries L1 = 15 years ≈ 180 months; total order goes from
      // Aries (15m) Taurus(8m) Gemini(20m) … so Libra is reached.
