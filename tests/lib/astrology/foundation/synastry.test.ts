...
describe('synastry', () => {
  describe('calculateSynastry', () => {
    it('should calculate synastry between two charts', () => {
      const chartA = createChart([createPlanet('Sun', 45), createPlanet('Moon', 120)])
      const chartB = createChart([createPlanet('Sun', 75), createPlanet('Moon', 180)])
    ...
    it('house overlay returns UNKNOWN_HOUSE(0), not a fabricated 1, when the other chart has malformed houses', () => {
      // 회귀: 예전엔 getHouseForLongitude 가 (a) no-match/결손 시 1 을 날조하고
      // (b) houses[i] 를 길이 가드 없이 인덱싱해 12칸 미만 차트에서 throw 했다.
      // 이제 엔진 SSOT(matchHouseForCusps)에 위임 → 결손 차트는 UNKNOWN_HOUSE(0).
      const chartA = createChart([createPlanet('Sun', 45)])
      const chartB = {
        ...createChart([createPlanet('Moon', 200)]),
        houses: createChart([]).houses.slice(0, 3), // 3칸만(결손)
      }

      const result = calculateSynastry({ chartA, chartB })
      const overlay = result.houseOverlaysAtoB.find((o) => o.planet === 'Sun')
      expect(overlay?.inHouse).toBe(0) // 날조된 1 이 아니라 UNKNOWN_HOUSE
    })

    it('should include harmony and tension scores', () => {
      const chartA = createChart([createPlanet('Sun', 0), createPlanet('Moon', 60)])
      const chartB = createChart([createPlanet('Sun', 120), createPlanet('Moon', 90)])
    ...
    it('should calculate house overlays A to B', () => {
      const chartA = createChart([createPlanet('Sun', 45), createPlanet('Moon', 100)])
    ...
    it('should calculate house overlays B to A', () => {
      ...
      const chartB = createChart([createPlanet('Venus', 45), createPlanet('Mars', 100)])
    ...
    it('should respect aspect orbs', () => {
      ...
      const validAspects = result.aspects.filter(
        (a) =>
    ...
    it('should handle empty charts', () => {
      ...
      const nonAngleAspects = result.aspects.filter(
        (a) =>
          !['Ascendant', 'MC'].includes(a.from.name) && !['Ascendant', 'MC'].includes(a.to.name)
      )
  ...
  describe('findSynastryAspects', () => {
    it('should find all synastry aspects', () => {
      const chartA = createChart([createPlanet('Sun', 0), createPlanet('Moon', 60)])
      const chartB = createChart([createPlanet('Venus', 120), createPlanet('Mars', 180)])
    ...
    it('should filter by aspect types when provided', () => {
      const chartA = createChart([createPlanet('Sun', 0)])
      const chartB = createChart([createPlanet('Moon', 120), createPlanet('Venus', 180)])
    ...
    it('should return sorted aspects', () => {
      const chartA = createChart([createPlanet('Sun', 0), createPlanet('Moon', 60)])
      const chartB = createChart([createPlanet('Venus', 5), createPlanet('Mars', 63)])
  ...
  describe('edge cases', () => {
    ...
    it('should calculate total score correctly', () => {
      const chartA = createChart([createPlanet('Sun', 0), createPlanet('Moon', 60)])
      const chartB = createChart([createPlanet('Venus', 120), createPlanet('Mars', 90)])
