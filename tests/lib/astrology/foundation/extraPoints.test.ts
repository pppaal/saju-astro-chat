...
import {
  calculateNatalChart,
  toChart,
} from '../../../../src/lib/astrology/foundation/astrologyService'
...
describe('extraPoints', () => {
  ...
  describe('calculateVertex', () => {
    ...
    it('should vary with location', () => {
      const houseCusps = testChart.houses.map((h) => h.cusp)
      const vertex1 = calculateVertex(testJd, 40.7128, -74.006, houseCusps) // New York
  ...
  describe('isNightChart', () => {
    ...
    it('falls back to longitude horizon check when house is UNKNOWN (0)', () => {
      // 회귀: sun.house 가 UNKNOWN_HOUSE(0) 면 예전엔 무조건 false(주간)로 떨어져
      // sect 가 뒤집혔다. 이제 태양·ASC 경도로 폴백 — ASC 순행 180°(지평선 아래)면 야간.
      // 태양이 ASC 바로 뒤(10°) → 지평선 아래 → 야간.
      expect(isNightChart(0, 10, 0)).toBe(true)
      // 태양이 ASC+200° → 지평선 위 → 주간.
      expect(isNightChart(0, 200, 0)).toBe(false)
      // 경계: 정확히 180°(DESC)는 주간 쪽(>=180).
      expect(isNightChart(0, 180, 0)).toBe(false)
      // wrap-around: ASC=350, 태양=10 → 상대 20° → 야간.
      expect(isNightChart(0, 10, 350)).toBe(true)
      // 경도 정보가 없으면 기존 동작(주간) 유지.
      expect(isNightChart(0)).toBe(false)
    })
  })
  ...
  describe('extendChartWithExtraPoints', () => {
    ...
    it('should throw error if Sun or Moon missing', () => {
      ...
      expect(() =>
        extendChartWithExtraPoints(chartWithoutSun, testJd, testInput.latitude, testInput.longitude)
      ).toThrow()
    })
  })
