...
describe('fixedStars', () => {
  describe('correctForPrecession', () => {
    ...
    it('should apply consistent precession rate', () => {
      ...
      const diff10years =
        correctForPrecession(baseLongitude, 2010) - correctForPrecession(baseLongitude, 2000)
      const diff20years =
        correctForPrecession(baseLongitude, 2020) - correctForPrecession(baseLongitude, 2000)
  ...
  describe('getAllFixedStars', () => {
    ...
    // 회귀 가드: 아래 별들은 값이 자기 주석/실제 J2000 tropical 위치보다 정확히
    // 한 사인(30°) 낮게 박혀 있어(sign-off-by-one) orb 1° 합을 놓치고 한 사인
    // 떨어진 행성에 가짜 합을 냈다. tropical 사인이 다시 어긋나지 않도록 잠근다.
    // (사인 index = floor(longitude/30): Leo=4, Scorpio=7, Aquarius=10)
    it('key stars sit in their documented tropical sign (no sign-off-by-one drift)', () => {
      const signIndex = (lon: number) => Math.floor(lon / 30)
      const expected: Record<string, number> = {
        Dubhe: 4, // Leo
        Acubens: 4, // Leo
        Zubeneschamali: 7, // Scorpio
        Zubenelgenubi: 7, // Scorpio
        'Deneb Algedi': 10, // Aquarius
        Regulus: 4, // Leo (변경하지 않은 대조군)
      }
      for (const [name, sign] of Object.entries(expected)) {
        const star = getFixedStar(name)
        expect(star, `${name} 누락`).toBeDefined()
        expect(signIndex(star!.longitude), `${name} @${star!.longitude}° 사인 어긋남`).toBe(sign)
      }
    })
  })
  ...
  describe('Star database integrity', () => {
    ...
    it('중복 항성 항목이 없다 (이름 유니크 + 동일 지점 별자리 모순 없음)', () => {
      ...
      // 진짜 중복(같은 별의 이름 별칭)만 잡는다: 경도·밝기가 거의 동일하면
      // 같은 물리적 별을 두 번 넣은 것이다. 단순 경도 근접만으로는 판정하지
      // 않는다 — 서로 다른 실제 별이 황도 경도상 매우 가까이(별자리 경계를 넘어)
      // 놓일 수 있기 때문(예: Deneb Algedi=Delta Capricorni 23°33' Aquarius 와
      // Sadalsuud=Beta Aquarii 23°24' Aquarius 는 0.08° 차의 서로 다른 별).
      for (let i = 0; i < stars.length; i++) {
        for (let j = i + 1; j < stars.length; j++) {
          const d = Math.abs(stars[i].longitude - stars[j].longitude)
          const nearLon = Math.min(d, 360 - d)
          const nearMag = Math.abs(stars[i].magnitude - stars[j].magnitude)
          const looksLikeSameStar = nearLon < 0.02 && nearMag < 0.02
          expect(
            looksLikeSameStar,
            `중복 의심: ${stars[i].name} / ${stars[j].name} (Δlon=${nearLon.toFixed(3)}, Δmag=${nearMag.toFixed(2)})`
          ).toBe(false)
        }
      }
    })
