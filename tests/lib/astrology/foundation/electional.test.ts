...
describe('findBestDates', () => {
  ...
  it('should include date, score, and interpretation for each result', () => {
    const charts = [{ date: new Date('2025-01-15'), chart: createMockChart() }]
    const result = findBestDates('marriage', new Date('2025-01-15'), new Date('2025-01-20'), charts)
...
// ============================================================
// Determinism: 택일 점수는 서버 타임존에 의존하면 안 된다.
// 회귀: calculatePlanetaryHour 가 date.getDay()(로컬 요일)를, analyzeElection 의
//   기본 일출/일몰이 setHours(로컬 시각)를 써서 같은 UTC 인스턴트라도 배포 리전에
//   따라 행성시/요일 지배성이 달라져 점수가 비결정적이었다. 이제 UTC 기준으로 고정.
// ============================================================
describe('electional determinism (timezone-independent)', () => {
  ...
  it('planetary day ruler follows UTC weekday, not server local weekday', () => {
    // 2025-01-12T02:30:00Z = UTC 일요일 → 지배성 Sun.
    // 미국 서부(UTC-8)에선 로컬 토요일 18:30 → 옛 코드라면 Saturn 으로 오판.
    const instant = new Date('2025-01-12T02:30:00Z')
    const sunrise = new Date('2025-01-12T00:00:00Z')
    const sunset = new Date('2025-01-12T12:00:00Z')

    process.env.TZ = 'America/Los_Angeles'
    const la = calculatePlanetaryHour(instant, 0, sunrise, sunset)
    process.env.TZ = 'UTC'
    const utc = calculatePlanetaryHour(instant, 0, sunrise, sunset)

    expect(la.dayRuler).toBe('Sun')
    expect(la.dayRuler).toBe(utc.dayRuler)
  })

  it('analyzeElection total score is identical across server timezones (default sunrise/sunset)', () => {
    const chart = createMockChart()
    const instant = new Date('2025-01-12T02:30:00Z')

    process.env.TZ = 'America/Los_Angeles'
    const west = analyzeElection(chart, 'marriage', instant)
    process.env.TZ = 'Asia/Seoul'
    const east = analyzeElection(chart, 'marriage', instant)

    expect(west.score.total).toBe(east.score.total)
    expect(west.score.specialConsiderations).toBe(east.score.specialConsiderations)
  })
})
