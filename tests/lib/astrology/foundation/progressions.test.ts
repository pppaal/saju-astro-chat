...
function createTestChart(): Chart {
  ...
  return {
    planets: [createPlanet('Sun', 85), createPlanet('Moon', 120), createPlanet('Mercury', 95)],
...
describe('progressions', () => {
  describe('calculateSecondaryProgressions', () => {
    ...
    it('targetDate 없으면 주입된 now 로 폴백 날짜를 결정한다(결정적 — ENGINE-AUDIT)', async () => {
      const pinned = new Date('2031-09-17T00:00:00Z')
      const a = await calculateSecondaryProgressions(null, pinned)
      const b = await calculateSecondaryProgressions(null, pinned)
      expect(a.progressedDate).toBe('2031-09-17')
      expect(b.progressedDate).toBe('2031-09-17') // 시계 미사용 → 호출마다 동일
    })
  })
