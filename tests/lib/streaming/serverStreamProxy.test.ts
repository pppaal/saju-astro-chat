...
describe('Server Stream Proxy', () => {
  ...
  describe('createSSEStreamProxy', () => {
    ...
    it('should handle stream read errors', async () => {
      ...
      // Create a mock source Response whose body reader rejects on read().
      // This avoids the unhandled rejection that occurs when throwing
      // inside ReadableStream start() or pull().
      const errorReader = {
      ...
      const sourceResponse = new Response(null)
      Object.defineProperty(sourceResponse, 'body', { value: mockBody })
      ...
      const reader = response.body?.getReader()
      if (reader) {
        // Read until the stream closes (the proxy catches the error and closes)
        while (true) {
          const { done } = await reader.read()
          if (done) break
        }
