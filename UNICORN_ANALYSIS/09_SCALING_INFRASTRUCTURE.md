...
export const CACHE_KEYS = {
  SAJU: (userId: string, birthDate: string) =>
    `saju:v1:${userId}:${birthDate}`,
  TAROT: (readingId: string) =>
    `tarot:v1:${readingId}`,
  COMPATIBILITY: (ids: string[]) =>
    `compat:v1:${ids.sort().join(':')}`,
  CHART: (birthData: string) =>
    `chart:v1:${birthData}`,
  DAILY_FORTUNE: (userId: string, date: string) =>
    `fortune:v1:${userId}:${date}`,
};
```
