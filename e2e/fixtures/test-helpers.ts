...
export class TestHelpers {
  ...
  async fillBirthInfo(birthDate: string, birthTime: string, city: string) {
    ...
    const dateInput = this.page
      .locator(
        'input[type="date"], input[name*="date"], input[placeholder*="날짜"], input[placeholder*="date"]'
      )
      .first()
    ...
    const timeInput = this.page
      .locator(
        'input[type="time"], input[name*="time"], input[placeholder*="시간"], input[placeholder*="time"]'
      )
      .first()
    ...
    const cityInput = this.page
      .locator(
        'input[name*="city"], input[name*="location"], input[placeholder*="도시"], input[placeholder*="city"]'
      )
      .first()
    if (await cityInput.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const firstOption = this.page
        .locator('[role="option"], .suggestion-item, .city-option')
        .first()
  ...
  /**
   * Wait until body text contains at least one of the keywords
   */
  async waitForAnyText(keywords: string[], timeout = 12000) {
  ...
  /**
   * Fetch My History endpoint and unwrap middleware payload when authenticated
   */
  async fetchMyHistory(service?: string): Promise<{
    status: number
    history: Array<{ date: string; records: Array<Record<string, unknown>> }> | null
  }> {
    const suffix = service ? `?service=${encodeURIComponent(service)}` : ''
    const response = await this.page.request.get(`/api/me/history${suffix}`, { timeout: 30000 })
