...
test.describe('Premium Subscription Flow', () => {
  ...
  test('should initiate Stripe checkout flow', async ({ page }) => {
    ...
    const subscribeButton = page
      .locator('button:has-text("구독"), button:has-text("Subscribe"), button:has-text("시작")')
      .first()
  ...
  test('should show correct pricing in user currency', async ({ page }) => {
    ...
    const bodyText = await page.textContent('body')
    const hasPricing =
      /\$\d+/.test(bodyText || '') || /₩\d+/.test(bodyText || '') || /\d+원/.test(bodyText || '')
  ...
  test('should show premium features in navigation for premium users', async ({ page }) => {
    ...
    if (isPremium) {
      const bodyText = await page.textContent('body')
      const hasPremiumFeatures = bodyText?.includes('프리미엄') || bodyText?.includes('Premium')
  ...
  test('should handle subscription downgrade', async ({ page }) => {
    ...
    const downgradeButton = page
      .locator('button:has-text("다운그레이드"), button:has-text("Downgrade")')
      .first()
  ...
  test('should show billing history for premium users', async ({ page }) => {
    ...
    if (isPremium) {
      ...
      const hasBillingHistory =
        bodyText?.includes('결제') || bodyText?.includes('billing') || bodyText?.includes('청구')
