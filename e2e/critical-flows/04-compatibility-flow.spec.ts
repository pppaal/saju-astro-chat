...
test.describe('Compatibility Analysis Flow', () => {
  ...
  test('should complete two-person compatibility analysis', async ({ page }) => {
    ...
    if (inputCount >= 6) {
      ...
      const submitButton = page
        .locator('button[type="submit"], button:has-text("분석"), button:has-text("궁합")')
        .first()
  ...
  test('should navigate to compatibility insights page', async ({ page }) => {
    ...
    const bodyText = await page.textContent('body')
    const hasInsights =
      bodyText?.includes('insight') || bodyText?.includes('분석') || bodyText?.includes('궁합')
  ...
  test('should show different compatibility aspects', async ({ page }) => {
    ...
    if (await submitButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const bodyText = await page.textContent('body')
      // Check for compatibility aspects - more flexible matching
      const hasMultipleAspects =
        bodyText?.includes('성격') ||
        bodyText?.includes('personality') ||
        bodyText?.includes('소통') ||
        bodyText?.includes('communication') ||
        bodyText?.includes('관계') ||
        bodyText?.includes('궁합') ||
        bodyText?.includes('결과') ||
        bodyText!.length > 200
  ...
  test('should enable compatibility chat counselor', async ({ page }) => {
    ...
    const bodyText = await page.textContent('body')
    const hasCounselor =
      bodyText?.includes('상담') || bodyText?.includes('counselor') || bodyText?.includes('채팅')
  ...
  test('should display visual compatibility chart', async ({ page }) => {
    ...
    if (await submitButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const hasChart =
        (await page.locator(".chart, [class*='chart'], [data-testid*='chart']").count()) > 0
