...
test.describe('Profile Management Flow', () => {
  ...
  test('should allow updating user name', async ({ page }) => {
    ...
    if (await nameInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const saveButton = page
        .locator('button[type="submit"], button:has-text("저장"), button:has-text("Save")')
        .first()
  ...
  test('should allow updating birth information', async ({ page }) => {
    ...
    if (await dateInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      if (await saveButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        const bodyText = await page.textContent('body')
        const hasSuccess =
          bodyText?.includes('저장') || bodyText?.includes('완료') || bodyText?.includes('업데이트')
  ...
  test("should display user's circle/social connections", async ({ page }) => {
    ...
    const bodyText = await page.textContent('body')
    const hasCircle =
      bodyText?.includes('circle') || bodyText?.includes('친구') || bodyText?.includes('인연')
  ...
  test('should allow user to delete account', async ({ page }) => {
    ...
    const deleteButton = page
      .locator('button:has-text("삭제"), button:has-text("탈퇴"), button:has-text("Delete")')
      .first()
  ...
  test('should handle profile update errors gracefully', async ({ page }) => {
    ...
    if (await nameInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      if (await saveButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        const bodyText = await page.textContent('body')
        const hasValidation =
          bodyText?.includes('필수') || bodyText?.includes('required') || bodyText?.includes('입력')
