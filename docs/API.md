...
| 엔드포인트 | 평균 응답 시간 | 설명 |
|-----------|--------------|------|
| `/api/saju` | < 3초 | RAG 병렬 처리로 3배 개선 |
| `/api/tarot` | < 2초 | AI 생성 포함 |
| `/api/destiny-map` | < 5초 | 종합 분석 |
| `/api/calendar` | < 1초 | Redis 캐싱 활용 |

### RAG 시스템 최적화 (2026년 1월)

**개선 내역**:
- ✅ **병렬 처리 구현**: 순차 1500ms → 병렬 500ms (**3배 향상**)
- ✅ **Thread-safe 실행**: AsyncIO + ThreadPoolExecutor
- ✅ **Graceful degradation**: 부분 실패 시에도 서비스 계속

**기술 상세**:
```python
# ThreadSafeRAGManager 사용
...
자세한 내용: [PERFORMANCE_OPTIMIZATION.md](./PERFORMANCE_OPTIMIZATION.md)

### 캐싱 전략

- **Redis**: 동일 사주 차트 24시간 캐싱
- **In-memory**: RAG 임베딩 캐시
- **CDN**: 정적 리소스 (카드 이미지 등)
...
- [성능 최적화 가이드](./PERFORMANCE_OPTIMIZATION.md) - RAG 병렬 처리 상세
- [아키텍처](./ARCHITECTURE.md) - 시스템 구조
- [테스트 가이드](../TESTING_GUIDE.md) - API 테스트

---

*이 문서는 최신 API 변경사항을 반영합니다. 마지막 업데이트: 2026년 1월*
