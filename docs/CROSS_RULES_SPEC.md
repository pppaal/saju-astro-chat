...
**발화 조건**
- 사주 측: _(predicate가 컨텍스트 조회. 예: ctx.hasSaju)_
- 점성 측: `astro.state.dignity.Saturn.domicile` / `astro.state.dignity.Saturn.exaltation` / `astro.state.planet.Saturn.house.10` / `astro.state.stellium.house.10`
**서술 (confirm)**
...
**발화 조건**
- 사주 측: _(predicate가 컨텍스트 조회. 예: ctx.hasSaju)_
- 점성 측: `astro.state.dignity.Jupiter.domicile` / `astro.state.dignity.Jupiter.exaltation` / `astro.state.planet.Jupiter.house.9`
**서술 (confirm)**
...
**발화 조건**
- 사주 측: _(predicate가 컨텍스트 조회. 예: ctx.hasSaju)_
- 점성 측: `astro.state.dignity.Mercury.exaltation` / `astro.state.planet.Jupiter.house.9` / `astro.state.planet.Mercury.house.9` / `astro.state.stellium.house.9`
**서술 (confirm)**
...
**발화 조건**
- 사주 측: _(predicate가 컨텍스트 조회. 예: ctx.hasSaju)_
- 점성 측: `astro.state.dignity.Venus.domicile` / `astro.state.dignity.Venus.exaltation` / `astro.state.planet.Venus.house.5` / `astro.state.stellium.house.5`
**서술 (confirm)**
...
**발화 조건**
- 사주 측: _(predicate가 컨텍스트 조회. 예: ctx.hasSaju)_
- 점성 측: `astro.state.dignity.Mercury.domicile` / `astro.state.planet.Mercury.house.3` / `astro.state.planet.Mercury.sign.Gemini`
**서술 (confirm)**
...
**발화 조건**
- 사주 측: _(predicate가 컨텍스트 조회. 예: ctx.hasSaju)_
- 점성 측: `astro.state.dignity.Mars.domicile` / `astro.state.planet.Mars.house.1` / `astro.state.stellium.house.1`
**서술 (confirm)**
> 사주 비겁 강세 또는 종왕격 + 점성 1궁 stellium 또는 Mars angular — 독립·자영·창업이 본인 결.

### `career.state.archetype.caregiver`
- **레이어**: 정적 (state)
- **도메인**: career
- **의미**: 의료·돌봄 적성
- **폴라리티 힌트**: 긍정 (양쪽 동의 → confirm)
**발화 조건**
- 사주 측: _(predicate가 컨텍스트 조회. 예: ctx.hasSaju)_
- 점성 측: `astro.state.planet.Moon.house.12` / `astro.state.planet.Moon.house.4` / `astro.state.planet.Moon.house.6` / `astro.state.stellium.house.6`
**서술 (confirm)**
...
**발화 조건**
- 사주 측: _(predicate가 컨텍스트 조회. 예: ctx.hasSaju)_
- 점성 측: `astro.state.planet.Mercury.house.1` / `astro.state.planet.Mercury.house.3` / `astro.state.stellium.house.3`
**서술 (confirm)**
...
**발화 조건**
- 사주 측: _(predicate가 컨텍스트 조회. 예: ctx.hasSaju)_
- 점성 측: `astro.state.planet.Neptune.house.12` / `astro.state.planet.Sun.house.12` / `astro.state.stellium.house.12`
**서술 (confirm)**
...
**발화 조건**
- 사주 측: _(predicate가 컨텍스트 조회. 예: ctx.hasSaju)_
- 점성 측: `astro.state.dignity.Mercury.exaltation` / `astro.state.planet.Mercury.house.2` / `astro.state.stellium.house.2`
**서술 (confirm)**
> 사주 정재격·정재 강 + 점성 Mercury domicile/exalt 또는 2궁 강조 — 금융·회계·자산관리 영역의 결.

### `career.state.archetype.analyst`
- **레이어**: 정적 (state)
- **도메인**: career
- **의미**: 분석·IT·과학 적성
- **폴라리티 힌트**: 긍정 (양쪽 동의 → confirm)
**발화 조건**
- 사주 측: _(predicate가 컨텍스트 조회. 예: ctx.hasSaju)_
- 점성 측: `astro.state.dignity.Saturn.exaltation` / `astro.state.planet.Mercury.house.1` / `astro.state.planet.Mercury.house.10`
**서술 (confirm)**
...
**발화 조건**
- 사주 측: _(predicate가 컨텍스트 조회. 예: ctx.hasSaju)_
- 점성 측: `astro.state.planet.Sun.house.5` / `astro.state.planet.Venus.house.1` / `astro.state.stellium.house.5`
**서술 (confirm)**
...
**발화 조건**
- 사주 측: _(predicate가 컨텍스트 조회. 예: ctx.hasSaju)_
- 점성 측: `astro.state.planet.Pluto.house.1` / `astro.state.planet.Pluto.house.10` / `astro.state.stellium.house.8`
**서술 (confirm)**
...
> 이상과 현실의 간극 — 환상이 깊을수록 실망도 큼. 영감과 자기기만의 양면.
...
> 자녀에 대한 갈망과 망설임이 공존 — 시기·환경 압박과 본인 준비도가 어긋날 수 있음.
...
> 돌봄의 보람과 자기 삶의 압박이 양립 — 책임이 깊어질수록 자기 시간 확보가 핵심 과제.
...
> 비전형 결합의 자유로움과 정서적 흔들림이 공존 — 정의를 스스로 만들수록 안정이 깊어짐.
...
**발화 조건**
- 사주 측: _(predicate가 컨텍스트 조회. 예: ctx.hasSaju)_
- 점성 측: `astro.timing.solarReturn.Saturn.house.10` / `astro.timing.transit.Jupiter.conjunction.natal.MC` / `astro.timing.transit.Saturn.sextile.natal.MC` / `astro.timing.transit.Saturn.trine.natal.MC`
**서술 (confirm)**
> 세운 정관 + 점성 Saturn이 10궁/MC 부드럽게 활성 — 외부 인정·직책 상승 호기. 평가·승진 시점 적합.

### `career.timing.year.move-job`
- **레이어**: 시점 (timing)  ·  **스케일**: 1년 (세운/SR)
- **도메인**: career
- **의미**: 이직 분기점
- **폴라리티 힌트**: 양면 (양쪽 동의 → conflict)
**발화 조건**
- 사주 측: _(predicate가 컨텍스트 조회. 예: ctx.hasSaju)_
- 점성 측: `astro.timing.transit.Uranus.`
**서술 (confirm)**
> 세운 편재·편관 + 점성 Uranus 또는 Node가 10궁/MC 자극 — 직장·역할 변동의 해. 새 자리·구조 변경 가능.

**서술 (conflict / 양면)**

> 안정 유지 vs 변동 신호 혼재 — 옮길지 머무를지 한 해 안에 답.

### `career.timing.year.start-business`
- **레이어**: 시점 (timing)  ·  **스케일**: 1년 (세운/SR)
- **도메인**: career
- **의미**: 창업·독립 호기
- **폴라리티 힌트**: 긍정 (양쪽 동의 → confirm)
**발화 조건**
- 사주 측: _(predicate가 컨텍스트 조회. 예: ctx.hasSaju)_
- 점성 측: `astro.timing.transit.Jupiter.house.1` / `astro.timing.transit.Mars.conjunction.natal.MC` / `astro.timing.transit.Mars.house.1`
**서술 (confirm)**
> 세운 비견·식신 + 점성 Mars가 1궁/10궁 활성 — 독립·창업·자기 사업 시작에 유리한 해.

### `career.timing.year.certification`
- **레이어**: 시점 (timing)  ·  **스케일**: 1년 (세운/SR)
- **도메인**: career
- **의미**: 자격·학습·전문성 강화
- **폴라리티 힌트**: 긍정 (양쪽 동의 → confirm)
...
### `career.timing.year.publication`
- **레이어**: 시점 (timing)  ·  **스케일**: 1년 (세운/SR)
- **도메인**: career
- **의미**: 발신·출판·콘텐츠 호기
- **폴라리티 힌트**: 긍정 (양쪽 동의 → confirm)
**발화 조건**
- 사주 측: _(predicate가 컨텍스트 조회. 예: ctx.hasSaju)_
- 점성 측: `astro.timing.solarReturn.Mercury.house.3` / `astro.timing.solarReturn.Venus.house.5` / `astro.timing.transit.Jupiter.house.3`
**서술 (confirm)**
...
> 영적 끌림과 현실 검증의 충돌 — 신비함이 깊어질수록 실체 확인이 늦어질 수 있음.
...
> 연결 회복과 거리 조정이 동시에 요구 — 일·자기 일정과 가족 시간의 균형이 핵심.
...
> 새로운 가능성과 뿌리 단절감의 양면 — 옮긴 후의 적응 단계에 의식적 안착이 필요.
...
> 회귀의 안정과 정체성 후퇴 사이의 긴장 — 과거 정리와 미래 좌표 잡기를 함께 해야 함.
...
### `career.timing.event.career-axis-shift`
- **레이어**: 시점 (timing)  ·  **스케일**: 이벤트 (활성화)
- **도메인**: career
- **의미**: MC 축 변동 — 인생 직업 축 재정의
- **폴라리티 힌트**: 양면 (양쪽 동의 → conflict)
**발화 조건**
- 사주 측: `saju.timing.event.day.`
- 점성 측: `astro.timing.transit.Pluto.conjunction.natal.MC` / `astro.timing.transit.Saturn.conjunction.natal.MC` / `astro.timing.transit.Saturn.opposition.natal.MC` / `astro.timing.transit.Uranus.conjunction.natal.MC`
**서술 (confirm)**
