...
export const SafeTextSchema = z
  .string()
  .refine((text) => !/<script|<\/script|javascript:|on\w+\s*=/i.test(text), {
...
// ============================================================
// Chat Message Schemas (shared across 8+ chat routes)
// ============================================================

/** Single chat message */
export const ChatMessageSchema = z.object({
  role: z.enum(['system', 'user', 'assistant']),
  content: z.string().min(1).max(LIMITS.MESSAGE),
})
...
export type IChingChangingLineRequest = z.infer<typeof IChingChangingLineSchema>

/** Tarot prefetch request */
export const TarotPrefetchSchema = z.object({
  categoryId: z.string().min(1).max(LIMITS.ID),
  spreadId: z.string().min(1).max(LIMITS.ID),
})
 ...
 */
export function parseQueryParams<T>(req: NextRequest, schema: ZodSchema<T>): ApiHandlerResult<T> {
 ...
 */
export function validationError(errors: ValidationError[]): ApiHandlerResult<never> {
  return {
    error: {
