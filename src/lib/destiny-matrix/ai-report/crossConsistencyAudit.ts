...
function asText(value: unknown): string {
  ...
  if (Array.isArray(value)) return value.map((v) => (typeof v === 'string' ? v : '')).join(' ').trim()
  return ''
}
...
export function auditCrossConsistency(input: {
  ...
  add('C01', 'Day master element is valid', dayMasterValid, `dayMaster=${matrixInput.dayMasterElement}`)
  ...
  add('C03', 'Sibsin distribution has sufficient density', sibsinEntries.length >= 2 && sibsinTotal >= 3, `keys=${sibsinEntries.length}, total=${sibsinTotal}`)
  ...
  add('C08', 'Planet houses are dense enough', planetHouseCount >= 7, `planetHouses=${planetHouseCount}`)
  add('C09', 'Planet signs are dense enough', planetSignCount >= 7, `planetSigns=${planetSignCount}`)
  ...
  add('C12', 'Aspect angle precision is sufficient', aspectsWithAngle >= 2, `withAngle=${aspectsWithAngle}`)
  ...
  add('C16', 'Profile context is complete enough', ctxPass, `birthDate=${ctx.birthDate || 'N/A'}, birthTime=${ctx.birthTime || 'N/A'}, timezone=${ctx.timezone || 'N/A'}`)

  const sectionTexts = collectSectionTexts(report)
  const crossSentenceCount = sectionTexts.filter((t) => SAJU_REGEX.test(t) && ASTRO_REGEX.test(t)).length
  const crossRatio = sectionTexts.length > 0 ? crossSentenceCount / sectionTexts.length : 0
  add('C17', 'Report has cross-grounded sections', crossRatio >= 0.6, `crossSections=${crossSentenceCount}/${sectionTexts.length}`)

  const totalChars = sectionTexts.reduce((sum, t) => sum + t.length, 0)
  const minChars = mode === 'comprehensive' ? 2200 : 1400
  add('C18', 'Report has minimum narrative volume', totalChars >= minChars, `totalChars=${totalChars}, min=${minChars}`)

  const anchors = graphEvidence?.anchors || []
  const anchorTarget = mode === 'comprehensive' ? 10 : 6
  add('C19', 'GraphRAG anchor coverage is sufficient', anchors.length >= anchorTarget, `anchors=${anchors.length}, target=${anchorTarget}`)
  ...
  const blockers = checks
    .filter((c) => !c.pass && ['C01', 'C04', 'C05', 'C08', 'C09', 'C10', 'C19', 'C20'].includes(c.id))
    .map((c) => `${c.id} ${c.title}`)
