...
const SECTION_MIN_FILL_KO: Record<string, string> = {
  introduction: '실행 시에는 우선순위가 아닌 항목을 과감히 보류해 집중도를 유지하세요. 핵심 한 건의 완성도가 전체 흐름을 안정시키는 출발점이 됩니다.',
  personalityDeep: '자기 리듬을 지키기 위해 하루 종료 전에 결정 로그를 3줄로 정리하세요. 이 기록이 다음 선택의 품질을 높이는 기준점이 됩니다.',
  careerPath: '오늘은 새 일 착수보다 진행 중 과업의 닫힘을 우선하세요. 완결된 결과물이 쌓일수록 다음 기회의 신뢰도와 협상력이 함께 올라갑니다.',
  relationshipDynamics: '관계에서는 맞는 말보다 맞는 순서가 중요합니다. 먼저 상대 맥락을 확인하고 그 다음에 요청을 제시하면 오해를 줄일 수 있습니다.',
  wealthPotential: '재정 결정은 당일 확정보다 검토 창을 두는 쪽이 유리합니다. 지출 전 체크리스트를 고정하면 손실 가능성을 구조적으로 줄일 수 있습니다.',
  healthGuidance: '회복 루틴은 의지가 아니라 시간표에 넣어야 지속됩니다. 기본 리듬이 안정되면 같은 업무량에서도 체감 피로가 크게 낮아집니다.',
  lifeMission: '장기 목표는 크기보다 지속성이 중요합니다. 작은 실행을 반복 가능한 형태로 남기는 것이 방향성과 성과를 동시에 지켜줍니다.',
  timingAdvice: '타이밍이 애매할수록 확정 단계를 뒤로 미루고 재확인을 앞당기세요. 이 원칙만 지켜도 실수 확률을 체감할 만큼 낮출 수 있습니다.',
  actionPlan: '실행안은 복잡하게 만들지 말고 오늘 바로 행동 가능한 수준으로 고정하세요. 완료 확인 기준을 미리 정하면 흔들림이 줄어듭니다.',
  conclusion: '결론은 단순합니다. 기준을 지키는 실행이 누적 성과를 만듭니다. 같은 원칙을 반복할수록 결과는 더 선명해집니다.',
}
...
function buildSectionExecutionSentence(
  ...
  if (lang === 'ko') {
    const lines: Record<string, string> = {
      introduction: '실행은 오늘 끝낼 1건을 먼저 고정하고, 대외 확정이 필요한 안건은 최소 한 번의 재검토 슬롯을 거친 뒤 처리하세요.',
      personalityDeep: '의사결정은 즉답보다 요약 기록을 먼저 남기고, 감정이 올라오는 대화는 템포를 한 단계 늦추는 것이 손실을 줄입니다.',
      careerPath: '업무에서는 새 착수보다 진행 중 과제의 완결률을 올리고, 협업 건은 역할·마감·책임 3항목이 합의된 뒤 확정하세요.',
      relationshipDynamics: '관계에서는 결론을 먼저 말하기보다 상대의 이해를 한 줄로 확인한 뒤 요청을 제시하면 충돌 비용이 크게 줄어듭니다.',
      wealthPotential: '재정은 금액·기한·취소 조건을 분리 점검하고, 당일 확정 대신 24시간 재확인 창을 두는 방식이 안정적입니다.',
      healthGuidance: '건강은 강도보다 회복 블록을 먼저 배치하세요. 수면·수분·휴식 시간을 고정하면 피로 편차를 줄일 수 있습니다.',
      lifeMission: '장기 방향은 큰 선언보다 주간 기록과 복기 루틴으로 고정할 때 더 오래 유지되고 실제 성과로 연결됩니다.',
      timingAdvice: '타이밍은 착수-검토-확정을 분리해 운영하세요. 특히 문서·계약 성격의 일은 검토 단계를 생략하지 않는 것이 핵심입니다.',
      actionPlan: '2주 계획은 완료 1건, 재확인 1건, 보류 1건의 구조로 단순화하면 실행 전환율이 높고 누락이 줄어듭니다.',
      conclusion: '결론적으로 속도보다 순서를 지키는 운영이 유리합니다. 같은 규칙을 며칠 유지하면 변동성이 빠르게 줄어듭니다.',
    }
...
function buildLifeJourneyNarrative(
  ...
  if (input.lang === 'ko') {
    const ageLine =
      age === null ? '현재 나이 정보가 없어 일반 생애 구간 기준으로 정리합니다.' : `현재 나이 ${age}세 기준으로 생애 구간을 재배치해 해석합니다.`
...
function buildSectionDeepDiveSentence(
  ...
  if (lang === 'ko') {
    ...
    const cautionLine = cText ? `${cText} 신호가 비용이 커지는 지점을 알려 줍니다.` : '리스크는 주로 확인 절차 누락에서 발생하기 쉽습니다.'
    const balanceLine = bText
      ? `${bText} 신호가 완충 역할을 맡으며, 속도와 확정 사이의 균형을 통해 결과를 안정화합니다.`
      : '약한 완충 신호에서도 착수와 확정을 분리하면 변동성을 줄일 수 있습니다.'
...
function buildSectionProtocolSentence(
  ...
  if (lang === 'ko') {
    const lines: Record<string, string> = {
      introduction:
        '운영 프로토콜은 단순하게 유지합니다. 오늘 끝낼 핵심 1건을 먼저 잠그고, 외부 확정이 필요한 안건은 재확인 슬롯을 거쳐 처리합니다.',
      personalityDeep:
        '기록형 프로토콜이 잘 맞습니다. 중요한 대화 후에는 합의 내용을 한 줄로 남겨 다음 결정에서 재사용하세요.',
      careerPath: '완결 우선 프로토콜이 핵심입니다. 새 일은 제한하고, 진행 중 과제의 완료 기준을 정한 뒤 마무리 후 확장으로 넘어가세요.',
      relationshipDynamics:
        '정렬 프로토콜을 적용하세요. 결론보다 해석 정렬을 먼저 맞추면 충돌 비용과 오해 비용을 동시에 줄일 수 있습니다.',
      wealthPotential: '조건 분리 프로토콜이 가장 효과적입니다. 금액, 기한, 취소 조건을 분리해 검증한 뒤 확정해야 손실 가능성이 낮아집니다.',
      healthGuidance:
        '회복 우선 프로토콜이 안정적입니다. 일정에서 수면과 회복 블록을 먼저 잠그고, 그 다음에 강도를 올리는 방식이 피로 누적을 막습니다.',
      lifeMission: '주간 복기 프로토콜이 강력합니다. 한 주 동안의 선택 결과를 원인-결과로 정리해 두면, 다음 선택의 정밀도가 올라갑니다.',
      timingAdvice:
        '단계 분리 프로토콜이 핵심입니다. 착수, 검토, 확정을 같은 창에 몰지 말고 분리해야 누락 위험과 실수 비용을 줄일 수 있습니다.',
      actionPlan:
        '2주 단위 프로토콜은 완료 1건, 검토 1건, 보류 1건 구조로 단순화하세요. 이 구조 유지가 누적 실행률을 높이면서 누락 가능성을 낮출 수 있습니다.',
      conclusion: '결론 프로토콜은 동일합니다. 빠른 확정보다 정확한 확정이 우선이며, 같은 운영 규칙을 유지할 때 신뢰가 쌓입니다.',
    }
    return lines[section]
  }
...
function formatActionSentence(claims: SynthesizedClaim[], lang: 'ko' | 'en'): string {
  ...
  if (lang === 'ko') {
    return plan || '결정과 실행 시점을 분리하고 확정 전에 재확인 단계를 두세요.'
  }
...
function formatTimingGrounding(input: MatrixCalculationInput, lang: 'ko' | 'en'): string {
  ...
  if (lang === 'ko') {
    if (daeun || saeun || wolun || iljin || iljinDate) {
      return `현재 대운 ${daeun || '미확인'}, 세운 ${saeun || '미확인'}, 월운 ${wolun || '미확인'}, 일진 ${iljin || '미확인'}${iljinDate ? `(${iljinDate})` : ''} 기준으로, 결론과 실행 시점을 분리해 운영해 주세요.`
    }
    return '대운·세운·월운·일진 정보가 제한적이므로 보수 기준의 24시간 재확인 창을 두는 편이 안전합니다.'
  }
  if (daeun || saeun || wolun || iljin || iljinDate) {
