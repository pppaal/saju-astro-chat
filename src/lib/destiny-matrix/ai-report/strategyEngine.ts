...
function buildDomainStrategy(
  ...
  const playbookKo: Record<SignalDomain, Partial<Record<StrategyPhaseCode, string>>> = {
    career: {
      expansion: '?? ?? 1~2?? ????, ?? ??? ????? ?? ? ?????.',
      high_tension_expansion: '??? ?? ?????? 24?? ??? ? ?????.',
      expansion_guarded: '?? ??? ?? ??? ??? ??? ?????.',
      stabilize: '?????????? ??? ? ?? ??? ????.',
      defensive_reset: '?? ???? ??? ???? ???? ???? ?? ???.',
    },
  ...
  const playbookEn: Record<SignalDomain, Partial<Record<StrategyPhaseCode, string>>> = {
    career: {
      expansion: 'Focus on 1-2 core deliverables and commit externally only after checklist pass.',
      high_tension_expansion: 'Make decisions now, but delay signing/sending until a 24h recheck.',
      expansion_guarded: 'Run expansion and condition verification in parallel.',
      stabilize: 'Lock role, scope, and deadline before increasing speed.',
      defensive_reset: 'Reallocate resources and priorities before new commitments.',
    },
  }

  const detail =
    lang === 'ko'
      ? playbookKo[domain]?.[phase] || '??? ?? ??? ???? ?? ???? ?? ?????.'
      : playbookEn[domain]?.[phase] ||
        'Keep offense/defense ratio and execute in staged, evidence-based steps.'

  if (lang === 'ko') {
    return `${domainLabel(domain, lang)} ??? ?? ${attackPercent}% / ?? ${100 - attackPercent}%? ?????. ${detail}`
  }
