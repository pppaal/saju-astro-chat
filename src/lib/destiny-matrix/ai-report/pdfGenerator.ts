...
import type { AIPremiumReport } from './aiReportService';
...
class PremiumPDFRenderer {
  ...
  drawCoverPage(report: AIPremiumReport) {
    ...
    page.drawEllipse({
      x: 297.5, y: 500, xScale: 200, yScale: 200,
      color: this.colors.primary,
      opacity: 0.15,
    });
    ...
    // Profile box
    page.drawRectangle({
      x: 50, y: 480, width: 495, height: 150,
    ...
    page.drawEllipse({
      x: 470, y: 555, xScale: 45, yScale: 45,
      color: gradeColor,
    });
  ...
  drawSectionPage(title: string, content: string, sectionNumber: number) {
    ...
    page.drawEllipse({
      x: 50, y: 770, xScale: 20, yScale: 20,
      color: this.colors.primary,
    });
  ...
  drawClosingPage(report: AIPremiumReport) {
    ...
    // Decorative element
    page.drawEllipse({
      x: 297.5, y: 550, xScale: 150, yScale: 150,
      color: this.colors.primary,
      opacity: 0.1,
    });

    // Thank you message
    const thankYou = this.lang === 'ko'
