export function formatDecisionActionLabel(
  ...
  const koMap: Record<string, string> = {
    commit_now: soften ? '바로 확정하지 말고 최종 확인을 거치세요.' : '확정 단계까지 밀어도 되는 흐름입니다.',
    ...
    route_recheck_first: '경로와 동선을 먼저 재확인하세요.',
    lease_review_first: '계약 조건을 먼저 재확인하고 필요한 부분을 다시 협의하세요.',
    basecamp_reset_first: '큰 이동보다 생활 거점과 운영 방식을 먼저 재정비하세요.',
  }

  const enMap: Record<string, string> = {
    commit_now: soften ? 'Avoid instant commitment; run a final verification step.' : 'Conditions support a direct commitment.',
    ...
    route_recheck_first: 'Recheck the route, commute, and daily path first.',
...
export function formatPolicyCheckLabels(
  ...
  return [...new Set((checks || []).map((item) => String(item || '').replace(/\s+/g, ' ').trim()).filter(Boolean))]
}
