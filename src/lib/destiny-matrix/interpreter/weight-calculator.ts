...
const TRANSIT_WEIGHT_MODIFIERS: Record<
  TransitCycle,
  {
...
export class DynamicWeightCalculator {
  ...
  calculateWeights(
    ...
    if (input.geokguk) {
      ...
      if (modifier) {
        ...
        this.log(
          '격국 조정',
          { geokguk: input.geokguk, modifier },
          weights,
          `${input.geokguk} 격국 가중치 적용`
        )
      }
    }
    ...
    if (input.yongsin) {
      ...
      if (modifier) {
        ...
        this.log(
          '용신 조정',
          { yongsin: input.yongsin, modifier },
          weights,
          `${input.yongsin} 용신 가중치 적용`
        )
      }
    }
    ...
    if (queryDomain) {
      ...
      if (modifier) {
        ...
        this.log(
          '도메인 조정',
          { domain: queryDomain, modifier },
          weights,
          `${queryDomain} 도메인 가중치 적용`
        )
      }
    }
    ...
    if (input.activeTransits && input.activeTransits.length > 0) {
      for (const transit of input.activeTransits) {
        ...
        if (transitConfig) {
          ...
          this.log(
            '트랜짓 조정',
            { transit, modifier: transitConfig.weights },
            weights,
            `${transit} 가중치 적용`
          )
        }
   ...
   * 가중치 수정자 적용
   */
  private applyModifier(weights: LayerWeights, modifier: Partial<LayerWeights>): LayerWeights {
