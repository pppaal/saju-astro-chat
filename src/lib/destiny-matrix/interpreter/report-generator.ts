...
export class FusionReportGenerator {
  ...
  generateReport(
    ...
    // 2. 상위 인사이트 추출
    const topInsights = this.insightGenerator.getTopInsights(
      allInsights,
      this.config.maxTopInsights
    )
    ...
    return {
      ...
      timingAnalysis,
      visualizations,
      detailedData: this.config.includeDetailedData
        ? {
            allInsights,
            layerData: layerResults,
            weightCalculationLog: [],
            narrativeGenerationLog: [],
          }
        : undefined,
    }
  }
  ...
  private generateProfileSummary(input: MatrixCalculationInput, lang: 'ko' | 'en'): ProfileSummary {
    const dayMasterDescriptions: Record<FiveElement, { ko: string; en: string }> = {
      목: {
        ko: '성장과 발전을 추구하는 목(木) 기운',
        en: 'Wood energy pursuing growth and development',
      },
      화: {
        ko: '열정과 표현을 즐기는 화(火) 기운',
        en: 'Fire energy enjoying passion and expression',
      },
      토: {
        ko: '안정과 중재를 중시하는 토(土) 기운',
        en: 'Earth energy valuing stability and mediation',
      },
      금: {
        ko: '결단과 정의를 추구하는 금(金) 기운',
        en: 'Metal energy pursuing decision and justice',
      },
   ...
   * 등급 정보
   */
  private getGradeInfo(score: number): {
    grade: 'S' | 'A' | 'B' | 'C' | 'D'
    ...
    if (score >= 90) {
      return {
        grade: 'S',
        gradeDescription: '최상의 조화! 운명의 시너지가 폭발합니다.',
        gradeDescriptionEn: 'Supreme harmony! Destiny synergy explodes.',
      }
    }
    if (score >= 75) {
      return {
        grade: 'A',
        gradeDescription: '훌륭한 조화! 성장의 기회가 열려 있습니다.',
        gradeDescriptionEn: 'Excellent harmony! Opportunities for growth are open.',
      }
    }
    if (score >= 60) {
      return {
        grade: 'B',
        gradeDescription: '좋은 균형. 안정적인 흐름입니다.',
        gradeDescriptionEn: 'Good balance. Stable flow.',
      }
    }
    if (score >= 45) {
      return {
        grade: 'C',
        gradeDescription: '주의가 필요한 영역이 있습니다. 신중하게 진행하세요.',
        gradeDescriptionEn: 'Some areas need attention. Proceed carefully.',
      }
    }
    return {
      grade: 'D',
      gradeDescription: '도전의 시기입니다. 내면 성찰과 준비의 시간으로 삼으세요.',
      gradeDescriptionEn:
        'A challenge-focused period. Use it as a time for inner reflection and preparation.',
    }
  }
  ...
  private generateDomainSummary(domain: InsightDomain, score: number, lang: 'ko' | 'en'): string {
    const templates: Record<
      InsightDomain,
      Record<'high' | 'mid' | 'low', { ko: string; en: string }>
    > = {
      personality: {
        high: {
          ko: '자아 정체성이 강하게 빛나는 시기입니다.',
          en: 'A time when self-identity shines strongly.',
        },
      ...
      career: {
        high: {
          ko: '직업적 성취가 기대되는 시기입니다.',
          en: 'A time when career achievement is expected.',
        },
        mid: { ko: '꾸준한 업무 진행이 가능합니다.', en: 'Steady work progress is possible.' },
        low: {
          ko: '직업적 전환을 신중히 고려하세요.',
          en: 'Carefully consider career transitions.',
        },
      },
      relationship: {
      ...
      wealth: {
        ...
        low: {
          ko: '재정 관리에 주의가 필요합니다.',
          en: 'Careful financial management is needed.',
        },
      },
      health: {
  ...
  private getTransitInfluence(transit: TransitCycle): 'positive' | 'neutral' | 'challenging' {
    ...
    const challengingTransits: TransitCycle[] = [
      'saturnReturn',
      'plutoTransit',
      'mercuryRetrograde',
      'marsRetrograde',
    ...
    if (positiveTransits.includes(transit)) {
      return 'positive'
    }
    if (challengingTransits.includes(transit)) {
      return 'challenging'
    }
    return 'neutral'
  }
  ...
  private getTimingDescription(score: number, lang: 'ko' | 'en'): string {
    if (score >= 70) {
      return lang === 'ko'
        ? '행동하기 좋은 시기입니다. 계획을 실행하세요.'
        : 'Good time for action. Execute your plans.'
    }
    if (score >= 50) {
      return lang === 'ko'
        ? '안정적인 흐름입니다. 꾸준히 진행하세요.'
        : 'Stable flow. Proceed steadily.'
    }
    return lang === 'ko'
      ? '신중한 접근이 필요합니다. 큰 결정은 미루세요.'
      : 'Careful approach needed. Postpone major decisions.'
  }
      ...
      if (now >= start && now <= end) {
        alerts.push({
          ...
          advice:
            lang === 'ko'
              ? '계약, 여행, 중요 결정을 재검토하세요.'
              : 'Review contracts, travel, and important decisions.',
   ...
   * 시너지 네트워크 생성
   */
  private generateSynergyNetwork(insights: FusionInsight[]): {
    nodes: SynergyNode[]
    edges: SynergyEdge[]
  } {
