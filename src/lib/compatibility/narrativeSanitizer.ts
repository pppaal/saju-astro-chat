 ...
 * Narrative Sanitizer
 *
 * Quick post-pass on the streamed Sonnet narrative to catch jargon
 * leaks (English term left bare, 한자 단독 노출 등) and patch them
 * with the friendly Korean equivalent. Best-effort — runs once at
 * stream finalize, not per-token.
 */
 ...
 */
export function sanitizeNarrative(text: string): {
  cleaned: string
