...
export const pentaclesCards: RawCard[] = [
  ...
  },
  {
    ...
    upright: {
      ...
      meaningKo:
        '펜타클 2는 무한대 기호로 묶인 두 개의 동전을 저글링하는 인물을 보여주며, 배경에서 배들이 거친 파도를 타고 있습니다. 이것은 움직이는 삶의 균형 잡기입니다—여러 우선순위 관리, 변화하는 상황에 적응, 여러 재정 과제를 동시에 처리하기. 인물은 힘들어하기보다 춤을 춥니다; 저글링에 가벼움이 있어 이 균형이 올바른 태도로 접근하면 지속 가능함을 시사합니다. 때로는 삶이 경쟁하는 책임을 다루도록 요구합니다: 일과 가족, 저축과 지출, 안정과 모험. 이 카드는 모든 것을 관리할 수 있음을 확인하지만, 지속적인 조정과 존재가 필요합니다. 뒤의 파도는 외부 상황이 결코 완전히 안정적이지 않다는 것을 상기시킵니다—적응력은 선택이 아니라 필수입니다. 사랑에서 다른 삶의 요구와 관계 필요의 균형을 맞추거나, 파트너십이 요구하는 주고받기를 탐색하고 있습니다. 직업에서 여러 프로젝트를 관리하거나, 변화하는 우선순위에 적응하거나, 재정적 결정을 신중히 저울질하고 있습니다. 메시지: 유연함을 유지하고, 유머를 잃지 말고, 균형은 목적지가 아니라 동사라는 것을 기억하세요.',
    },
    reversed: {
      keywords: [
        'Overwhelmed',
