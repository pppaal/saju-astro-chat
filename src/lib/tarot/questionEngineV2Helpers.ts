...
export function buildHeuristicIntent(
  ...
  if (
    ...
    subject === 'relationship' &&
    /((눈|인상).*(어때|어떤|어떨)|보여|어케 보여|어떻게 보여|어떻게 봐|어떤 사람)/.test(joined)
  ) {
...
export function buildHeuristicDirectAnswer(
  intent: StructuredIntent,
  questionVariants: string[],
  language: EngineLanguage
) {
...
export function resolveDeterministicSpread(
  ...
  if (
    ...
    /(누가\s*내게\s*올까|내게\s*누가\s*들어올까|새\s*사람\s*들어올까|인연)/i.test(
      normalizedQuestion
    )
  )
    stableCandidates.push({ themeId: 'love-relationships', spreadId: 'relationship-check-in' })
  if (
    (intent.tone === 'flow' || intent.questionType === 'unknown') &&
    isCareerQuestion(normalizedQuestion)
  )
    stableCandidates.push({ themeId: 'career-work', spreadId: 'career-path' })
  if (
    (intent.tone === 'flow' || intent.questionType === 'unknown') &&
    isMoneyQuestion(normalizedQuestion)
  ) {
  ...
  if (
    (intent.tone === 'flow' || intent.questionType === 'unknown') &&
    isHealthQuestion(normalizedQuestion)
  ) {
  ...
  if (
    ...
    /(직장|이직|옮기|퇴사|job change|quit)/i.test(normalizedQuestion)
  )
  ...
  if (
    ...
    stableCandidates.push({ themeId: 'general-insight', spreadId: 'past-present-future' })
  if (intent.subject === 'external_situation' && /(면접|interview)/i.test(normalizedQuestion))
    stableCandidates.push({ themeId: 'career-work', spreadId: 'interview-result' })
  if (intent.subject === 'external_situation' && /(시험|합격|exam)/i.test(normalizedQuestion))
  ...
  const ranked = candidates.flatMap((q) =>
    recommendSpreads(q, 3).map((rec) => ({ ...rec, sourceQuestion: q }))
  )
...
export function chooseResolvedIntent(
