...
function getDayMasterElement(
  dayMaster?: string | { name?: string; heavenlyStem?: string }
): string | null {
  if (!dayMaster) {
    return null
  }
  const dmStr =
    typeof dayMaster === 'string' ? dayMaster : dayMaster?.name || dayMaster?.heavenlyStem || ''
  if (!dmStr || typeof dmStr !== 'string') {
    return null
  }
...
export function generateDailyNotifications(
  ...
  // 일진 데이터 추출
  const iljin = Array.isArray(saju.unse?.iljin) ? saju.unse.iljin[0] : saju.unse?.iljin
  const iljinStem = iljin?.heavenlyStem || iljin?.stem || iljin?.ganji?.charAt(0) || ''
  const iljinElement = stemToElement(iljinStem) || 'earth'

  // 카테고리별 점수 계산
  const scores = calculateCategoryScores(dayMasterElement, iljinElement, astrology.transits)

  // 최적 시간대 계산
  const { luckyHours, cautionHours, peakHour } = findOptimalHours(dayMasterElement, iljinElement)
...
function localizeNotification(
  ...
  if (locale === 'en') {
    const typeTranslations: Partial<
      Record<
        NotificationType,
        { title: string; positive: string; neutral: string; caution: string }
      >
    > &
      Record<string, { title: string; positive: string; neutral: string; caution: string }> = {
      daily_fortune: {
