...
const labelStr = (labels?: Labels) => {
  ...
  const result = Object.entries(labels)
    ...
    .map(([k, v]) => `${k}="${String(v)}"`)
    .join(',')
  labelKeyCache.set(labels, result)
  return result
}
...
export function toPrometheus() {
  ...
  for (const sample of Object.values(timings)) {
    ...
    lines.push(
      `${sample.name}_seconds{quantile="0.5"${suffix ? ',' + suffix : ''}} ${(p50 / 1000).toFixed(6)}`
    )
    lines.push(
      `${sample.name}_seconds{quantile="0.95"${suffix ? ',' + suffix : ''}} ${(p95 / 1000).toFixed(6)}`
    )
    lines.push(
      `${sample.name}_seconds{quantile="0.99"${suffix ? ',' + suffix : ''}} ${(p99 / 1000).toFixed(6)}`
    )
  }
  return lines.join('\n')
}
