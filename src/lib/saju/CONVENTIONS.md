...
- **시간 미상 계산 앵커 = 정오(12:00)** — SSOT [`birthTimeAnchor.ts`](./birthTimeAnchor.ts) (`resolveBirthTimeAnchor`). 빈 값 / `'00:00'`(저장 규약상 미상) / 명시 플래그 전부 정오로 수렴한다. 자정 앵커는 진태양시 보정(서울 -32분)으로 출생 인스턴트가 전날로 밀려 **일주**(절입일엔 월주·년주까지)가 어긋난다 — 실제로 궁합/상담사/캘린더 세션 경로가 `'00:00'` 앵커를 써서 통합리포트(정오)와 다른 사주를 내던 버그의 원인. 시간 미상 입력을 엔진(사주·점성)에 넣는 모든 경로는 이 헬퍼를 거칠 것.
...
**위치:** [`saju.ts`](./saju.ts) — `calculateSajuData` 의 daeWoon 산출이 단일 출처. (옛 `unse.getDaeunCycles` raw 경로는 2026-06 제거.)
