...
const DATE_PATTERNS = [
  // ISO format: 1990-05-15
  {
    ...
    extract: (m: RegExpMatchArray) => ({ y: m[1], M: m[2], d: m[3] }),
  },
  ...
  // US format: 05/15/1990, 5/15/1990
  {
    ...
    extract: (m: RegExpMatchArray) => ({ y: m[3], M: m[1], d: m[2] }),
  },
]
...
const TIME_PATTERNS = [
  ...
  {
    regex: /^(\d{1,2}):(\d{2})$/,
    extract: (m: RegExpMatchArray) => ({ h: m[1], m: m[2], period: null }),
  },
  // With AM/PM: 2:30 PM, 2:30PM
  {
    regex: /^(\d{1,2}):(\d{2})\s*(AM|PM)$/i,
    extract: (m: RegExpMatchArray) => ({ h: m[1], m: m[2], period: m[3].toUpperCase() }),
  },
  // Korean 24h: 14시 30분
  {
  ...
  // Korean 12h: 오후 2시 30분
  {
    ...
    extract: (m: RegExpMatchArray) => ({ h: m[2], m: m[3], period: m[1] === '오후' ? 'PM' : 'AM' }),
  },
  // Korean short: 오후 2시
  {
    regex: /^(오전|오후)\s*(\d{1,2})시$/,
    extract: (m: RegExpMatchArray) => ({ h: m[2], m: '0', period: m[1] === '오후' ? 'PM' : 'AM' }),
  },
  // Hour only: 14시
...
export function normalizeBirthInfo(input: BirthInfoInput): NormalizedBirthInfo {
  ...
  const calendarType = input.calendarType ? normalizeCalendarType(input.calendarType) : 'solar'
