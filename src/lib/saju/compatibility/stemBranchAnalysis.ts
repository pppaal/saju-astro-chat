...
function generateBranchAnalysis(
  ...
  if (yukhapPairs.length > 0) {
    parts.push(
      `지지 육합(${yukhapPairs.map((p) => `${p.branch1}-${p.branch2}`).join(', ')})으로 깊은 정이 형성됩니다`
    )
  }
  ...
  if (chungPairs.length > 0) {
    parts.push(
      `지지 충(${chungPairs.map((p) => `${p.branch1}-${p.branch2}`).join(', ')})이 있어 변동이나 갈등에 주의가 필요합니다`
    )
  }
...
export function analyzeBranchCompatibility(
  ...
  const analysis = generateBranchAnalysis(
    yukhapPairs,
    samhapGroups,
    chungPairs,
    hyeongPairs,
