...
export function analyzeComprehensiveCompatibility(
  ...
  // 카테고리별 점수
  const categories =
    options?.categories || (['love', 'business', 'friendship'] as CompatibilityCategory[])
  const categoryScores = categories.map((cat) =>
    analyzeByCategory(person1.pillars, person2.pillars, cat)
  )
  ...
  if (elementCompatibility.missing.length > 0) {
    recommendations.push(
      `함께 ${elementCompatibility.missing.join(', ')} 오행을 보강하는 활동을 해보세요.`
    )
  }
...
export function analyzeMultiPersonCompatibility(
  ...
  const challengingPairs = sortedPairs
    .slice(-2)
    .map((p) => `${p.person1}-${p.person2} (${p.score}점)`)
