...
const SIBSIN_INTERPRETATIONS: Record<
  SibsinKind,
  {
    theme: string
    fortune: string
...
> = {
  비견: {
    theme: '경쟁과 동반자',
    fortune:
      '동료와의 협력이 중요한 시기입니다. 같은 목표를 가진 사람들과 함께할 때 시너지가 발휘됩니다.',
...
const TWELVE_STAGE_TEXTS: Record<
  string,
  {
    energy: string
    meaning: string
...
export function generateFortuneText(
  ...
  // 조언 조합
  const advice = [elementText.advice, sibsinText?.advice, stageText?.advice]
    .filter(Boolean)
    .join(' ')
...
function getElementRelation(dayElement: FiveElement, unseElement: FiveElement): string {
  if (dayElement === unseElement) {
    return '비화'
  }
  if (FIVE_ELEMENT_RELATIONS['생받는관계'][dayElement] === unseElement) {
    return '생조'
  }
  if (FIVE_ELEMENT_RELATIONS['생하는관계'][dayElement] === unseElement) {
    return '설기'
  }
  if (FIVE_ELEMENT_RELATIONS['극하는관계'][dayElement] === unseElement) {
    return '극출'
  }
  if (FIVE_ELEMENT_RELATIONS['극받는관계'][dayElement] === unseElement) {
    return '극입'
  }
  return '중립'
}
...
export function generateComprehensiveText(
  ...
  const advice = [personalityText.advice, fortuneText?.advice].filter(Boolean).join(' ')
