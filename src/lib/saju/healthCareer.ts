...
export function analyzeHealth(pillars: SajuPillars): HealthAnalysis {
  ...
  for (const elem of elements) {
    ...
    const controller = Object.entries(ELEMENT_CONTROLS).find(([, v]) => v === elem)?.[0] as
      | FiveElement
      | undefined
    ...
    if (status === 'vulnerable' || status === 'weak') {
      if (elem === '목') {
        risks.push('간 기능 저하', '눈 피로')
        prevention.push('녹색 채소 섭취', '눈 휴식')
      } else if (elem === '화') {
        risks.push('심장 질환', '불면증')
        prevention.push('적절한 운동', '스트레스 관리')
      } else if (elem === '토') {
        risks.push('소화기 문제', '비만')
        prevention.push('규칙적인 식사', '과식 금지')
      } else if (elem === '금') {
        risks.push('호흡기 질환', '피부 문제')
        prevention.push('금연', '공기 정화')
      } else if (elem === '수') {
        risks.push('신장 기능 저하', '뼈 관절 약화')
        prevention.push('충분한 수분 섭취', '칼슘 보충')
      }
    }

    organHealth.push({
      organ: `${organs.yin}/${organs.yang}`,
      element: elem,
      status,
      score,
  ...
  if (dominantElement === '목') {
    constitution = '목형 체질 - 활동적이고 창의적'
  } else if (dominantElement === '화') {
    constitution = '화형 체질 - 열정적이고 외향적'
  } else if (dominantElement === '토') {
    constitution = '토형 체질 - 안정적이고 실용적'
  } else if (dominantElement === '금') {
    constitution = '금형 체질 - 꼼꼼하고 예민함'
  } else {
    constitution = '수형 체질 - 지혜롭고 내향적'
  }
  ...
  if (weakElement === '목') {
    preventionAdvice.push('간 해독, 눈 건강 관리')
  }
  if (weakElement === '화') {
    preventionAdvice.push('심장 검진, 혈압 관리')
  }
  if (weakElement === '토') {
    preventionAdvice.push('소화기 검진, 당뇨 검사')
  }
  if (weakElement === '금') {
    preventionAdvice.push('호흡기 검진, 피부 관리')
  }
  if (weakElement === '수') {
    preventionAdvice.push('신장 검진, 뼈 건강 체크')
  }
...
export function analyzeCareer(pillars: SajuPillars): CareerAnalysis {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    for (const career of careers) {
      const field: CareerField = {
        ...
        jobs: career.jobs,
        fitScore: elementScores[elem],
        elementBasis: [elem],
        description: `${elem} 오행 기반`,
        successFactors: [],
        challenges: [],
      }

      if (elementCount[elem] >= 2) {
        field.successFactors.push(`${elem} 기운이 강함`)
      }
      if (i === 0) {
  ...
  if (elementCount['화'] >= 3) {
    workStyle = {
      type: '열정형',
      description: '에너지 넘치고 추진력이 강한 스타일',
      strengths: ['빠른 실행력', '동기부여'],
      weaknesses: ['지속력 부족'],
  ...
  if (elementCount['화'] >= 2) {
    entrepreneurialScore += 15
  }
  if (elementCount['목'] >= 2) {
    entrepreneurialScore += 10
  }
  ...
  if (entrepreneurialScore >= 70) {
    careerPath.push('창업 및 자영업 적합')
  }
  if (leadershipScore >= 70) {
    careerPath.push('관리직/임원 승진 가능')
  }
  if (creativityScore >= 70) {
    careerPath.push('창작/기획 분야 성장')
  }
  if (stabilityPreference >= 70) {
    careerPath.push('대기업/공공기관 장기근속')
  }

  const peakCareerAges: number[] = []
  if (elementCount['화'] >= 2) {
    peakCareerAges.push(35)
  }
  if (elementCount['금'] >= 2) {
    peakCareerAges.push(45)
  }
  if (elementCount['수'] >= 2) {
    peakCareerAges.push(55)
  }
  if (peakCareerAges.length === 0) {
    peakCareerAges.push(40, 50)
  }
...
export function analyzeHealthCareer(pillars: SajuPillars): HealthCareerComprehensive {
  ...
  if (
    career.workStyle.type === '열정형' &&
    health.organHealth.find((o) => o.element === '화' && o.status === 'vulnerable')
  ) {
...
export function getElementRecommendations(weakElements: FiveElement[]): ElementRecommendations {
  ...
  if (weakElements.includes('목')) {
    beneficialActivities.push('산책', '등산', '원예 활동')
  }
  if (weakElements.includes('화')) {
    beneficialActivities.push('햇볕 쬐기', '사우나')
  }
  if (weakElements.includes('토')) {
    beneficialActivities.push('흙 만지기', '도예', '요리')
  }
  if (weakElements.includes('금')) {
    beneficialActivities.push('호흡 운동', '악기 연주')
  }
  if (weakElements.includes('수')) {
    beneficialActivities.push('수영', '명상', '독서')
  }
