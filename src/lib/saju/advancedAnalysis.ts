...
function isHelpingElement(dayMasterElement: FiveElement, targetElement: FiveElement): boolean {
  ...
  // 일간을 생하는 오행 (인성)
  if (FIVE_ELEMENT_RELATIONS.생받는관계[dayMasterElement] === targetElement) {
    return true
  }
  return false
}
...
function getElementCategory(
  ...
  if (FIVE_ELEMENT_RELATIONS.생하는관계[dayMasterElement] === targetElement) {
    return '식상'
  }
  if (FIVE_ELEMENT_RELATIONS.극하는관계[dayMasterElement] === targetElement) {
    return '재성'
  }
  if (FIVE_ELEMENT_RELATIONS.극받는관계[dayMasterElement] === targetElement) {
    return '관성'
  }
  return '비겁' // fallback
}
...
export function analyzeGeokguk(
  ...
  if (strength.level === '극신약') {
    ...
    if (
      strength.details.재성 >= strength.details.관성 &&
      strength.details.재성 >= strength.details.식상
    ) {
      return {
        type: '종재격',
...
export function analyzeYongsin(
  ...
  if (strength.level === '극신강' || strength.level === '신강') {
    ...
    // 가장 부족한 오행을 용신으로
    if (
      strength.details.관성 <= strength.details.재성 &&
      strength.details.관성 <= strength.details.식상
    ) {
      return {
...
export function evaluateElementInfluence(
  ...
  if (element === yongsin.primary) {
    return '용신'
  }
  if (yongsin.favorable.includes(element)) {
    return '희신'
  }
  if (yongsin.unfavorable.includes(element)) {
    return '기신'
  }
  // 그 외는 한신(閑神) 또는 구신(仇神)
  return '한신'
...
export function scoreUnseElement(
  ...
  // 천간 점수
  if (stemElement === yongsin.primary) {
    score += 3
  } else if (yongsin.secondary && stemElement === yongsin.secondary) {
    score += 2
  } else if (yongsin.favorable.includes(stemElement)) {
    score += 1
  } else if (yongsin.unfavorable.includes(stemElement)) {
    score -= 2
  }

  // 지지 점수
  if (branchElement === yongsin.primary) {
    score += 3
  } else if (yongsin.secondary && branchElement === yongsin.secondary) {
    score += 2
  } else if (yongsin.favorable.includes(branchElement)) {
    score += 1
  } else if (yongsin.unfavorable.includes(branchElement)) {
    score -= 2
  }
...
export function analyzeRoot(
  ...
  for (const branch of branches) {
    ...
    for (const { stem, weight } of hiddenStems) {
      ...
      if (
        stem.element === dayElement ||
        FIVE_ELEMENT_RELATIONS.생받는관계[dayElement] === stem.element
      ) {
...
// 조후용신 테이블
const JOHU_YONGSIN_TABLE: Record<
  string,
  Record<Season, { primary: FiveElement; secondary: FiveElement }>
> = {
  甲: {
    춘: { primary: '화', secondary: '수' },
    하: { primary: '수', secondary: '금' },
    추: { primary: '화', secondary: '수' },
    동: { primary: '화', secondary: '토' },
  },
  乙: {
    춘: { primary: '화', secondary: '수' },
    하: { primary: '수', secondary: '금' },
  ...
  丙: {
    춘: { primary: '목', secondary: '토' },
    하: { primary: '수', secondary: '금' },
    추: { primary: '목', secondary: '토' },
    동: { primary: '목', secondary: '토' },
  },
  丁: {
    춘: { primary: '목', secondary: '금' },
    하: { primary: '수', secondary: '금' },
    추: { primary: '목', secondary: '토' },
    동: { primary: '목', secondary: '토' },
  },
  戊: {
    춘: { primary: '화', secondary: '금' },
    하: { primary: '수', secondary: '금' },
    추: { primary: '화', secondary: '목' },
    동: { primary: '화', secondary: '목' },
  },
  己: {
    춘: { primary: '화', secondary: '금' },
    하: { primary: '수', secondary: '금' },
  ...
  庚: {
    춘: { primary: '토', secondary: '화' },
    하: { primary: '수', secondary: '토' },
    추: { primary: '화', secondary: '목' },
    동: { primary: '화', secondary: '토' },
  },
  辛: {
    춘: { primary: '토', secondary: '수' },
    하: { primary: '수', secondary: '토' },
    추: { primary: '화', secondary: '수' },
    동: { primary: '화', secondary: '토' },
  },
  壬: {
    춘: { primary: '금', secondary: '토' },
    하: { primary: '금', secondary: '토' },
    추: { primary: '화', secondary: '목' },
    동: { primary: '화', secondary: '목' },
  },
  癸: {
    춘: { primary: '금', secondary: '화' },
    하: { primary: '금', secondary: '수' },
    추: { primary: '화', secondary: '목' },
    동: { primary: '화', secondary: '목' },
  },
}
