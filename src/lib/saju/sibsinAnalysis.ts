...
function getSibsinCategory(sibsin: SibsinType): SibsinCategory {
  if (sibsin === '비견' || sibsin === '겁재') {
    return '비겁'
  }
  if (sibsin === '식신' || sibsin === '상관') {
    return '식상'
  }
  if (sibsin === '편재' || sibsin === '정재') {
    return '재성'
  }
  if (sibsin === '편관' || sibsin === '정관') {
    return '관성'
  }
  return '인성'
}
...
export function analyzeSibsinPositions(pillars: SajuPillars): SibsinPosition[] {
  ...
  for (const p of pillarData) {
    positions.push({
      position: p.position,
      sibsin: getSibsin(dayStem, p.stem),
      stem: p.stem,
      hidden: false,
    })
  }
  ...
  for (const b of branchData) {
    ...
    if (jijangganData) {
      ...
      if (mainStem) {
        positions.push({
          position: b.position,
          sibsin: getSibsin(dayStem, mainStem),
          stem: mainStem,
          hidden: true,
        })
      }
...
export function analyzeSibsinPatterns(
  count: SibsinCount,
  categoryCount: SibsinCategoryCount
): SibsinPattern[] {
...
const SIBSIN_INTERACTIONS_MAP: Record<
  SibsinCategory,
  { generates: SibsinCategory; controls: SibsinCategory }
> = {
    ...
    for (const targetSibsin of presentSibsins) {
      ...
      if (targetCategory === generates) {
        interactions.push({
          ...
          from: sibsin,
          to: targetSibsin,
          description: `${sibsin}${iga(sibsin)} ${targetSibsin}${eulReul(targetSibsin)} 생함`,
          impact: 'positive',
        })
      }
      if (targetCategory === controls) {
        interactions.push({
          ...
          from: sibsin,
          to: targetSibsin,
          description: `${sibsin}${iga(sibsin)} ${targetSibsin}${eulReul(targetSibsin)} 극함`,
          impact: 'neutral',
        })
      }
    }
...
export function analyzeRelationshipPatterns(
  ...
  if (categoryCount.인성 >= 2) {
    patterns.push({
      ...
      sibsin: '정인',
      quality: 'good',
      description: '어머니의 영향력이 크며, 부모의 지원을 받음',
    })
    ...
    patterns.push({
      ...
      sibsin: '정인',
      quality: 'challenging',
      description: '부모와의 인연이 약하거나 일찍 독립',
    })
  }
  ...
  if (categoryCount.재성 >= 2) {
    patterns.push({
      type: '배우자',
      sibsin: '정재',
      quality: count.정재 >= 2 ? 'excellent' : 'good',
      description: count.정재 >= 2 ? '안정적인 결혼 생활' : '배우자와의 인연이 강함',
    })
  }
  ...
  if (categoryCount.식상 >= 2) {
    patterns.push({
      type: '자녀',
      sibsin: '식신',
      quality: count.식신 >= 2 ? 'excellent' : 'good',
      description: '자녀와의 인연이 깊음',
    })
  }
  ...
  if (categoryCount.비겁 >= 2) {
    patterns.push({
      type: '형제',
      sibsin: '비견',
      quality: count.비견 >= 2 ? 'good' : 'neutral',
      description: count.비견 >= 2 ? '형제자매와의 협력' : '형제간 경쟁 가능',
    })
    patterns.push({
      type: '친구',
      sibsin: '비견',
      quality: 'good',
      description: '동료와 친구 관계가 활발함',
    })
  }
  ...
  if (categoryCount.관성 >= 1) {
    patterns.push({
      type: '상사',
      sibsin: count.정관 >= count.편관 ? '정관' : '편관',
      quality: count.정관 >= count.편관 ? 'good' : 'challenging',
      description: count.정관 >= count.편관 ? '상사의 인정과 지원' : '상사와의 갈등 가능성',
    })
  }
...
export function analyzeSibsinComprehensive(pillars: SajuPillars): SibsinComprehensiveAnalysis {
  ...
  for (const sibsin of SIBSIN_TYPES) {
    if (count[sibsin] >= 2) {
      dominantSibsin.push(sibsin)
    }
    if (count[sibsin] === 0) {
      missingSibsin.push(sibsin)
    }
  }
  ...
  if (categoryCount.비겁 >= 2) {
    strengths.push('강한 자아와 독립심')
  } else if (categoryCount.비겁 === 0) {
    weaknesses.push('주체성 부족')
  }

  if (categoryCount.식상 >= 2) {
    strengths.push('창의력과 표현력')
  } else if (categoryCount.식상 === 0) {
    weaknesses.push('자기표현의 어려움')
  }

  if (categoryCount.재성 >= 2) {
    strengths.push('재물 관리와 현실 감각')
  } else if (categoryCount.재성 === 0) {
    weaknesses.push('재물 운용의 어려움')
  }

  if (categoryCount.관성 >= 2) {
    strengths.push('사회적 성취와 책임감')
  } else if (categoryCount.관성 === 0) {
    weaknesses.push('사회적 인정 부족 가능')
  }

  if (categoryCount.인성 >= 2) {
    strengths.push('학습 능력과 분석력')
  } else if (categoryCount.인성 === 0) {
    weaknesses.push('지원자 부족')
  }

  const lifeThemes: string[] = []
  if (dominantSibsin.includes('정관') || dominantSibsin.includes('편관')) {
    lifeThemes.push('직업과 사회적 성취')
  }
  if (dominantSibsin.includes('정재') || dominantSibsin.includes('편재')) {
    lifeThemes.push('재물과 경제적 안정')
  }
  if (dominantSibsin.includes('식신') || dominantSibsin.includes('상관')) {
    lifeThemes.push('창작과 자기표현')
  }
  if (dominantSibsin.includes('정인') || dominantSibsin.includes('편인')) {
    lifeThemes.push('학문과 정신적 성장')
  }
  if (dominantSibsin.includes('비견') || dominantSibsin.includes('겁재')) {
    lifeThemes.push('독립과 자기 확립')
  }
  ...
  for (const missing of missingSibsin.slice(0, 2)) {
    ...
    if (category === '비겁') {
      advice.push('자기 주장과 독립심을 키우세요')
    } else if (category === '식상') {
      advice.push('창의적 표현과 취미 활동을 늘리세요')
    } else if (category === '재성') {
      advice.push('재테크와 경제 관념을 길러세요')
    } else if (category === '관성') {
      advice.push('사회적 책임감과 직업 의식을 높이세요')
    } else if (category === '인성') {
      advice.push('학습과 자기계발에 투자하세요')
    }
  }
