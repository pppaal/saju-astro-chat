...
function getElementFromPillar(p: { heavenlyStem?: { element?: string } } | undefined): string {
...
function buildDecisiveTimings(
  ...
  return entries.map((entry) => ({
    ...entry,
    ageWindows: entry.ageWindows.length > 0 ? entry.ageWindows : [`${koreanAge}세 전후`],
  }))
}
...
function buildKarmicInsight(saju: CalculateSajuDataResult, geokguk?: string): KarmicInsight {
