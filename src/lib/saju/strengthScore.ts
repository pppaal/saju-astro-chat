...
export function calculateComprehensiveScore(
  ...
  // 용신 적합도
  const yongsin = calculateYongsinFitScore(pillars, options?.yongsin || '수', strength.level)
  ...
  if (strength.total >= 60) {
    ...
    recommendations.push(
      `인성(${FIVE_ELEMENT_RELATIONS['생받는관계'][getStemElement(pillars.day.heavenlyStem.name)]}) 오행을 보강하세요`
    )
  }
...
function generateScoreSummary(overall: number, grade: string, strengthLevel: string): string {
