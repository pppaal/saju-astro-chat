...
const PATTERN_DEFINITIONS: PatternDefinition[] = [
  // === 특수 구조 패턴 ===
  {
    ...
    matchFunction: (pillars) => {
      ...
      return {
        ...
        details: `최대 오행 비율: ${(ratio * 100).toFixed(1)}%`,
      }
    },
  ...
  },
  {
    ...
    matchFunction: (pillars) => {
      ...
      return {
        matched: balanced,
        score: Math.round(100 - variance * 20),
        details: `분산: ${variance.toFixed(2)}`,
      }
    },
    ...
    matchFunction: (pillars) => {
      ...
      for (let i = 0; i < branches.length; i++) {
        for (let j = i + 1; j < branches.length; j++) {
          if (YUKHAP_MAP[branches[i]] === branches[j]) {
            hapCount++
          }
        }
      }
  ...
  // === 충 패턴 ===
  {
    ...
    matchFunction: (pillars) => {
      ...
      for (let i = 0; i < branches.length; i++) {
        for (let j = i + 1; j < branches.length; j++) {
          if (CHUNG_MAP[branches[i]] === branches[j]) {
            chungCount++
          }
        }
      }
...
function generatePatternSummary(matches: PatternMatch[]): string {
  ...
  return (
    `주요 패턴: ${top3.map((m) => m.patternName).join(', ')}. ` +
 ...
 * 패턴 기반 추천
 */
export function getPatternRecommendations(patterns: PatternMatch[]): {
  careers: string[]
  activities: string[]
  cautions: string[]
} {
...
export function getPatternStatistics(patterns: PatternMatch[]): {
  ...
  const averageScore =
    patterns.length > 0 ? patterns.reduce((sum, p) => sum + p.matchScore, 0) / patterns.length : 0
