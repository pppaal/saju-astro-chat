...
export const HOUR_THEME_NARRATIVE: Record<Branch, Record<Theme, ThemeLine>> = {
  ...
  축: {
    love:   { ko: '연락보다 깊은 수면이 관계 회복에 도움이 되는 시간이에요.',
              en: 'Deep sleep helps relationships more than late messages now.' },
    money:  { ko: '돈 결정은 미루고 충분히 자는 편이 다음 날 판단에 좋아요.',
              en: 'Postpone money decisions; sleep helps tomorrow\'s judgment.' },
    career: { ko: '일은 내려놓고 푹 자는 게 다음 날 성과를 키워요.',
              en: 'Set work aside; sleep grows tomorrow\'s output.' },
  ...
  해: {
    love:   { ko: '예민한 주제는 다음 날로 미루고 따뜻한 인사로 마무리하세요.',
              en: 'Postpone sensitive topics; end with a warm note.' },
    money:  { ko: '재정 결정은 미루고 충분한 수면이 다음 날 판단을 도와줘요.',
              en: 'Postpone money calls; sleep aids tomorrow\'s judgment.' },
    career: { ko: '내일 핵심 업무 1-2개만 메모해두면 다음 날이 가벼워져요.',
              en: 'Note 1-2 core tasks for tomorrow — the next day feels lighter.' },
    health: { ko: '늦은 식사·SNS·자극적인 콘텐츠는 수면을 흩뜨려요.',
              en: 'Late meals, social media, intense content disrupt sleep.' },
    growth: { ko: '잠들기 전 짧은 영감 노트나 가벼운 독서가 자리잡기 좋아요.',
              en: 'Pre-sleep insight notes or light reading settle here.' },
  },
}
