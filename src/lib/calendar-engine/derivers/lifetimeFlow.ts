...
const TONE_VARIANTS_KO: Record<'good' | 'hard' | 'mid', readonly string[]> = {
  good: [
    ...
    '내딛는 걸음마다 길이 열리는 때라, 미뤄뒀던 거 지금 지르면 멀리 가요.',
    '뿌린 게 어렵잖게 자라는 시기예요. 애쓴 것보다 더 크게 돌아와요.',
    '사람도 기회도 곁에 모이는 때예요. 혼자 낑낑대던 게 갑자기 수월해져요.',
    '때가 딱 받쳐주는 구간이라, 작은 시도도 곧잘 결과로 맺혀요. 자신감 가져도 되는.',
    '막히는 게 적어서, 품고만 있던 큰 뜻을 멀리 밀어붙이기 좋은 때예요.',
  ],
  hard: [
    '남들은 쉽게 가는 것 같은데 나만 매번 애쓰는 것 같은 때예요. 근데 이때 버틴 게, 나중에 남들 없는 뚝심이 돼요.',
    '해도 해도 뜻대로 안 풀려서 “나만 제자리인가” 싶어지는 시기예요. 근데 여기서 안 놓은 사람이 결국 오래가요.',
    '부딪히고 깎이는 일이 많은데, 이상하게 그럴수록 속은 단단해져요. 지나고 보면 이때가 나를 만든 구간.',
    '길이 좁아진 느낌이라, 밀어붙이기보다 때를 고르는 게 나은 때예요. 조급함만 내려놓으면 견딜 만해요.',
    '맞바람이 자주 부는 구간이에요. 근데 그 저항이 결국 당신 뿌리를 더 깊게 박아줘요.',
    '쉽지 않은 고비를 하나 지나는 때예요. 힘들지만, 여기서 배운 게 평생 가는 무기가 돼요.',
    '지금은 무리해서 나아갈 때가 아니라 숨 고르고 채비할 때예요. 물러서는 게 지는 게 아니에요.',
  ],
  mid: [
    '큰 사건은 없어요. 근데 그게 나빠서가 아니라, 조용히 내 실력 쌓기 딱 좋은 때라 그래요. 티 안 나도 속으로 단단해지는.',
    '한 방보다, 그동안 쌓은 게 하나씩 결과로 돌아오는 때예요. 반짝보다 오래 가는 사람의 구간.',
    '평지를 고르게 걷는 느낌이에요. 내 보폭만 지키면 무리 없이 흘러가는.',
    '큰일보다, 하루하루 포개지는 게 힘이 되는 때예요. 꾸준함이 제일 빛나는.',
    '기복이 적은 구간이라, 오히려 흔들림 없이 갈 수 있는 때예요.',
    '무리만 안 하면 무던하게 흘러가는 시기예요. 욕심 안 부려도 되는.',
    '눈에 띄는 변화보다, 어질러진 걸 차분히 정리하기 좋은 때예요.',
  ],
}
const TONE_VARIANTS_EN: Record<'good' | 'hard' | 'mid', readonly string[]> = {
  good: [
    ...
    'A path opens with each step, so whatever you’ve been putting off, push it now and it carries far.',
    'What you planted grows without much fight — it comes back bigger than the effort you put in.',
    'People and chances gather close; what you’ve been struggling with alone suddenly gets easier.',
    'The timing has your back, so even small attempts land as real results. You’re allowed to feel confident.',
    'Little gets in the way — a good stretch to push that big idea you’ve only been holding onto.',
  ],
  hard: [
    ...
    'Plenty knocks into you and wears you down, yet somehow that’s exactly what hardens your core. Looking back, this is the stretch that made you.',
    'The road feels narrow, so it’s better to pick your moment than to force it. Drop the rush and it’s bearable.',
    'Headwinds blow often here — but that resistance is what drives your roots in deeper.',
    'You pass through one real hard patch. It’s tough, but what you learn here becomes a weapon that lasts a lifetime.',
    'This isn’t the time to push forward — it’s the time to catch your breath and prepare. Stepping back isn’t losing.',
  ],
  mid: [
    ...
    'It feels like walking even ground — keep your own stride and it flows without trouble.',
    'Less about big moments, more about days stacking up. This is where steadiness shines brightest.',
    'Few swings here, so you can actually move without being rattled.',
    'As long as you don’t overreach, it flows along easily. No need to force it.',
    'Less visible change — a good time to calmly put what’s scattered back in order.',
  ],
}
