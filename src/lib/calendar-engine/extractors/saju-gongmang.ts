...
const GONGMANG_BASE_WEIGHT = 0.30
...
const sajuGongmangExtractor: SignalExtractor = {
  ...
  extract(ctx: ExtractorContext): ActiveSignal[] {
    ...
    // 입춘(절기) — 세운 경계.
    const ipchun = (y: number): Date =>
      getSolarTermKST(y, 2) ?? new Date(Date.UTC(y, 1, 4))
    ...
    for (const d of natal.saju.daeun) {
      ...
      signals.push(makeGongmangSignal({
...
function makeGongmangSignal(args: MakeArgs): ActiveSignal {
  const weight = Math.min(
    GONGMANG_BASE_WEIGHT + LAYER_WEIGHT_FACTOR[args.layer] * 0.20,
    0.55,
  )
  return {
