...
function themesForPattern(category: string, keywords: string[]): AstroThemeKey[] {
  ...
  if (/love|romance|연애|도화/.test(text)) themes.add('love')
  if (/career|관성|직업|관운/.test(text)) themes.add('career')
  if (/study|학문|문창/.test(text)) themes.add('study')
  if (/health|건강/.test(text)) themes.add('health')
  if (/family|가족/.test(text)) themes.add('family')
  if (/reputation|명예|장성/.test(text)) themes.add('reputation')
  if (/creative|창의|식상/.test(text)) themes.add('creativity')
  if (/spiritual|영성|화개/.test(text)) themes.add('spirituality')
  if (themes.size === 0) themes.add('personality')
