...
import { RETROGRADE_SCHEDULE } from '@/lib/calendar-engine/data/retrogradeSchedule'
...
const TWELVE_STAGE_SCORES: Record<
  TwelveStage,
  { score: number; energy: MonthlyTimingScore['stageEnergy']; description: string }
> = {
...
function getRetrogradeEffects(year: number, month: number): RetrogradeEffect[] {
  ...
  const checkRetrograde = (
    planet: string,
    periods:
      | readonly { readonly start: string; readonly end: string; readonly sign: string }[]
      | undefined,
    baseScore: number,
    advice: string[]
  ) => {
  ...
  checkRetrograde('토성', schedule.saturn, -10, ['구조 재정비', '장기 계획 재검토', '책임 재평가'])
...
export function calculateMonthlyTimingScore(
  params: CalculateMonthlyScoreParams
): MonthlyTimingScore {
...
export function calculateDetailedConfidence(
  ...
  const overall = Math.round(
    dataQuality * 0.3 + methodAlignment * 0.35 + cycleSynchrony * 0.2 + historicalAccuracy * 0.15
  )
...
export function generateYearlyPrediction(params: GenerateYearlyPredictionParams): YearlyPrediction {
  ...
  const annualElements: FiveElement[] = [
    '수',
    '토',
    '목',
    '목',
  ...
  const yearTheme =
    avgScore >= 65 ? '성장과 발전의 해' : avgScore >= 50 ? '안정과 유지의 해' : '재정비와 성찰의 해'
  ...
  return {
    ...
    challengingMonths,
    yearTheme,
    daeunPhase: params.currentDaeunElement
      ? `${params.currentDaeunElement} 대운`
      : '대운 정보 없음',
    annualElement,
...
export function generatePredictionPromptContext(
