 ...
 */
export async function getCachedPremiumStatus(userId: string): Promise<boolean | null> {
 ...
 */
export async function checkPremiumFromSubscription(userId: string): Promise<boolean> {
  // Check cache first
  const cached = await getCachedPremiumStatus(userId)
  if (cached !== null) {
    return cached
