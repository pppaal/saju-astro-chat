...
export const loveDeepRules: Rule[] = [
  // ─── state: 평생 관계 결 ─────────────────────────────────
  {
    ...
    astroPredicate: (a) =>
      hitByKeys(a, ['astro.state.dignity.Venus.domicile', 'astro.state.dignity.Venus.exaltation']),
  },
  {
  ...
  },
  {
    ...
    sajuPredicate: (s) => hitByKeys(s, ['saju.relation.day.공망', 'saju.state.shinsal.화개']),
    astroPredicate: (a) =>
      hitByKeys(a, ['astro.state.planet.Saturn.house.7', 'astro.state.retrograde.Venus']),
  },
  {
    ...
    sajuPredicate: (s) => hitByKeys(s, ['saju.state.shinsal.도화', 'saju.state.shinsal.홍염']),
    astroPredicate: (a) =>
      hitByPrefix(a, [
  ...
  },
  {
    ...
    sajuPredicate: (s) =>
      hitByKeys(s, ['saju.state.sibsin.strong.정관', 'saju.state.geokguk.정관격']),
    astroPredicate: (a) =>
      hitByKeys(a, ['astro.state.planet.Ascendant.sign.Libra', 'astro.state.stellium.house.7']),
  },
  {
    ...
    sajuPredicate: (s) =>
      hitByPrefix(s, ['saju.state.sibsinGroup.비겁.strong', 'saju.state.sibsinGroup.식상.strong']),
    astroPredicate: (a) =>
      hitByPrefix(a, ['astro.state.planet.Mars.house.7', 'astro.state.planet.Uranus.house.7']),
  },
  {
  ...
  },
  {
    ...
    sajuPredicate: (s) =>
      hitByKeys(s, ['saju.state.sibsin.strong.편관', 'saju.state.geokguk.양인격']),
    astroPredicate: (a) =>
      hitByPrefix(a, [
  ...
  // 길신·길성이 금성을 도움 = 관계에 복·도움이 따르는 결.
  {
    id: 'love.state.graced-affection',
    layer: 'state',
    domain: 'love',
    meaning: '관계의 복 (보호받는 인연)',
    polarityHint: 'pos',
    narrative: {
      confirm:
        '사주 길신(천을귀인 등)이 받쳐주고 + 점성 길성(목성·금성)이 금성을 도와주거나 금성이 본인 자리에 듦 — 관계에 복과 도움이 자연스럽게 따르는 결. 사람 복이 인생의 자산이에요.',
    },
    sajuPredicate: (s) =>
      hitByPrefix(s, ['saju.state.shinsal.lucky.', 'saju.state.palace.배우자.sibsin.정관']),
    astroPredicate: (a) =>
      hitByPrefix(a, [
        'astro.state.bonif.overcoming.benefic.Venus',
        'astro.state.bonif.opposition.benefic.Venus',
        'astro.state.bonif.adherence.benefic.Venus',
        'astro.state.bonified.Venus',
        'astro.state.sectBenefic.greater.Venus',
        'astro.state.sectBenefic.lesser.Venus',
  ...
  },
  {
    ...
    sajuPredicate: (s) =>
      hitByKeys(s, ['saju.timing.seun.sibsin.정관', 'saju.timing.seun.sibsin.정재']),
    astroPredicate: (a) =>
      hitByKeys(a, ['astro.timing.transit.Saturn.house.7', 'astro.timing.transit.Jupiter.house.7']),
  },
  {
    ...
    sajuPredicate: (s) =>
      hitByPrefix(s, ['saju.timing.event.day.지지충', 'saju.timing.event.day.지지형']),
    astroPredicate: (a) =>
      hitByPrefix(a, [
  ...
  },
  {
    ...
    sajuPredicate: (s) =>
      hitByPrefix(s, ['saju.timing.event.day.지지육합', 'saju.timing.event.day.지지삼합']),
    astroPredicate: (a) =>
      hitByPrefix(a, [
  ...
  },
  {
    ...
    sajuPredicate: (s) =>
      hitByKeys(s, ['saju.timing.seun.sibsin.상관', 'saju.timing.seun.sibsinJi.도화']),
    astroPredicate: (a) =>
      hitByPrefix(a, [
