...
const JUPITER_HOUSE_LINES: Record<HouseNumber, string> = {
  1: '자기 자신·정체성에 풍요와 자신감이 깃드는 자리, 본인이 곧 기회의 통로인 결',
  2: '자원·소유·수입의 길이 넉넉히 열리는 결, 가치가 풍성하게 쌓이는 자리',
  3: '학습·소통·이웃과의 교류에서 기회가 트이는 결, 말과 글이 자산이 되는 자리',
  4: '가정·뿌리·부동산이 풍요의 무대인 결, 안에서부터 크게 자라는 자리',
  5: '창작·연애·자녀의 자리에서 행운이 가장 또렷한 결, 표현이 풍요로 이어지는 자리',
  6: '일상의 노동·건강 관리에서 의미가 커지는 결, 작은 봉사가 큰 자산이 되는 자리',
  7: '관계·파트너십이 확장의 통로인 결, 사람을 통해 세계가 넓어지는 자리',
  8: '심층 변화·공유 자원에서 큰 기회가 열리는 결, 위기 뒤에 풍요가 따라오는 자리',
  9: '먼 곳·학문·믿음에서 본인다움이 가장 크게 펼쳐지는 결, 확장이 곧 정체성인 자리',
  10: '사회 무대·커리어가 풍요의 정점인 결, 공적 성취가 그대로 자산이 되는 자리',
  11: '커뮤니티·미래 비전이 기회의 통로인 결, 친구와 그룹에서 풍요가 들어오는 자리',
  12: '내면·영성·은둔에서 큰 의미가 자라는 결, 보이지 않는 곳에서 풍요가 익는 자리',
}
...
const SATURN_HOUSE_LINES: Record<HouseNumber, string> = {
  1: '자기 자신·정체성에 무게가 실린 결, 일찍부터 어른의 옷을 입는 자리',
  2: '자원·소유·가치 위에 책임이 얹히는 결, 천천히 단단해지는 자리',
  3: '소통·학습이 단련을 거치는 결, 말과 사고에 신중함이 자리잡는 결',
  4: '가정·뿌리에 무게와 책임이 깔리는 결, 안에서부터 단단해져야 하는 자리',
  5: '창작·연애·자녀의 자리에서 시험이 따라오는 결, 표현이 책임으로 다듬어지는 자리',
  6: '일상·노동·건강의 자리에서 꾸준함이 빛나는 결, 루틴이 곧 구조인 자리',
  7: '관계 안에서 책임과 시간이 시험대인 결, 오래 견디는 만남이 본인을 빚는 자리',
  8: '심층 변화·공유 자원에서 깊은 책임이 쌓이는 결, 위기와 마주해 단단해지는 자리',
  9: '신념·학문·먼 곳에서 단련이 따라오는 결, 천천히 익히는 진리의 자리',
  10: '커리어·사회 무대가 본인의 정점이자 시험대인 결, 평판이 곧 구조인 자리',
  11: '커뮤니티·미래 비전에 책임이 따라오는 결, 그룹 안에서 어른 역할이 자리잡는 결',
  12: '내면의 그림자·고독에서 깊은 단련이 일어나는 결, 보이지 않는 무게를 짊어지는 자리',
}
...
const URANUS_HOUSE_LINES: Record<HouseNumber, string> = {
  1: '자기 자신을 끊임없이 새로 정의하는 결, 정체성이 곧 파격인 자리',
  2: '자원·가치에 파격이 따라오는 결, 새로운 방식으로 손에 잡는 자리',
  3: '사고·소통에 돌발과 영감이 깃드는 결, 한 박자 빠른 통찰의 자리',
  4: '가정·뿌리가 보통과 다른 결, 떠나고 다시 만드는 자리',
  5: '창작·연애의 자리에서 자유가 가장 또렷한 결, 틀을 벗는 표현의 자리',
  6: '일상·노동의 자리에서 혁신이 풀리는 결, 새로운 방식의 작업이 본인의 결',
  7: '관계가 보통과 다른 결, 자유와 평등이 만남의 조건인 자리',
  8: '심층·금기·공유 자원에서 파격이 일어나는 결, 변혁이 본인을 깨우는 자리',
  9: '신념·학문에 혁명이 깃드는 결, 기존 진리를 부수고 새 길을 여는 자리',
  10: '커리어가 보통과 다른 결, 새로운 직업·길을 여는 자리',
  11: '커뮤니티·비전 안에서 혁명이 펼쳐지는 결, 미래의 그룹에 본인이 깃드는 자리',
  12: '무의식·은둔에서 돌발적 통찰이 터지는 결, 보이지 않는 자리에서 깨어나는 결',
}
...
const NEPTUNE_HOUSE_LINES: Record<HouseNumber, string> = {
  1: '자기 자신이 안개·꿈처럼 흐르는 결, 정체성이 유동적인 자리',
  2: '자원·소유의 경계가 흐려지는 결, 가치가 영적인 방향으로 옮겨지는 자리',
  3: '소통·사고가 직관에 잠기는 결, 말이 시처럼 흐르는 자리',
  4: '가정·뿌리에 영적 결이 깃드는 결, 집이 곧 성소가 되는 자리',
  5: '창작·연애의 자리에서 환상이 가장 짙은 결, 영감이 표현으로 직접 흐르는 자리',
  6: '일상·건강의 경계가 모호해지는 결, 돌봄이 영적인 일이 되는 자리',
  7: '관계가 환상에 잠기는 결, 상대를 통해 무한을 보는 자리',
  8: '심층·공유 자원이 신비에 잠기는 결, 변혁과 영성이 같은 자리에 모이는 결',
  9: '신념·학문이 영성에 녹는 결, 진리가 직관으로 다가오는 자리',
  10: '커리어가 비전·예술에 잠기는 결, 공적 자리에서 환상이 자산이 되는 결',
  11: '커뮤니티·미래 비전이 영적인 결, 큰 꿈이 그룹과 함께 흐르는 자리',
  12: '내면·은둔·영성이 본인의 본거지인 결, 보이지 않는 자리에서 가장 또렷한 결',
}
...
const PLUTO_HOUSE_LINES: Record<HouseNumber, string> = {
  1: '자기 자신을 통째로 다시 빚는 결, 정체성이 죽고 다시 태어나는 자리',
  2: '자원·소유·가치의 격변을 거치는 결, 가치 체계가 통째로 새로 쓰이는 자리',
  3: '사고·소통이 깊이 변혁되는 결, 말 한 마디에 권력이 실리는 자리',
  4: '가정·뿌리의 격변을 거치는 결, 가족 패턴이 통째로 다시 짜이는 자리',
  5: '창작·연애의 자리에서 강렬함이 가장 또렷한 결, 표현이 곧 변혁인 자리',
  6: '일상·건강이 변혁의 자리인 결, 작은 루틴 안에서 깊은 권력이 풀리는 결',
  7: '관계의 격변을 통해 본인이 다시 빚어지는 결, 상대가 곧 거울인 자리',
  8: '심층·공유 자원이 본인의 정체성 자체인 결, 변혁이 본거지인 자리',
  9: '신념·학문이 변혁의 자리인 결, 진리가 죽고 다시 태어나는 자리',
  10: '커리어·권력이 정점에 닿는 결, 사회 무대에서 통째로 다시 빚어지는 자리',
  11: '커뮤니티·집단의 변혁을 이끄는 결, 그룹의 권력 구조가 본인 손에 흐르는 자리',
  12: '무의식의 변혁이 일어나는 결, 보이지 않는 자리에서 죽고 다시 태어나는 결',
}
