...
export function buildWisdom(input: BuilderInput): DomainNarrative {
  ...
  if (sangwan >= 2) {
    ...
    p2piecesEn.push(
      'Strong 상관 means wisdom hardens when you teach and express, not only when you absorb.'
    )
  } else if (sikshin >= 2) {
    p2pieces.push('여유로운 표현의 자질이 깔려 있어, 즐기듯 배우는 방식이 잘 맞아요.')
    p2piecesEn.push(
      '식신 places ease at the centre — learning works best when it doubles as enjoyment.'
    )
  }
  ...
  const p2en = paragraph(
    p2piecesEn.length
      ? p2piecesEn
      : [
          'Your learning grain is evenly distributed — keeping multiple channels open lets luck flow more naturally.',
        ]
  )
  ...
  // Pallas × house — 전략·지혜 소행성 (destiny-matrix layer9 활용)
  const pal = pallas(astro)
  if (pal) {
    astroUsed.push('asteroids.pallas')
    const palCrossKo = asteroidHouseLine('Pallas', pal.house, 'ko')
    const palCrossEn = asteroidHouseLine('Pallas', pal.house, 'en')
    if (palCrossKo) {
      astroUsed.push('pools.asteroid.pallas.house')
      p3pieces.push(palCrossKo)
      p3piecesEn.push(palCrossEn)
    }
  }
  if (nn) {
    p3pieces.push(
  ...
  if (h9 && h9.strength >= 40) {
    ...
    p3piecesEn.push(
      'Harmonics 9 resonates clearly in your chart — pushing one field to its bottom unlocks insight beyond ordinary study.'
    )
  } else if (h9) {
    p3pieces.push(
    ...
    p3piecesEn.push(
      'Harmonics 9 sits quietly — deliberate reflection and meditation will strengthen it over time.'
    )
  }
  if (geokguk && geokguk.includes('인')) {
    p3pieces.push(
      '인생의 큰 패턴 자체가 인성 격국이라, 가르치고 다루는 사람이 되는 길이 가장 본인답게 풀려요.'
    )
    p3piecesEn.push(
      `Your geokguk (${geokguk}) is in the 인성 family — the teacher/carrier path fits most natively.`
    )
  }
  if (munchang) {
    p3pieces.push('학문과 창작의 별이 명식에 들어와 있어서, 글·이론·자료를 통해 성취가 빨라요.')
    p3piecesEn.push(
      `A 문창 (literary star) lights your chart — writing, theory and material study accelerate your achievement.`
    )
  }
  if (hakdang) {
    p3pieces.push('학당의 별도 함께 있어, 공식적인 교육과 자격 과정에서 운이 잘 풀려요.')
    p3piecesEn.push(
      `A 학당 (academy) star also sits in your chart — formal education and accreditation tracks suit you.`
    )
  }
  ...
  if (relKoWisdom) {
    ...
    if (relEnWisdom)
      p3piecesEn.push(
        `${relEnWisdom} Pairing with teachers or books carries the learning foundation.`
      )
  }
  ...
  if (daimon) {
    ...
    p3piecesEn.push(
      `Your Lot of Daimon in ${signLabel(daimon.sign, 'en')} — the seat where real wisdom grows shares that grain.`
    )
  }
  ...
  const p3en = paragraph(
    p3piecesEn.length
      ? p3piecesEn
      : ['Current signals favour deepening what you already know rather than chasing new domains.']
  )
  ...
  if (inseong >= 2) {
    ...
    guidePiecesEn.push(
      'Explain what you learn to someone right away — teaching is when knowledge becomes wisdom.'
    )
  } else {
    guidePiecesKo.push('주제를 좁히고 한 분야에 머무는 시간을 늘려보세요. 깊이가 운을 끌어옵니다.')
    guidePiecesEn.push('Narrow the topic and stay longer in one field — depth attracts luck.')
  }
  if (profIsLearning && prof) {
    ...
    guidePiecesEn.push(
      `This year activates house ${prof.house} — learning and expansion will translate into tangible results.`
    )
  }
