...
export function buildWisdom(input: BuilderInput): DomainNarrative {
  ...
  if (sangwan >= 2) {
    ...
    p2piecesEn.push(
      'A strong free-talent current means wisdom hardens when you teach and express, not only when you absorb.'
    )
  } else if (sikshin >= 2) {
    p2pieces.push('여유로운 표현의 자질이 깔려 있어, 즐기듯 배우는 방식이 잘 맞아요.')
    p2piecesEn.push(
      'An easeful-expression current places ease at the centre — learning works best when it doubles as enjoyment.'
    )
  }
  ...
  if (inseong >= 2) {
    ...
    guidePiecesEn.push(
      'Record one thing each day — your wisdom-and-care current grows through notes.'
    )
  } else if (sangwan >= 2) {
