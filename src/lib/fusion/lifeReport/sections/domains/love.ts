...
export function buildLove(input: BuilderInput): DomainNarrative {
  ...
  if (j) {
    ...
    // 소행성-하우스 cross (destiny-matrix layer9 — 48 entry orphan 활용)
    const junoCrossKo = asteroidHouseLine('Juno', j.house, 'ko')
    const junoCrossEn = asteroidHouseLine('Juno', j.house, 'en')
    if (junoCrossKo) {
      astroUsed.push('pools.asteroid.juno.house')
      deepKo.push(junoCrossKo)
      deepEn.push(junoCrossEn)
    }
  }
  if (pofInSeventh) {
