...
export function buildFamily(input: BuilderInput): DomainNarrative {
  ...
  const p1ko = paragraph([
    inseong >= 2
      ? '당신은 어머니·돌봄 라인의 인연이 자연스럽게 깊은 사람이에요.'
      : inseong === 0
        ? '어머니·돌봄 라인의 인연은 의식적 노력으로 자라요.'
        : '당신은 가족과의 결을 잔잔하게 가져가는 사람이에요.',
    `사주의 인성은 ${inseong}개, 비겁은 ${bijeon}개로 ${familyShapeKo(inseong, bijeon)}.`,
    fourth
      ? `점성의 4집은 ${signLabel(fourth.sign, 'ko')}에서 시작해서, 가정의 결이 ${fourthSignFlavorKo(fourth.sign)}이에요.`
      : '',
  ])
  ...
  const p2ko = paragraph([
    sun
      ? `아버지상은 ${signLabel(sun.sign, 'ko')}·${houseLabel(sun.house, 'ko')}에 자리한 태양처럼, ${parentSignFlavorKo(sun.sign)}이 결이에요.`
      : '',
    moon
      ? `어머니상은 ${signLabel(moon.sign, 'ko')}·${houseLabel(moon.house, 'ko')}의 달처럼, ${parentSignFlavorKo(moon.sign)}이 결이에요.`
      : '',
    bijeon >= 2
      ? '비겁이 풍성해 형제·동등 관계가 인생에 큰 자리를 차지해요.'
      : bijeon === 0
        ? '비겁이 없어 동등 관계는 의식적으로 만드는 결이에요.'
        : '',
  ])
  ...
  if (sunMoon) {
    deepKo.push(
      `왜냐하면 부모 축인 태양과 달이 ${aspectQuality(sunMoon.type, 'ko')} ${sunMoonFlavorKo(sunMoon.type)} 가정 분위기를 만들어 두었기 때문이에요.`
    )
    deepEn.push(
  ...
  if (moonSaturn) {
    deepKo.push(
      `달과 토성이 ${aspectQuality(moonSaturn.type, 'ko')} 어머니·정서 라인에 책임의 무게가 일찍 실린 결이에요.`
    )
    deepEn.push(
  ...
  if (yearOrMonthGongmang) {
    deepKo.push('년·월주에 공망이 있어, 조부모·부모 라인 인연이 얇거나 일찍 떨어지는 결이 있을 수 있어요.')
    deepEn.push('A 공망 (void) sits on the year or month pillar — ancestor / parent line can feel thinner or separates earlier.')
  }
  if (ves) {
    deepKo.push(`Vesta가 ${signLabel(ves.sign, 'ko')}·${houseLabel(ves.house, 'ko')}에 있어 가족에 헌신하는 결이 있어요.`)
    deepEn.push(`Vesta in ${signLabel(ves.sign, 'en')} ${houseLabel(ves.house, 'en')} brings family-devotion as a thread.`)
  }
  if (ce) {
    deepKo.push(`Ceres가 ${signLabel(ce.sign, 'ko')}·${houseLabel(ce.house, 'ko')}에 있어 ${ceresMomFlavorKo(ce.house)} 양육의 결이 새겨져 있어요.`)
  ...
  const p3ko = paragraph(deepKo.length ? deepKo : [
    '왜냐하면 가족 신호들이 평탄히 정렬되어 있어, 큰 드라마보다 잔잔한 연속이 인생의 무대에요.'
  ])
...
function familyShapeKo(inseong: number, bijeon: number): string {
  if (inseong >= 2 && bijeon >= 2) return '돌봄과 동등 관계가 둘 다 풍부한 그릇이에요'
  if (inseong >= 2) return '돌봄 라인이 두텁고 형제 라인은 잔잔해요'
  if (bijeon >= 2) return '형제·동료 라인이 두텁고 돌봄은 잔잔해요'
  if (inseong === 0 && bijeon === 0) return '가족 신호가 가벼워, 본인이 새 가족을 만드는 결이에요'
  return '돌봄과 동등의 결이 균형적으로 흐릅니다'
}
