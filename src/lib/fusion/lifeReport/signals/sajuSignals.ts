...
function dayMasterRootPhraseKo(
  ...
  if (level === 'strong') {
    return pillarKo
      ? `자기 자신의 힘이 ${pillarKo}를 단단히 잡고 있어서, 흔들리지 않는 중심이 있어요.`
      : '자기 자신의 힘이 단단해서, 흔들리지 않는 중심이 있어요.'
  }
  if (level === 'moderate') {
    return pillarKo
      ? `자기 자신의 힘이 ${pillarKo}에 무난히 자리 잡고 있어서, 자기다움을 차분히 지켜가시는 편이에요.`
      : '자기 자신의 힘이 무난해서, 자기다움을 차분히 지켜가시는 편이에요.'
  }
  if (level === 'weak') {
    return '자기 자신의 힘이 약한 편이라, 주변 환경이 본인에게 큰 영향을 줘요.'
  }
  return '자기 자신만의 힘은 약한 편이지만, 주변에 맞춰가며 자기 자리를 잡아가시는 분이에요.'
}
