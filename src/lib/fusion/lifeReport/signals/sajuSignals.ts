...
function dayMasterRootPhraseKo(
  ...
  if (level === 'strong') {
    return pillarKo
      ? `자기 자신의 힘이 ${pillarKo}에 단단히 자리잡아, 흔들리지 않는 중심이 있어요.`
      : '자기 자신의 힘이 단단히 자리잡아, 흔들리지 않는 중심이 있어요.'
  }
  if (level === 'moderate') {
    return pillarKo
      ? `자기 자신의 힘이 ${pillarKo}에 무난히 자리잡아, 자기다움을 차분히 지켜가요.`
      : '자기 자신의 힘이 무난히 자리잡아, 자기다움을 차분히 지켜가요.'
  }
  if (level === 'weak') {
    return '자기 자신의 힘이 약한 편이라, 주변 환경이 본인에게 큰 영향을 줘요.'
  }
  return '자기 자신만의 힘은 약하지만, 주변 환경에 맞춰가며 자기 자리를 잡아가는 스타일이에요.'
}
