    ...
    return deps.formatNarrativeParagraphs(
      deps.sanitizeUserFacingNarrative(
        [
          ...
          '배우자상으로 보면 겉으로 화려한 사람보다 기준이 분명하고, 감정이 흔들릴 때도 생활을 무너뜨리지 않는 사람이 더 맞습니다. 대화가 잘 되는 것만큼이나 관계 간격과 경계 감각이 맞는지가 핵심입니다.',
          riskLabel
  ...
  if (lang === 'ko') {
    const timingLine = matrixRow?.cells?.length
      ? (() => {
          ...
          return '앞으로 몇 년은 열려 있는 흐름이 이어지지만, 움직이는 간격과 기준 관리가 결과를 더 크게 가를 가능성이 큽니다.'
        })()
      : ''
