// src/lib/matrix/ai-report/prompts/themedPrompts.ts
// 테마별 심화 리포트용 프롬프트 생성
...
- **교차 분석 핵심**: 사주와 점성술이 같은 연애 특징, 관계 속도, 재접근 가능성, 결혼 조건을 다른 방식으로 확인할 때 그게 진실
...
"재접근 가능성을 보면, 사주로는 관성은 남아 있는데 충이 걸려서 마음은 남아도 속도가 흔들리고,
점성술로도 금성-토성 긴장이 있으면 좋아도 쉽게 확답을 못 해.
둘 다 감정은 있지만 재접근은 천천히 풀린다고 말하니 밀어붙이기보다 조건 정리가 먼저야."
...
현재 연애운은 대운/세운과 트랜싯 행성을 함께 분석. 올해 어느 달이 좋은지, 도화살이나 홍염살 같은 신살도 언급하면서 금성-화성 배치도 함께. **둘 다 좋은 시기면 진짜 골든타임, 둘 다 나쁘면 진짜 조심**. 특히 썸 시작, 재접근, 관계 확정, 결혼/동거 논의를 같은 것으로 보지 말고 각각 구분해서 말해줘.
...
현재 상황별 조언은 최소한 솔로, 썸/애매한 관계, 연애중, 재회/재접근 고민, 결혼/장기 관계 고민을 분리해서 써줘. **둘이 공통으로 추천하는 행동 위주로**, 지금 질문자가 어느 구간에 가까운지 먼저 짚고 들어가.

감정이 남아 있는데 왜 관계가 안 풀리는지, 혹은 좋은데 왜 확정이 늦는지도 꼭 설명해. 여기서는 감정 강도뿐 아니라 속도 차이, 표현 차이, 생활 적합도, 현실 조건을 나눠서 말해야 해.
...
- **교차 분석 핵심**: 사주와 점성술이 같은 가족 역학, 경계선, 돌봄/경제 부담을 다른 방식으로 확인할 때 그게 진실
- 사주와 점성술을 50:50으로 융합 (번갈아가며)
- 모든 destiny-matrix 10개 레이어 데이터 자연스럽게 녹이기
- 문단 나눠서 읽기 편하게
- 14000~18000자 분량 (절대 짧게 쓰지 말 것 — 짧으면 다시 작성) (테마의 모든 측면을 심층 분석)
...
점성술로도 4하우스에 달이 강해서 집안 분위기가 감정 안정에 큰 영향을 줘.
둘 다 가족 정서의 영향력이 크다고 말하니, 독립 문제도 단순 거리보다 심리적 분리가 먼저야."

"형제 관계는, 사주로 비겁이 많아서 경쟁이나 비교 의식이 있을 수 있는데,
점성술로도 3하우스에 화성이 있어서 형제랑 다툼이 잦아.
둘 다 형제 갈등을 말하니 의식적으로 협력 관계 만드는 게 중요해."

"가족 안에서 돈과 돌봄 역할을 보면, 사주에서 재성과 인성이 같이 강하면 책임을 대신 떠안기 쉽고,
점성술로도 4하우스/6하우스가 연결되면 집안 문제를 실무적으로 다 처리하는 역할이 돼.
둘 다 가족 내 실무·감정 부담이 몰릴 수 있다고 말하니 경계선 설계가 핵심이야."

## 필수 내용 (자연스럽게 풀어서)

가족 DNA를 분석해볼게. 사주로 보면 인성과 비겁이 어떻게 배치됐는지, 점성술로는 4하우스와 10하우스가 어떤지 함께 봐야 해. **이 둘이 만나서 어떤 가족 역할, 책임 배분, 정서적 위치가 교차 확인되는지 명확히 설명**.

부모와의 관계도 사주의 인성 작용과 점성술의 4하우스/10하우스를 연결해서. 단순히 "부모와 가깝다"가 아니라, **기대/죄책감/책임감/독립 갈등 중 무엇이 핵심인지**까지 구체적으로 말해줘.

형제자매 관계는 사주의 비겁과 점성술의 3하우스를 함께 분석. **비교, 경쟁, 거리감, 역할 고정** 중 어떤 패턴이 반복되는지 분명히 설명해.

현재 가족운은 대운/세운과 트랜싯 행성을 함께 분석. 올해 언제 가족과 화합하고 언제 갈등 조심해야 하는지, 사주의 충극과 점성술의 4하우스 트랜싯을 융합해서. **둘 다 경고하는 시기는 진짜 말조심 + 결정 보류 포인트까지** 제시해.

소통 방법은 사주의 일간 특성과 점성술의 수성/달 배치를 보고. "사주로 목이 강하면 직설적인데, 점성술로도 수성 양자리면 말이 빨라. **둘 다 빠른 소통을 말하니 가족한테도 천천히 설명하는 연습 필요**"처럼. 특히 **누구에게는 설명이 필요하고, 누구에게는 거리 조절이 필요한지** 구분해.

가족 유산과 패턴은 사주의 격국과 점성술의 IC를 함께 봐서. **두 시스템이 공통으로 말하는 세대 패턴, 경제적 부담, 돌봄 부담, 감정 노동의 반복**을 강조해.

가족 내 돈/돌봄/실무 분담도 꼭 넣어. 누가 책임을 과하게 떠맡기 쉬운지, 어디서 억울함이 쌓이는지, 무엇을 문장으로 합의해야 하는지까지 말해줘.
