...
import type { DestinyCoreQuality } from '@/lib/matrix/core/runDestinyCore'
...
function withObjectParticleFallback(text: string, fallback: string): string {
  return withObjectParticle(text) || fallback
}
  ...
  const localized =
    lang === 'ko'
      ? value
          ...
          .replace(/\bwealth volatility pattern\b/gi, '재정 변동 흐름')
      : value
...
export function renderLifeMissionSection(
  ...
  const leadScenarios = normalizeNarrativeLabels(
    reportCore.topScenarioIds.slice(0, 3).map((id) => deps.formatScenarioIdForNarrative(id, lang)),
    lang
  )
    ...
    const body = deps.formatNarrativeParagraphs(
      deps.sanitizeUserFacingNarrative(
        [
          ...
          `지금 타이밍의 힘은 속도를 올리는 데 있지 않고, ${withObjectParticleFallback(metaphor.edge, '기준을')} 언제 꺼내 쓸지 아는 데 있습니다.`,
    ...
    deps.sanitizeUserFacingNarrative(
      lang === 'ko'
        ? [
            ...
            `지금 차이를 만드는 건 ${withObjectParticleFallback(metaphor.edge, '기준을')} 어떤 순서로 쓰느냐입니다.`,
            reportCore.riskControl,
          ].join(' ')
        : [
