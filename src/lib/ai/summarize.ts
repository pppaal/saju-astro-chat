...
function extractKeyInsights(assistantMessages: string[], locale: string): string[] {
  ...
  const patterns =
    locale === 'ko'
      ? [/중요한 것은[^.]+\./g, /핵심은[^.]+\./g, /추천드리는 것은[^.]+\./g, /조언드리자면[^.]+\./g]
      : [
          /the key is[^.]+\./gi,
...
export function buildLongTermMemory(
  ...
  for (const session of sessions) {
    ...
    if (session.keyTopics) {
      allTopics.push(...session.keyTopics)
    }
    if (session.summary) {
...
export function longTermMemoryToPrompt(memory: LongTermMemory, locale: string = 'ko'): string {
  ...
  if (memory.userProfile) {
    parts.push(
      locale === 'ko' ? `[프로필] ${memory.userProfile}` : `[Profile] ${memory.userProfile}`
    )
  }
  ...
  if (memory.journeyNarrative) {
    parts.push(
      locale === 'ko'
        ? `[상담 여정] ${memory.journeyNarrative}`
        : `[Journey] ${memory.journeyNarrative}`
    )
  }
