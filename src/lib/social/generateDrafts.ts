...
function buildPrompt(
  ...
  if (locale === 'ko') {
    return {
      systemPrompt: [
        ...
        '- 쓰레드 톤(중요): 친한 친구가 다정하게 챙겨주는 말투(부드러운 해요체, 가벼운 구어체 허용).',
        '  · "이 글이 보였다면 ..." 류의 부드러운 저격 훅으로 시작해 스크롤을 멈추게 하라.',
        '  · 1~2문장마다 줄바꿈해 리듬을 만들어라(문단 덩어리 금지).',
        '  · 마지막에 가벼운 참여 유도 1개를 반드시 넣어라(예: "당신의 생일 월을 댓글로 남겨보세요, 몇 분께는 답글 드릴게요").',
  ...
  return {
    systemPrompt: [
      ...
      '- Threads tone (important): lowercase-casual, like a caring friend checking in.',
      ...
      '- 5-8 relevant hashtags (not spammy). Always include #koreanastrology plus 1-2 zodiac/astrology tags for discovery.',
