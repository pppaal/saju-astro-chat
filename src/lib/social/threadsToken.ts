// src/lib/social/threadsToken.ts
//
// Threads 장기 토큰 자동 갱신 — Meta 장기 토큰은 60일 만료라 그대로 두면
// 두 달마다 발행이 조용히 죽는다. 주간 크론이 refresh_access_token 을 호출해
// 새 토큰을 받고, AES-256-GCM 으로 암호화해 Redis 에 저장한다. 발행/조회수
// 코드는 Redis 토큰을 우선 사용하고 없으면 env 로 폴백한다(부트스트랩).
//
...
const STORE_KEY = 'social:threads:token'
...
export async function getThreadsAccessToken(): Promise<string | null> {
  ...
  if (stored?.enc) {
    ...
    logger.warn('[social/threadsToken] stored token decrypt failed — falling back to env')
  }
  return envToken()
}

/** 저장된 갱신 상태(만료 예정일 표시용). 갱신본이 없으면 null. */
export async function getThreadsTokenStatus(): Promise<{
  refreshedAt: string
  expiresAt: string
} | null> {
  const stored = await cacheGet<StoredToken>(STORE_KEY)
...
> {
  const current = await getThreadsAccessToken()
  if (!current) return { ok: false, error: 'not_configured' }

  try {
    const res = await fetch(
      `${GRAPH.replace('/v1.0', '')}/refresh_access_token?grant_type=th_refresh_token&access_token=${encodeURIComponent(current)}`,
      { cache: 'no-store' }
    )
    ...
    if (!res.ok || !body?.access_token) {
      ...
      logger.error('[social/threadsToken] refresh failed', { error: msg })
    ...
    logger.info('[social/threadsToken] refreshed', { expiresAt })
    return { ok: true, expiresAt }
  } catch (error) {
    logger.error('[social/threadsToken] refresh error', { error })
