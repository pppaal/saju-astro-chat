...
// Monthly prompt (사주+점성 교차 전용, 섹션 고정)
export function buildMonthPrompt(lang: string, data: CombinedResult, _useStructured = true) {
  ...
  return [
    ...
    `Locale: ${lang}`,
    "[Task] 사주+점성 교차 근거로 월간 리딩 작성. 섹션 누락 금지, 각 조언에 근거 1줄.",
    "[Sections - 모두 포함]",
    "- 월간 한줄테마: 2~3줄",
    "- 핵심 주/날짜: 좋은 시기 2개, 주의 시기 2개 (근거: 대운/연운/월운/트랜짓)",
    ...
    "근거 소스: 사주(월운/대운/십신/신살) + 점성(월 트랜짓, 행성/하우스) 교차만 사용.",
    tone,
    info,
    `Respond in ${lang}.`,
  ].join("\n");
