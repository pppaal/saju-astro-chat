...
async function getSinsal(pillars: SajuPillars, enableDebugLogs = false): Promise<unknown> {
  try {
...
async function calculateAdvancedSajuAnalysis(
  ...
  try {
    ...
    const dayMasterElement = (dm?.element ||
      pillars.day.heavenlyStem?.element ||
      pillars.day.earthlyBranch?.element ||
      '') as DayMaster['element']
    const dayMasterYinYang = (dm?.yin_yang ||
      dm?.yinYang ||
      getYinYangFromName(dayMasterName) ||
      '') as DayMaster['yin_yang']

    // Pillars for analysis - with heavenlyStem/earthlyBranch structure
    const pillarsForAnalysis = {
      yearPillar: {
        heavenlyStem: {
          name: pillars.year.heavenlyStem?.name || '',
          element: (pillars.year.heavenlyStem?.element || '목') as '목' | '화' | '토' | '금' | '수',
        },
        earthlyBranch: {
          name: pillars.year.earthlyBranch?.name || '',
          element: (pillars.year.earthlyBranch?.element || '목') as
            | '목'
            | '화'
            | '토'
            | '금'
      ...
      monthPillar: {
        heavenlyStem: {
          name: pillars.month.heavenlyStem?.name || '',
          element: (pillars.month.heavenlyStem?.element || '목') as
            | '목'
            | '화'
            | '토'
            | '금'
        ...
        earthlyBranch: {
          name: pillars.month.earthlyBranch?.name || '',
          element: (pillars.month.earthlyBranch?.element || '목') as
            | '목'
            | '화'
            | '토'
            | '금'
      ...
      dayPillar: {
        heavenlyStem: {
          ...
          element: (pillars.day.heavenlyStem?.element || '목') as '목' | '화' | '토' | '금' | '수',
        },
        earthlyBranch: {
          name: pillars.day.earthlyBranch?.name || '',
          element: (pillars.day.earthlyBranch?.element || '목') as '목' | '화' | '토' | '금' | '수',
        },
      },
      timePillar: {
        heavenlyStem: {
          name: pillars.time.heavenlyStem?.name || '',
          element: (pillars.time.heavenlyStem?.element || '목') as '목' | '화' | '토' | '금' | '수',
        },
        earthlyBranch: {
          name: pillars.time.earthlyBranch?.name || '',
          element: (pillars.time.earthlyBranch?.element || '목') as
            | '목'
            | '화'
            | '토'
            | '금'
    ...
    try {
      ...
      if (enableDebugLogs) {
        logger.debug('[Extended analysis]:', {
          strength: extended.strength.level,
          geokguk: extended.geokguk.type,
        })
      }
    } catch (e) {
      if (enableDebugLogs) {
        logger.debug('[Extended analysis skipped]', e)
      }
    }
    ...
    try {
      ...
      if (enableDebugLogs) {
        logger.debug('[Geokguk skipped]', e)
      }
    }
    ...
    try {
      ...
      if (enableDebugLogs) {
        logger.debug('[Yongsin skipped]', e)
      }
    }
    ...
    try {
      ...
      if (enableDebugLogs) {
        logger.debug('[Tonggeun/Tuechul/Hoeguk skipped]', e)
      }
    }
    ...
    try {
      ...
      if (enableDebugLogs) {
        logger.debug('[Hyeongchung skipped]', e)
      }
    }
    ...
    try {
      ...
      if (enableDebugLogs) {
        logger.debug('[Sibsin analysis skipped]', e)
      }
    }
    ...
    try {
      ...
      if (enableDebugLogs) {
        logger.debug('[Health/Career analysis skipped]', e)
      }
    }
    ...
    try {
      const pillarsForScore = {
        year: {
          heavenlyStem: {
            name: pillars.year.heavenlyStem?.name || '',
            element: (pillars.year.heavenlyStem?.element || '목') as
              | '목'
              | '화'
              | '토'
              | '금'
              | '수',
            yin_yang: getYinYangFromName(pillars.year.heavenlyStem?.name || ''),
          },
          earthlyBranch: {
            name: pillars.year.earthlyBranch?.name || '',
            element: (pillars.year.earthlyBranch?.element || '목') as
              ...
              | '금'
              | '수',
            yin_yang: getYinYangFromName(pillars.year.earthlyBranch?.name || ''),
          },
        ...
        month: {
          heavenlyStem: {
            ...
            element: (pillars.month.heavenlyStem?.element || '목') as
              ...
              | '금'
              | '수',
            yin_yang: getYinYangFromName(pillars.month.heavenlyStem?.name || ''),
          },
          earthlyBranch: {
            name: pillars.month.earthlyBranch?.name || '',
            element: (pillars.month.earthlyBranch?.element || '목') as
              ...
              | '금'
              | '수',
            yin_yang: getYinYangFromName(pillars.month.earthlyBranch?.name || ''),
          },
        ...
        day: {
          heavenlyStem: {
            name: pillars.day.heavenlyStem?.name || '',
            element: (pillars.day.heavenlyStem?.element || '목') as
              | '목'
              | '화'
              | '토'
              | '금'
              | '수',
            yin_yang: getYinYangFromName(pillars.day.heavenlyStem?.name || ''),
          },
          earthlyBranch: {
            name: pillars.day.earthlyBranch?.name || '',
            element: (pillars.day.earthlyBranch?.element || '목') as
              | '목'
              | '화'
              | '토'
              | '금'
              | '수',
            yin_yang: getYinYangFromName(pillars.day.earthlyBranch?.name || ''),
          },
        ...
        time: {
          heavenlyStem: {
            name: pillars.time.heavenlyStem?.name || '',
            element: (pillars.time.heavenlyStem?.element || '목') as
              | '목'
              | '화'
              | '토'
              | '금'
              | '수',
            yin_yang: getYinYangFromName(pillars.time.heavenlyStem?.name || ''),
          },
          earthlyBranch: {
            name: pillars.time.earthlyBranch?.name || '',
            element: (pillars.time.earthlyBranch?.element || '목') as
              ...
              | '금'
              | '수',
            yin_yang: getYinYangFromName(pillars.time.earthlyBranch?.name || ''),
          },
          jijanggan: (pillars.time as { jijanggan?: Record<string, unknown> }).jijanggan || {},
        },
      }
      advancedAnalysis.score = calculateComprehensiveScore(
        pillarsForScore as unknown as Parameters<typeof calculateComprehensiveScore>[0]
      )
    } catch (e) {
      if (enableDebugLogs) {
        logger.debug('[Score calculation skipped]', e)
      }
    }
    ...
    try {
      ...
      advancedAnalysis.ultraAdvanced = performUltraAdvancedAnalysis(
        pillarsForUltra as unknown as Parameters<typeof performUltraAdvancedAnalysis>[0]
      )
    } catch (e) {
      if (enableDebugLogs) {
        logger.debug('[Ultra Advanced analysis skipped]', e)
      }
    }
...
export async function calculateSajuOrchestrated(
  ...
  try {
    sajuFacts = (await calculateSajuData(
      birthDate.trim(),
      safeBirthTime,
      gender,
      'solar',
  ...
  if (hasValidPillars) {
    try {
      ...
      const dayMasterNameForCycles =
        (dayMaster as { name?: string })?.name || pillars.day?.heavenlyStem?.name || ''
      const dayMasterElementForCycles =
        (dayMaster as { element?: string })?.element || pillars.day?.heavenlyStem?.element || '?'
      const dayMasterYinYangForCycles =
        (dayMaster as { yinYang?: string; yin_yang?: string })?.yinYang ||
        (dayMaster as { yin_yang?: string })?.yin_yang ||
        getYinYangFromName(dayMasterNameForCycles)
