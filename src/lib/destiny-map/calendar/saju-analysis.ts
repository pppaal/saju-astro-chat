...
export function calculateIljinScore(
  ...
  const dateHash =
    (targetDate.getFullYear() * 10000 + (targetDate.getMonth() + 1) * 100 + targetDate.getDate()) %
    7
...
// ============================================================
// 용신(用神) — 특정 날짜에 사용자의 용신이 얼마나 작용하는지 점수화.
// (정통 용신 판단은 src/lib/saju/advancedAnalysis.ts::analyzeYongsin —
//  이름이 같았으나 책임이 달라 2026-05 분리.)
// ============================================================
export function scoreYongsinFitForDate(
  ...
  const secondaryYongsin = yongsin.secondary
    ? ELEMENT_KO_TO_EN[yongsin.secondary] || yongsin.secondary.toLowerCase()
    : undefined
...
// ============================================================
// 격국(格局) — 특정 날짜에 사용자의 격국이 얼마나 작용하는지 점수화.
// (정통 격국 판단은 src/lib/saju/advancedAnalysis.ts::analyzeGeokguk —
//  이름이 같았으나 책임이 달라 2026-05 분리.)
// ============================================================
export function scoreGeokgukFitForDate(
  ...
  if (!geokguk?.type || !pillars?.day?.stem) {
    return result
  }
