...
export interface SajuAnalysisResult {
  ...
  geokgukAnalysis: ReturnType<typeof scoreGeokgukFitForDate>
...
export function analyzeSaju(input: SajuAnalysisInput): SajuAnalysisResult {
  const { dayMasterElement, dayBranch, dayMasterStem, sajuProfile, ganzhi, year, month, date } =
    input
  ...
  const yongsinAnalysis = scoreYongsinFitForDate(sajuProfile.yongsin, ganzhiResult, date)

  // 격국(格局) 분석 - 사주 구조 유형
  const geokgukAnalysis = scoreGeokgukFitForDate(
    sajuProfile.geokguk,
    ganzhiResult,
    sajuProfile.pillars
  )
  ...
  // 일진(日辰) 분석 - 일별 운세
  const iljinAnalysis = calculateIljinScore(
    dayMasterElement,
    dayMasterStem,
    dayBranch,
    ganzhiResult
  ...
  const hasYeokma = sajuProfile.yearBranch
    ? isYeokmaDay(sajuProfile.yearBranch, ganzhi.branch)
    : false
  const hasDohwa = sajuProfile.yearBranch
    ? isDohwaDay(sajuProfile.yearBranch, ganzhi.branch)
    : false

  const yearGanzhi = getYearGanzhi(year)
  const isSamjaeYearFlag = sajuProfile.yearBranch
    ? isSamjaeYear(sajuProfile.yearBranch, yearGanzhi.branch)
    : false
