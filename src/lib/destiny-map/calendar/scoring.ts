...
import { DAEUN_CONFIG, SEUN_CONFIG, WOLUN_CONFIG, ILJIN_CONFIG } from './scoring-factory-config'
...
function calculateYongsinScore(input: SajuScoreInput['yongsin']): number {
  ...
  // 긍정 요소 - config 비율 사용
  if (input.hasPrimaryMatch) {
    adjustments.push(YONGSIN_SCORES.positive.primaryMatch)
  }
  if (input.hasSecondaryMatch) {
    adjustments.push(YONGSIN_SCORES.positive.secondaryMatch)
  }
  if (input.hasBranchMatch) {
    adjustments.push(YONGSIN_SCORES.positive.branchMatch)
  }
  if (input.hasSupport) {
    adjustments.push(YONGSIN_SCORES.positive.support)
  }

  // 부정 요소
  if (input.hasKibsinMatch) {
    adjustments.push(YONGSIN_SCORES.negative.kibsinMatch)
  }
  if (input.hasKibsinBranch) {
    adjustments.push(YONGSIN_SCORES.negative.kibsinBranch)
  }
  if (input.hasHarm) {
    adjustments.push(YONGSIN_SCORES.negative.harm)
  }

  // 격국
  if (input.geokgukFavor) {
    adjustments.push(YONGSIN_SCORES.geokguk.favor)
  }
  if (input.geokgukAvoid) {
    adjustments.push(YONGSIN_SCORES.geokguk.avoid)
  }
  if (input.strengthBalance) {
    adjustments.push(YONGSIN_SCORES.geokguk.strengthBalance)
  }
  if (input.strengthImbalance) {
    adjustments.push(YONGSIN_SCORES.geokguk.strengthImbalance)
  }
...
function calculateTransitMoonScore(input: AstroScoreInput['transitMoon']): number {
  ...
  if (input.isVoidOfCourse) {
    adjustments.push(TRANSIT_MOON_SCORES.voidOfCourse)
  }
...
function calculateMajorPlanetsScore(input: AstroScoreInput['majorPlanets']): number {
  ...
  for (const planet of planets) {
    ...
    if (planetData.isRetrograde) {
      const retrograde =
        MAJOR_PLANETS_SCORES.retrograde[planet as keyof typeof MAJOR_PLANETS_SCORES.retrograde]
      if (retrograde) {
        adjustments.push(retrograde)
      }
    }
  }
...
function calculateSolarReturnScore(input: AstroScoreInput['solarReturn']): number {
  ...
  if (days === 0) {
    adjustments.push(SOLAR_RETURN_SCORES.exactBirthday)
  } else if (days <= 1) {
    adjustments.push(SOLAR_RETURN_SCORES.nearBirthday1)
  } else if (days <= 3) {
    adjustments.push(SOLAR_RETURN_SCORES.nearBirthday3)
  } else if (days <= 7) {
    adjustments.push(SOLAR_RETURN_SCORES.nearBirthday7)
  }

  // Progression - config 사용
  if (input.progressionSupport) {
    adjustments.push(SOLAR_RETURN_SCORES.progressionSupport)
  }
  if (input.progressionChallenge) {
    adjustments.push(SOLAR_RETURN_SCORES.progressionChallenge)
  }
...
export function calculateTotalScore(
  ...
  const baseAstroScore =
    transitSunScore + transitMoonScore + majorPlanetsScore + lunarPhaseScore + solarReturnScore
  // 추가 점수는 보정으로 적용 (최대 ±10점)
  const astroBonus = Math.max(
    -10,
    Math.min(10, outerPlanetsScore + specialPointsScore + eclipseScore)
  )
