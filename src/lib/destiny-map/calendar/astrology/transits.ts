...
// Route through the canonical `air → metal` mapping in calendar/utils.
// This file previously imported a duplicate normalizeElement from
// ./helpers that returned `air → fire` instead, so the same
// (zodiac sign → 5-element) coercion landed on a different mapping
// depending on which call site invoked it. The helpers.ts variant is
// gone now; everyone consumes the same one.
