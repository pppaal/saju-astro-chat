...
// ============================================================
// 해석 — Bound ruler가 부여하는 추가 dignity 결
// ============================================================

const BOUND_RULER_TONE: Record<AstroPlanetName, string> = {
  Sun:     '자아·권위 강화',
  Moon:    '정서·내면 강화',
  Mercury: '사고·소통 강화',
  Venus:   '미·조화 강화',
...
export function getBoundInterpretation(result: BoundResult, planetName?: string): string {
  const tone = BOUND_RULER_TONE[result.ruler] || ''
  const planetPart = planetName ? `${planetName} ` : ''
  return `${planetPart}in ${result.sign} ${result.degreeInSign.toFixed(1)}° (Egyptian bound ${result.range.start}-${result.range.end}°, ruler ${result.ruler}) — ${tone} layer 추가.`
}
