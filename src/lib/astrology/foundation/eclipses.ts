 ...
 */
export function getEclipsesBetween(startDate: string | Date, endDate: string | Date): Eclipse[] {
 ...
 */
export function getUpcomingEclipses(fromDate: Date = new Date(), count: number = 4): Eclipse[] {
 ...
 */
export function getEclipsesInSign(sign: ZodiacKo, startDate?: Date, endDate?: Date): Eclipse[] {
 ...
 */
export function getLifetimeEclipses(birthDate: Date, horizonYearsAhead: number = 100): Eclipse[] {
  const now = new Date()
    ...
    for (const point of allPoints) {
      ...
      if (diff <= orb) {
        impacts.push({
          ...
          orb: diff,
          house: eclipseHouse,
          interpretation: getEclipseInterpretation(
            eclipse,
            point.name,
            'conjunction',
            eclipseHouse
