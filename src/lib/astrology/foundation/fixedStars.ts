...
const FIXED_STARS: FixedStar[] = [
  // ========== 왕의 별 4대 (Royal Stars) ==========
  {
    ...
    interpretation:
      '가장 왕족적인 별. 성공과 명예를 가져오지만, 복수심이 있으면 추락할 수 있습니다.',
  },
  {
    name: 'Aldebaran',
    name_ko: '알데바란 (봄의 눈)',
  ...
  // ========== 쌍둥이자리/오리온자리 ==========
  {
    name: 'Castor',
    name_ko: '카스토르 (쌍둥이 형)',
    // A3 fix: 옛 코드 80.17° (20° Gemini 10') 는 의도 (주석 "20 Cancer 14") 와
    // 사인 1개 어긋남 → Cancer 행성과의 conjunction 검출 못함. 정정: Cancer
    // 시작 120° + 20°14' = 140.233°.
    longitude: 140.233, // 20 Cancer 14
    magnitude: 1.58,
    nature: 'Mercury',
  ...
  },
  {
    name: 'Pollux',
    name_ko: '폴룩스 (쌍둥이 동생)',
    // A3 fix: 옛 코드 83.25° (23° Gemini 15') 는 의도 (주석 "23 Cancer 13") 와
    // 사인 1개 어긋남. 정정: Cancer 시작 120° + 23°13' = 143.217°.
    longitude: 143.217, // 23 Cancer 13
    magnitude: 1.14,
    nature: 'Mars',
    constellation: 'Gemini',
...
export function getFixedStar(name: string): FixedStar | undefined {
  return FIXED_STARS.find((s) => s.name.toLowerCase() === name.toLowerCase())
}
...
export function getBrightestStars(count: number = 5): FixedStar[] {
  return [...FIXED_STARS].sort((a, b) => a.magnitude - b.magnitude).slice(0, count)
}
