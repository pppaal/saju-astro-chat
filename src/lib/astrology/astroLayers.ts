...
export async function getAstroLayersForDate(input: AstroLayersInput): Promise<AstroLayersBundle> {
  ...
  // ── Asteroids / Midpoints / Harmonics → lifetime layer highlights 으로 변환 ──
  // raw 만 수집하고 끝나면 score 에 영향 0. 여기서 의미있는 hits 를
  // SajuTimingAnalysis 스타일 highlights 로 변환해 lifetime layer 에 합침.
  try {
    ...
    if (extraHighlights.length > 0) {
      // lifetime 이 있으면 추가, 없으면 새로 생성
      if (bundle.lifetime) {
        bundle.lifetime = {
          ...bundle.lifetime,
          highlights: [...bundle.lifetime.highlights, ...extraHighlights],
        }
      } else {
        bundle.lifetime = {
          unit: 'lifetime',
          periodLabel: 'Asteroids · Midpoints · Harmonics (natal)',
          highlights: extraHighlights,
          summary: `${extraHighlights.length}개 추가 자극`,
        }
      }
    }
  } catch { /* skip */ }
