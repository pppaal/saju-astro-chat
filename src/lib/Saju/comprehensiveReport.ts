...
function analyzeInteractions(pillars: SajuInput): ComprehensiveReportSection {
  ...
  const interpretation =
    score.balance === '매우 길함'
      ? '사주에 매우 좋은 길한 작용이 많습니다.'
      : score.balance === '길함'
        ? '사주에 길한 작용이 우세합니다.'
        : score.balance === '중립'
          ? '사주에 길흉이 균형을 이루고 있습니다.'
          : score.balance === '흉함'
            ? '사주에 흉한 작용이 있으니 주의가 필요합니다.'
            : '사주에 매우 흉한 작용이 많으니 각별한 주의가 필요합니다.'
...
export function generateComprehensiveReport(
  ...
  const unluckyElements: FiveElement[] = elementAnalysis.dominant ? [elementAnalysis.dominant] : []

  // 요약문 생성
  const summary =
    `${getKoreanPillar(pillars.day)} 일주, 일간 ${dayMaster}(${getStemElement(dayMaster)}). ` +
...
export function generateQuickSummary(pillars: SajuInput): string {
  ...
  summary += `\n**오행 분포**: ${Object.entries(elementCounts)
    .map(([k, v]) => `${k}${v}`)
    .join(' ')}\n`
