...
function countSibsinCategories(pillars: SajuPillars, dayElement: FiveElement): Record<string, number> {
...
function generateJonggeokDescription(type: JonggeokType, element: FiveElement, purity: number): string {
  if (type === '비종격') {
...
function generateJonggeokAdvice(type: JonggeokType, follow: FiveElement, avoid: FiveElement): string {
  if (type === '비종격') {
...
function checkSeasonSupport(monthBranch: string, targetElement: FiveElement): boolean {
  ...
  return branchElement === targetElement ||
...
/** 60갑자 일주 특성 */
const ILJU_CHARACTERISTICS: Record<string, {
  character: string;
  strengths: string[];
  weaknesses: string[];
  career: string[];
...
export function analyzeIljuDeep(pillars: SajuPillars): IljuDeepAnalysis {
  ...
  // 공망
  const gongmang = calculateGongmang(pillars.year.heavenlyStem.name, pillars.year.earthlyBranch.name);
...
function calculateNaeum(stem: string, branch: string): string {
  ...
  const naeumMap: Record<string, string> = {
    '甲子': '海中金', '甲寅': '大溪水', '甲辰': '覆燈火',
    '甲午': '沙中金', '甲申': '泉中水', '甲戌': '山頭火',
    '乙丑': '海中金', '乙卯': '大溪水', '乙巳': '覆燈火',
    // ... 나머지
  };
...
function getLuckyFactors(element: FiveElement): { direction: string; color: string; number: number[] } {
...
export function analyzeGongmangDeep(pillars: SajuPillars): GongmangDeepAnalysis {
  ...
  const effects = {
    ...
    negative: affectedPillars.length > 0
      ? ['해당 영역의 불안정', '채워지지 않는 느낌']
      : [],
  };
