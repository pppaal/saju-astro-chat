...
export type FamilyRole = 'father' | 'mother' | 'self' | 'spouse' | 'child' | 'sibling' | 'grandparent';
...
// 가족 역할별 오행 에너지
const ROLE_ELEMENT_ENERGY: Record<FamilyRole, { primary: string; secondary: string }> = {
 ...
 * 천간 관계 분석
 */
export function analyzeStemRelation(
  saju1: SajuResult,
  saju2: SajuResult
): StemRelationResult {
  const stems1 = [
 ...
 * 역할 조화 분석
 */
export function analyzeRoleHarmony(
  member: FamilyMember,
  saju: SajuResult
): RoleHarmonyResult {
...
export function analyzeSiblings(
  ...
  // 출생 순서 영향
  const birthOrderInfluence = siblings.length > 1
...
export function analyzeSpouse(
  ...
  if (elementHarmony.relation === '상극') {
    frictionAreas.push(`오행 상극(${elementHarmony.dominant1}-${elementHarmony.dominant2})으로 인한 관점 차이`);
  }
  ...
  for (let i = 0; i < 5; i++) {
    ...
    let theme: string;
    if (YUKAP[yearBranch] === self.fourPillars.day.branch ||
