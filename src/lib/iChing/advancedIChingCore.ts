...
function getHaekEnergy(
  lower: Trigram | undefined,
  upper: Trigram | undefined,
  locale: Locale
): string {
  if (!lower || !upper) {
    return ta(locale, 'haekEnergyAnalyzing')
  }
...
export function analyzeTrigramInteraction(binary: string, locale: Locale = 'ko'): TrigramAnalysis {
  ...
  return {
    ...
    interaction: ta(locale, 'trigramInteraction', {
      lowerNature: 하괘.nature,
      lowerKo: 하괘.korean,
      upperNature: 상괘.nature,
      upperKo: 상괘.korean,
    }),
...
export function analyzeYaoPositions(binary: string, locale: Locale = 'ko'): YaoPositionAnalysis[] {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const isCorrect = (isYang && positionNumber % 2 === 1) || (!isYang && positionNumber % 2 === 0)
...
export function analyzeChangingLines(
  ...
  return {
    ...
    toHexagram,
    changingLines,
    interpretation: getChangingInterpretation(
      changingLines,
      fromHexagram.number,
      toHexagram.number,
      locale
    ),
    transitionAdvice: getTransitionAdvice(
      changingLines,
      fromHexagram.number,
      toHexagram.number,
      locale
...
function getChangingInterpretation(
  ...
  if (lineCount === 4) {
    const unchangedLines = [1, 2, 3, 4, 5, 6]
      .filter((n) => !lines.includes(n))
      .sort((a, b) => a - b)
    const lowerUnchanged = unchangedLines[0]
    return ta(locale, 'changingQuadruple', {
      unchanged: unchangedLines.join(', '),
      lower: lowerUnchanged,
    })
  }
  if (lineCount === 5) {
...
function getTransitionAdvice(
  ...
  if (lineCount === 1) {
    return ta(locale, 'transitionAdvice1')
  }
  if (lineCount === 2) {
    return ta(locale, 'transitionAdvice2')
  }
  if (lineCount === 3) {
    return ta(locale, 'transitionAdvice3')
  }
  if (lineCount === 4) {
    return ta(locale, 'transitionAdvice4')
  }
  if (lineCount === 5) {
    return ta(locale, 'transitionAdvice5')
  }
  if (lineCount === 6) {
    if (fromNum === 1 && toNum === 2) {
      return ta(locale, 'transitionAdvice6YongJiu')
    }
    if (fromNum === 2 && toNum === 1) {
      return ta(locale, 'transitionAdvice6YongYuk')
    }
...
function getKeyMoment(
  lines: number[],
  fromHex: HexagramBasic,
  toHex: HexagramBasic,
  locale: Locale
): string {
  const lineCount = lines.length
  ...
  return ta(locale, 'keyMomentPartial', {
    count: lineCount,
    from: fromHex.korean || '',
    to: toHex.korean || '',
  })
 ...
 * 종합 괘 분석
 */
export function performComprehensiveHexagramAnalysis(
  binary: string,
  locale: Locale = 'ko'
): ComprehensiveHexagramAnalysis {
...
function generateOverallInsightFromData(
  trigrams: TrigramAnalysis,
  hoGwa: HoGwaAnalysis,
  locale: Locale
): string {
  return ta(locale, 'overallInsight', {
    lowerNature: trigrams.하괘.nature,
    lowerKo: trigrams.하괘.korean,
    upperNature: trigrams.상괘.nature,
    upperKo: trigrams.상괘.korean,
...
function generateActionAdviceFromData(
  yaoPositions: YaoPositionAnalysis[],
  locale: Locale
): string[] {
  const advice: string[] = []
 ...
 * 두 괘 비교 분석
 */
export function compareHexagrams(
  binary1: string,
  binary2: string,
  locale: Locale = 'ko'
): {
