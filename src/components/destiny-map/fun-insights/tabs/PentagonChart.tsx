...
const clampScore = (v: unknown): number => {
  if (typeof v === 'number') {
    return Math.max(SCORE_LIMITS.MIN, Math.min(SCORE_LIMITS.FULL, v))
  }
  return 0
}
...
const getScoreLevel = (score: number) => {
  if (score >= SCORE_LEVELS.EXCELLENT.threshold) {
    return SCORE_LEVELS.EXCELLENT
  }
  if (score >= SCORE_LEVELS.GOOD.threshold) {
    return SCORE_LEVELS.GOOD
  }
  if (score >= SCORE_LEVELS.AVERAGE.threshold) {
    return SCORE_LEVELS.AVERAGE
  }
  return SCORE_LEVELS.NEEDS_CARE
}

// 원소 시너지 점수 계산
const calculateElementSynergy = (
  el1: string | null,
  el2: string | null,
  sameScore: number,
  ...
  if (ELEMENT_SYNERGY_PAIRS.some(([a, b]) => a === el1 && b === el2)) {
...
const PentagonChart = memo(function PentagonChart({
  saju,
  astro,
  lang: _lang,
  isKo,
  ...
  const strengthTotal =
    clampScore(strength.total) || clampScore(strength.score) || DEFAULT_SCORES.STRENGTH
  ...
  if (elementScores.length > 0) {
    ...
    sajuBalance = capAt95(
      clampScore(sc.balance) || clampScore(sc.elementBalance) || DEFAULT_SCORES.BALANCE
    )
  }

  const sajuPotential = capAt95(
    clampScore(geokguk.purity) ||
      clampScore(sc.structure) ||
      clampScore(sc.geokguk) ||
      DEFAULT_SCORES.POTENTIAL
  )
  const sajuLuck = capAt95(
    clampScore(yongsinData.fitScore) ||
      clampScore(sc.yongsin) ||
      clampScore(sc.usefulGod) ||
      DEFAULT_SCORES.LUCK
  )
  const sajuStability = capAt95(
    clampScore(tonggeun.score) || clampScore(tonggeun.totalScore) || DEFAULT_SCORES.STABILITY
  )
  ...
  // ============ 통합 5축 점수 ============
  const innerPower = Math.min(
    100,
    Math.round(sajuEnergy * 0.65 + (sunSign ? 25 + dmSunSynergy * 1.0 : sajuEnergy * 0.35))
  )

  const harmony = Math.min(
    100,
    Math.round(sajuBalance * 0.55 + sunMoonHarmony * 1.2 + (dmSunSynergy + dmMoonSynergy) * 0.6)
  )

  const potential = Math.min(
    100,
    Math.round(sajuPotential * 0.65 + ascBonus * 1.2 + (ascSign ? 15 : sajuPotential * 0.25))
  )

  const totalSynergy = dmSunSynergy + dmMoonSynergy + sunMoonHarmony
  const fortune = Math.min(
    100,
    Math.round(sajuLuck * 0.55 + Math.min(30, totalSynergy * 1.0) + (sunSign && moonSign ? 15 : 8))
  )

  const stability = Math.min(
    100,
    Math.round(sajuStability * 0.65 + dmMoonSynergy * 1.2 + (moonSign ? 18 : sajuStability * 0.25))
  )
  ...
  const totalScore = Math.round(
    innerPower * 0.25 + harmony * 0.2 + potential * 0.2 + fortune * 0.2 + stability * 0.15
  )
  ...
  const dataPoints = scores.map((s, i) =>
    getChartPoint(i, (s.value / SCORE_LIMITS.FULL) * CHART.MAX_RADIUS)
  )
    ...
    <div className="rounded-2xl bg-gradient-to-br from-slate-900/80 to-yellow-900/20 border border-yellow-500/30 p-6">
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-lg font-bold text-yellow-300">
          {isKo ? '종합 운세 점수' : 'Overall Fortune Score'}
        </h3>
        <span
          className={`ml-auto px-2 py-1 rounded-full text-xs font-bold ${level.color} ${level.bg}`}
        >
            ...
            <div className="flex flex-wrap gap-2">
              {scores
                ...
                .map((s, i) => (
                  <span
                    key={i}
                    className="px-2 py-1 rounded-full text-xs bg-orange-500/20 text-orange-300"
                  >
                    {s.emoji} {s.label} {s.value}점
                  </span>
                ))}
