...
export function getCurrentTimingAnalysis(saju: any, astro: any, lang: string): {
  ...
  const yongsinActions: Record<string, { ko: string; en: string }> = {
    wood: { ko: "🌿 녹색 옷 입기, 식물 가까이, 산책하기", en: "🌿 Wear green, be near plants, take walks" },
    fire: { ko: "🔥 밝은 색 입기, 햇빛 쬐기, 사람 만나기", en: "🔥 Wear bright colors, get sunlight, socialize" },
    earth: { ko: "🏔️ 편안한 색 입기, 규칙적 식사, 자연 가까이", en: "🏔️ Wear earth tones, regular meals, be in nature" },
    metal: { ko: "⚔️ 정돈하기, 명확한 계획, 깔끔한 환경", en: "⚔️ Organize, clear plans, tidy environment" },
    water: { ko: "💧 휴식 취하기, 물 충분히, 조용한 시간", en: "💧 Rest well, stay hydrated, quiet time" }
  };
    ...
    message: isKo
      ? `${currentMonth}월은 ${monthEnergyDesc[monthElement].ko}의 달이에요.\n\n` +
        (monthYongsinMatch
          ...
          : monthElement === "wood" ? "새로운 프로젝트를 시작하기 좋아요." :
            monthElement === "fire" ? "적극적으로 행동하고 표현하세요." :
            monthElement === "earth" ? "쌓아온 것들을 점검하고 정리하세요." :
            monthElement === "metal" ? "중요한 결정을 내리기 좋은 시기예요." :
            "무리하지 말고 에너지를 충전하세요.")
      : `${currentMonth} is the month of ${monthEnergyDesc[monthElement].en}.\n\n` +
        (monthYongsinMatch
          ...
          : monthElement === "wood" ? "Good time to start new projects." :
            monthElement === "fire" ? "Act and express yourself actively." :
            monthElement === "earth" ? "Review and organize what you've built." :
            monthElement === "metal" ? "Good time for important decisions." :
    ...
    message: isKo
      ? `${age}세, ${currentStageInfo.ko}입니다.\n\n` +
        (yongsinAdvice
          ...
          : daeunProgress < 3 ? "지금은 준비하고 배우는 시기예요. 조급해하지 마세요." :
            daeunProgress < 7 ? "가장 활발하게 활동하는 시기예요. 적극적으로 도전하세요." :
      ...
      : `Age ${age}, ${currentStageInfo.en}.\n\n` +
        (yongsinAdvice
          ...
          : daeunProgress < 3 ? "Time to prepare and learn. Don't rush." :
            daeunProgress < 7 ? "Most active period. Challenge yourself." :
