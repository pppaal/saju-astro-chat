...
export function getCurrentFlowAnalysis(
  saju: SajuData | undefined,
  lang: string
): { title: string; flow: string; advice: string; emoji: string } | null {
  const isKo = lang === 'ko'
  ...
  const daeunSibsin =
    typeof daeunSibsinRaw === 'object' && daeunSibsinRaw?.cheon
      ? daeunSibsinRaw.cheon
      : typeof daeunSibsinRaw === 'string'
        ? daeunSibsinRaw
        : ''
  const yearSibsinRaw = thisYear?.sibsin
  const yearSibsin =
    typeof yearSibsinRaw === 'object' && yearSibsinRaw?.cheon
      ? yearSibsinRaw.cheon
      : typeof yearSibsinRaw === 'string'
        ? yearSibsinRaw
        : ''
