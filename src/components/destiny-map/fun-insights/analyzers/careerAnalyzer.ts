...
const HOUSE9_PATTERNS: Record<ZodiacSign, { ko: string; en: string }> = {
  aries: {
    ko: '모험적으로 해외를 개척해요. 새로운 시장을 열어요.',
    en: 'Adventurously pioneer overseas. Open new markets.',
  },
  taurus: {
    ko: '안정적인 해외 투자나 부동산이 유리해요.',
    en: 'Stable overseas investments or real estate favored.',
  },
  gemini: {
    ko: '해외와의 소통, 무역, 교육 분야가 어울려요.',
    en: 'Communication, trade, education with foreign countries suit you.',
  },
  cancer: {
    ko: '해외에서 가정을 꾸리거나 이민 가능성이 있어요.',
    en: 'Possibility of making home abroad or immigration.',
  },
  leo: {
    ko: '해외에서 주목받는 위치에 오를 수 있어요.',
    en: 'Can rise to prominent positions overseas.',
  },
  virgo: {
    ko: '해외 기술 연수나 전문성 활용이 좋아요.',
    en: 'Foreign technical training or utilizing expertise good.',
  },
  libra: {
    ko: '해외 파트너십이나 외교적 역할이 어울려요.',
    en: 'Foreign partnerships or diplomatic roles suit you.',
  },
  scorpio: {
    ko: '해외에서 깊이 있는 연구나 변혁적 일을 해요.',
    en: 'Deep research or transformative work overseas.',
  },
  sagittarius: {
    ko: '타고난 해외운! 글로벌하게 활동하기 좋아요.',
    en: 'Natural overseas fortune! Great for global activities.',
  },
  capricorn: {
    ko: '해외에서 장기적 커리어를 쌓을 수 있어요.',
    en: 'Can build long-term career overseas.',
  },
  aquarius: {
    ko: '해외 IT, 혁신 분야에서 두각을 나타내요.',
    en: 'Excel in foreign IT, innovation fields.',
  },
  pisces: {
    ko: '해외 예술, 영적 분야, 봉사에서 기회가 와요.',
    en: 'Opportunities in foreign arts, spirituality, service.',
  },
}
...
function getCurrentDaeunElement(saju: SajuData | undefined): string | null {
  ...
  const birthYear = saju?.facts?.birthDate
    ...
    : saju?.birthDate
      ? new Date(saju.birthDate).getFullYear()
      : null
...
function getOverseasFortune(
  ...
  const luckyList =
    (sinsal as { luckyList?: Array<{ name?: string } | string> } | undefined)?.luckyList || []
  const unluckyList =
    (sinsal as { unluckyList?: Array<{ name?: string } | string> } | undefined)?.unluckyList || []
  ...
  if (hasYeokma) {
    result += isKo
      ? '역마살이 있어 해외 이동이 활발해요. '
      : 'You have travel energy (Yeokma) for active overseas movement. '
  }

  if (jupiterIn9) {
    result += isKo
      ? '목성이 9하우스에서 해외에서 큰 행운이 기다려요! '
      : 'Jupiter in 9th house brings great fortune overseas! '
  }
...
function getSuccessTiming(saju: SajuData | undefined, isKo: boolean): string | null {
  ...
  const yongsinElements = (Array.isArray(yongsinList) ? yongsinList : [])
    .map((y: { element?: string } | string) => (typeof y === 'string' ? y : y.element))
    .filter(Boolean)
