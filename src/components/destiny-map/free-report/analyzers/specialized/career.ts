...
export function getCareerAdvancedAnalysis(
    ...
    for (const [houseStr, planets] of Object.entries(housePlanets)) {
      ...
      if (HOUSE_CAREER_AREAS[house]) {
        ...
        const strength: 'strong' | 'moderate' | 'weak' =
          planetCount >= 3 ? 'strong' : planetCount >= 2 ? 'moderate' : 'weak'
        houseCareerMap.push({
          house,
          planets,
  ...
  if (currentDaeun?.element) {
    ...
    if (interaction) {
      careerTiming.push({
        ...
        icon: '🌟',
        strength:
          interaction.score >= 70 ? 'strong' : interaction.score >= 50 ? 'moderate' : 'weak',
        score: interaction.score,
        description: {
  ...
  const houseScore =
    houseCareerMap.length > 0
      ? houseCareerMap.reduce(
          (sum, h) => sum + (h.strength === 'strong' ? 80 : h.strength === 'moderate' ? 60 : 40),
          0
        ) / houseCareerMap.length
      : 0
