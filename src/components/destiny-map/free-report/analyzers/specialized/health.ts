...
export function getHealthMatrixAnalysis(
  ...
  const sibsinToElement: Record<string, FiveElement> = {
    비견: sajuEl,
    겁재: sajuEl,
    식신: getGeneratedElement(sajuEl),
    상관: getGeneratedElement(sajuEl),
    편재: getControlledElement(sajuEl),
    정재: getControlledElement(sajuEl),
    편관: getControllerElement(sajuEl),
    정관: getControllerElement(sajuEl),
    편인: getGeneratorElement(sajuEl),
    정인: getGeneratorElement(sajuEl),
  }
  ...
  for (const [sibsin, count] of Object.entries(elementDist)) {
    ...
    if (el) {
      elementCounts[el] += count as number
    }
  }
  ...
  for (const el of excessElements) {
    ...
    if (healthInfo) {
      vulnerableAreas.push({
        ...
        risk: 'medium',
        advice: isKo
          ? `${el.element} 기운이 과해요. 균형이 필요해요.`
          : `${el.element} energy is excessive. Balance needed.`,
        icon: '⚡',
      })
    }
  }
  ...
  const overallVitality = Math.max(
    0,
    Math.min(100, (balanceScore + vitalityScore) / 2 - healthRisk)
  )
