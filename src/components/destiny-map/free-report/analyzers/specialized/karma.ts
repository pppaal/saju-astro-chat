...
export function getKarmaMatrixAnalysis(
  ...
  if (astro?.planets && Array.isArray(astro.planets)) {
    const northNode = astro.planets.find(
      (p) => p.name?.toLowerCase() === 'north node' || p.name?.toLowerCase() === 'northnode'
    )
    const southNode = astro.planets.find(
      (p) => p.name?.toLowerCase() === 'south node' || p.name?.toLowerCase() === 'southnode'
    )
  ...
  const nodeScore = nodeAxis
    ? (nodeAxis.northNode.fusion.score + nodeAxis.southNode.fusion.score) / 2
    : 0
  const relationScore =
    karmicRelations.length > 0
      ? karmicRelations.reduce((sum, r) => sum + r.fusion.score, 0) / karmicRelations.length
      : 0
