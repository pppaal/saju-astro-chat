...
// ── helpers ────────────────────────────────────────────────────────────
const elClass: Record<string, string> = { wood: s.elWood, fire: s.elFire, earth: s.elEarth, metal: s.elMetal, water: s.elWater }
...
function Pentagon({ fe }: { fe: ReportData['saju']['fiveElements'] }) {
  ...
  const dataPts = order.map((k, i) => polar(cx, cy, R * (fe[k] / max), angleAt(i)).join(',')).join(' ')
  return (
    <svg viewBox="0 0 280 264" className={s.pentagon}>
      {grid.map((p, i) => <polygon key={i} points={p} className={s.pentGrid} />)}
      {order.map((_, i) => {
      ...
      {order.map((k, i) => {
        ...
        return (
          <text key={k} x={lx} y={ly} className={`${s.pentLabel} ${elClass[k]}`}>
            {ELEMENTS[k].han}
            <tspan className={s.pentCnt} dx="3">{fe[k]}</tspan>
          </text>
        )
      })}
    </svg>
...
function Wheel({ astro }: { astro: ReportData['astro'] }) {
  const SZ = 360, cx = SZ / 2, cy = SZ / 2
  const rOuter = 168, rSign = 150, rInner = 120, rPlanet = 100
  ...
  return (
    <svg viewBox={`0 0 ${SZ} ${SZ}`} className={s.pentagon} style={{ maxWidth: 360 }}>
        ...
        return (
          <g key={sg}>
            ...
            <text x={gx} y={gy} textAnchor="middle" dominantBaseline="central"
              className={elClass[meta.el]} style={{ fontSize: 17, fontFamily: 'var(--sym)' }}>
              {meta.glyph}
            </text>
          </g>
        )
      ...
      {/* ASC / MC 축 */}
      {[['ASC', ascLon], ['MC', astro.mc.lon]].map(([lab, lon]) => {
        ...
        return (
          <g key={lab as string}>
            ...
            <text x={x2} y={y2} textAnchor="middle" className={s.mono} style={{ fontSize: 10, fill: 'var(--ink-2)', fontWeight: 600 }}>{lab as string}</text>
          </g>
        )
      })}
      {/* 어스펙트 선 */}
      {astro.aspects.map((a, i) => {
        ...
        const stroke = cls === 'hard' ? 'var(--asp-hard)' : cls === 'soft' ? 'var(--asp-soft)' : 'var(--asp-neutral)'
      ...
      {planetsSorted.map((p) => {
        ...
        return (
          <g key={p.name}>
            <circle cx={px} cy={py} r={11} fill="var(--card)" stroke="var(--line)" strokeWidth={1} />
            <text x={px} y={py} textAnchor="middle" dominantBaseline="central" style={{ fontFamily: 'var(--sym)', fontSize: 14, fill: 'var(--ink)' }}>{p.glyph}</text>
          </g>
        )
      })}
    </svg>
        ...
        {present.map((row, ri) => (
          <tr key={row}>
            {present.map((col, ci) => {
              ...
              if (ci === ri) return <td key={col} className={s.agDiag}>{glyph(row)}</td>
...
export function IntegratedReport({ data, cross }: IntegratedReportProps) {
  ...
  const pillarsArr: Array<['시' | '일' | '월' | '년', ReportData['saju']['pillars'][keyof ReportData['saju']['pillars']]]> = [
    ['시', S.pillars.hour], ['일', S.pillars.day], ['월', S.pillars.month], ['년', S.pillars.year],
  ]
      ...
      <div className={s.wrap}>
        ...
        <header className={s.reportHead}>
          <div className={s.rhLeft}>
            ...
            <h1>통합 <span className={s.accent}>명식</span> 리포트</h1>
            <div className={s.sub}>사주 명식과 출생 천궁도를 하나의 평면에서 교차 분석</div>
          </div>
          <div className={s.rhRight}>
            <div>{input.name} · <b>{input.gender === 'male' ? '남' : '여'}</b></div>
            <div>{input.calendar} <b>{input.date}</b></div>
            <div>{input.place}</div>
          </div>
        </header>
        ...
        <div className={s.metaGrid}>
          {[
            ...
            <div className={s.metaCell} key={k}>
              <span className={s.metaK}>{k}</span><span className={s.metaV}>{v}</span>
            </div>
          ))}
        </div>
        ...
        <section className={s.section}>
          <div className={s.secHead}>
            ...
            <span className={s.secTitle}>사주 명식<span className={s.han}>四柱命式</span></span>
          ...
          <div className={s.pillars}>
            {pillarsArr.map(([head, p]) => (
              <div className={`${s.pillar} ${p.isDay ? s.isDay : ''}`} key={head}>
                ...
                <div className={s.jjg}>
                  {p.jijanggan.map((j, i) => (
                    <div className={s.jjgI} key={i}><b className={elClass[stemEl(j.g)]}>{j.g}</b><i>{j.d}</i></div>
                  ))}
                </div>
            ...
            <div>
              ...
              <div className={s.chips}>
                {S.natalShinsal.map((sh, i) => (
                  <span className={`${s.chip} ${sh.polarity > 0 ? s.pos : sh.polarity < 0 ? s.neg : s.neu}`} key={i}>
                    <b>{sh.ko}</b><i>{sh.pillar}{sh.sub ? `·${sh.sub}` : ''}</i>
                  </span>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className={s.relations}>
                {S.natalRelations.map((r, i) => (
                  <div className={`${s.rel} ${r.tone === 'pos' ? s.relPos : r.tone === 'neg' ? s.relNeg : ''}`} key={i}>
                    <b>{r.type}</b>{r.detail}
                  </div>
                ))}
              </div>
          ...
          <div className={s.subcap} style={{ marginTop: 24 }}>대운 · 大運</div>
          <div className={s.daeun}>
            {S.daeun.map((d) => (
              <div className={`${s.du} ${d.current ? s.duNow : ''}`} key={d.age}>
                <div className={s.duAge}>{d.age}</div>
                <div className={s.duGz}><b className={elClass[stemEl(d.stem)]}>{d.stem}</b><b className={elClass[branchEl(d.branch)]}>{d.branch}</b></div>
        ...
        <section className={s.section}>
          <div className={s.secHead}>
            ...
            <span className={s.secTitle}>오행과 용신<span className={s.han}>五行·用神</span></span>
            ...
            <div className={`${s.card} ${s.cardPad} ${s.elemSide}`}>
              ...
              <div>
                ...
                <div className={s.yongRow}>
                  ...
                  <span className={`${s.yong} ${s.yongPri} ${elClass[S.yongsin.primary]}`}>{ELEMENTS[S.yongsin.primary]?.han}<i>{ELEMENTS[S.yongsin.primary]?.ko}</i></span>
                  {S.yongsin.secondary && <span className={`${s.yong} ${s.yongSec} ${elClass[S.yongsin.secondary]}`}>{ELEMENTS[S.yongsin.secondary]?.han}<i>{ELEMENTS[S.yongsin.secondary]?.ko}</i></span>}
                </div>
                <div className={s.yongRow}>
                  <span className={s.yongLab}>기신</span>
                  {S.yongsin.avoid.map((a) => (
                    <span className={`${s.yong} ${s.yongAvo} ${elClass[a]}`} key={a}>{ELEMENTS[a]?.han}<i>{ELEMENTS[a]?.ko}</i></span>
                  ))}
                </div>
                <div className={s.gaugeScale} style={{ marginTop: 8 }}>
                  {(['wood', 'fire', 'earth', 'metal', 'water'] as const).map((k) => (
                    <i key={k} className={elClass[k]}>{ELEMENTS[k].ko} {S.fiveElements[k]}</i>
                  ))}
        ...
        <section className={s.section}>
          <div className={s.secHead}>
            ...
            <span className={s.secTitle}>출생 천궁도<span className={s.han}>本命 天宮圖</span></span>
            <span className={s.secEn}>Natal Chart</span>
          </div>
          <div className={s.gridChart}>
            <div className={`${s.card} ${s.wheelCard}`}><Wheel astro={A} /></div>
            <div>
              <div className={`${s.card} ${s.cardPad}`}>
                <div className={s.subcap}>행성 위치 · Planets</div>
                <table className={s.planetTable}><tbody>
                  {[...A.planets, ...A.extraPoints.map((e) => ({ ...e, retro: false, speed: 0 }))].map((p) => (
                    <tr key={p.name}>
                      <td className={s.plG}>{p.glyph}</td>
                      <td className={s.plN}>{p.ko}<i>{p.name}</i></td>
                      <td className={`${s.plS} ${elClass[SIGN_META[abbr(p.sign)]?.el]}`}>{SIGN_META[abbr(p.sign)]?.glyph} {SIGN_META[abbr(p.sign)]?.ko}</td>
              ...
              <div className={s.axes}>
                {[['ASC', A.ascendant], ['MC', A.mc]].map(([lab, ax]) => {
                  const a = ax as { sign: string; deg: string }
                  return (
                    <div className={s.axisItem} key={lab as string}>
                      <span>{lab as string}</span><b className={elClass[SIGN_META[abbr(a.sign)]?.el]}>{SIGN_META[abbr(a.sign)]?.glyph} {a.deg}</b>
                    </div>
                  )
                })}
                <div className={s.axisItem}><span>Sect</span><b>{A.sect === 'day' ? '주간 (晝)' : '야간 (夜)'}</b></div>
                <div className={s.axisItem}><span>House</span><b>{A.houseSystem}</b></div>
              </div>
              <div className={s.daeun} style={{ gridTemplateColumns: 'repeat(6,1fr)' }}>
                {A.houses.slice(0, 6).map((h) => (
                  <div className={s.du} key={h.i}><div className={s.duAge}>{h.i}H</div><div className={`${s.gzHan ?? ''}`} style={{ fontSize: 14, fontFamily: 'var(--sym)' }}><span className={elClass[SIGN_META[h.sign]?.el]}>{SIGN_META[h.sign]?.glyph}</span></div></div>
                ))}
              </div>
            </div>
          </div>
        ...
        <section className={s.section}>
          <div className={s.secHead}>
            ...
            <span className={s.secTitle}>어스펙트<span className={s.han}>行星 角度</span></span>
        ...
        {cross && cross.rows.length > 0 && (
          <section className={s.section}>
            <div className={s.secHead}>
              ...
              <span className={s.secTitle}>통합 교차<span className={s.han}>交叉 統合</span></span>
                  ...
                  <div className={s.theme} key={i} style={{ ['--tc' as string]: t.color }}>
                    ...
                    {(r.left || r.right) && (
                      <div className={s.themeCross}>
                        <div className={s.themeSide}><div className={s.themeSideK}>사주</div><div className={s.themeSideV}>{r.left}</div></div>
                        <div className={s.themeSide}><div className={s.themeSideK}>점성</div><div className={s.themeSideV}>{r.right}</div></div>
                      </div>
                    )}
