 ...
 */
export function toMonth(opts: ToMonthOptions): { month: DestinypalMonth; calendar: DestinypalCalendarDay[] } {
  const th = {
  ...
  for (const cell of opts.cells) {
    ...
    if (score < th.avoid) { mark = 'avoid'; avoidDays.push(ds) }
    else if (score < th.caution) { mark = 'caution'; cautionDays.push(ds) }
    else if (score >= th.good) { mark = 'good'; goodDays.push(ds) }
    if (score > bestScore) { bestScore = score; bestDay = d; bestDs = ds }
    ...
    for (const k of Object.keys(themeAccum) as DestinypalMonthThemeBar['key'][]) {
      ...
      if (typeof v === 'number') { themeAccum[k].sum += v; themeAccum[k].n += 1 }
    }
  ...
  if (opts.converge?.date) {
    ...
    if (c) { c.mark = 'converge'; c.intensity = Math.max(c.intensity, 0.9) }
  }

  // themes 평균 → 0..100
  const themes: DestinypalMonthThemeBar[] = (Object.keys(themeAccum) as DestinypalMonthThemeBar['key'][])
    .map((k) => ({
      key: k,
      ko: THEME_KO[k],
  ...
  const woolun = opts.woolunStem && opts.woolunBranch ? toGanji(opts.woolunStem, opts.woolunBranch) : undefined
