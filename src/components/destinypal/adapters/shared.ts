...
export function rootLine(
  ...
  if (typeof tonggeunStrength === 'number') {
    const tg =
      tonggeunStrength >= 100 ? '통근 단단' :
      tonggeunStrength >= 50 ? '통근 뚜렷' :
      tonggeunStrength > 0 ? '통근 얇음' :
      '뿌리 없음'
    parts.push(tg)
  }
...
export function isoToYmd(iso: string): string { return iso.slice(0, 10) }
