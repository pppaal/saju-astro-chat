...
const SIBSIN_THEME_KO: Record<string, { theme: string; themeEn: string; headline: string }> = {
  비견: { theme: '주체·동료의 무대', themeEn: 'Self · Peer', headline: '비견 대운 — 같은 결의 사람들 속에서 내 자리를 짓는 10년.' },
  겁재: { theme: '경쟁·분탈의 무대', themeEn: 'Rivalry · Split', headline: '겁재 대운 — 경쟁과 협업이 동시에 치솟는 10년.' },
  식신: { theme: '표현·창작의 무대', themeEn: 'Creativity · Output', headline: '식신 대운 — 끼와 표현이 자기 결을 만드는 10년.' },
  상관: { theme: '자유·재능의 무대', themeEn: 'Talent · Free Expression', headline: '상관 대운 — 재능이 자유롭게 뻗어가는 10년.' },
  편재: { theme: '현실 성취 · 재물의 무대', themeEn: 'Wealth · Worldly Achievement', headline: '편재 대운 — 손에 잡히는 결실의 10년.' },
  정재: { theme: '꾸준한 축적의 무대', themeEn: 'Steady Accumulation', headline: '정재 대운 — 차근차근 자산이 쌓이는 10년.' },
  편관: { theme: '추진·도전의 무대', themeEn: 'Drive · Challenge', headline: '편관 대운 — 강하게 밀어붙이는 10년.' },
  정관: { theme: '책임·자리의 무대', themeEn: 'Duty · Position', headline: '정관 대운 — 사회적 자리를 다지는 10년.' },
  편인: { theme: '독자적 사유의 무대', themeEn: 'Independent Mind', headline: '편인 대운 — 독학과 깊은 사고가 빛나는 10년.' },
  정인: { theme: '배움·지원의 무대', themeEn: 'Learning · Support', headline: '정인 대운 — 배움과 지원이 든든한 10년.' },
}
 ...
 */
export function toDecade(
  natal: NatalContext,
  opts: ToDecadeOptions = {},
): DestinypalDecade | null {
  ...
  // 현재 대운
  const current =
    daeunList.find((d) => currentAge >= d.startAge && currentAge < d.startAge + 10) ??
    daeunList[0]
...
function safeStemElement(stem: string): string {
  try { return getStemElement(stem) ?? '' } catch { return '' }
}
function safeBranchElement(branch: string): string {
  try { return getBranchElement(branch) ?? '' } catch { return '' }
}
