        ...
        <div className={styles.instructionContent}>
          {gameState === 'picking' && selectedIndices.length === 0 && (
            <p className={styles.guidanceText}>
              ✨ {isKo ? '마음이 이끄는 대로 카드를 선택하세요' : 'Let your intuition guide you to the cards'}
            </p>
          )}
          {gameState === 'revealing' && (
            <>
      ...
      {showTooltip && gameState === 'picking' && (
        <div className={styles.tooltipOverlay} onClick={handleDismissTooltip}>
          <div className={styles.tooltipCard} onClick={(e) => e.stopPropagation()}>
            <button className={styles.tooltipClose} onClick={handleDismissTooltip} aria-label="Close">×</button>
            ...
            <div className={styles.tooltipSteps}>
              <div className={styles.tooltipStep}>
                ...
                <p>{isKo ? '질문에 집중하며 마음을 가라앉히세요' : 'Focus on your question and calm your mind'}</p>
              </div>
              <div className={styles.tooltipStep}>
                <span className={styles.tooltipStepNumber}>2</span>
                <p>{isKo ? `직관적으로 끌리는 카드 ${cardCount}장을 선택하세요` : `Intuitively select ${cardCount} cards that call to you`}</p>
              </div>
              <div className={styles.tooltipStep}>
                <span className={styles.tooltipStepNumber}>3</span>
                <p>{isKo ? '첫 느낌을 믿고 너무 고민하지 마세요' : 'Trust your first instinct, don\'t overthink'}</p>
              </div>
            </div>
        ...
        {Array.from({ length: 78 }).map((_, index) => {
          ...
          return (
            <button
              ...
              aria-label={isKo ? `카드 ${index + 1}${isSelected ? `, 선택됨 (${displayNumber}번째)` : ''}` : `Card ${index + 1}${isSelected ? `, selected (${displayNumber})` : ''}`}
