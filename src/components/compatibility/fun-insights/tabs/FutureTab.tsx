...
export default function FutureTab({ data, isKo }: TabProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <InsightCard emoji="📅" title={isKo ? "관계 타임라인" : "Relationship Timeline"} colorTheme="indigo">
        <div className="space-y-4">
          {futurePhases.map((phase, idx) => (
            <div key={idx} className={`relative p-4 rounded-xl bg-${phase.color}-500/10 border border-${phase.color}-500/20`}>
              {/* Timeline connector */}
              ...
              <div className="flex items-start gap-4">
                <div className={`w-10 h-10 rounded-full bg-${phase.color}-500/20 border border-${phase.color}-500/30 flex items-center justify-center text-xl`}>
                  {phase.emoji}
                </div>
                <div className="flex-1">
                  ...
                  <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full bg-${phase.color}-500/20 text-${phase.color}-300 text-xs`}>
                    💡 {phase.advice}
                  </div>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <InsightCard emoji="⚖️" title={isKo ? "관계 균형" : "Relationship Balance"} colorTheme="cyan">
          <div className="space-y-4">
            <div>
              ...
              <div className="w-full h-2 bg-slate-700/50 rounded-full overflow-hidden">
                <div className="h-full bg-emerald-500" style={{ width: `${Math.min(strengthCount * 15, 100)}%` }} />
              </div>
            </div>
            <div>
              <div className="flex justify-between text-sm mb-1">
              ...
              <div className="w-full h-2 bg-slate-700/50 rounded-full overflow-hidden">
                <div className="h-full bg-orange-500" style={{ width: `${Math.min(potentialChallenges * 20, 100)}%` }} />
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 p-3 rounded-xl bg-cyan-500/10 border border-cyan-500/20">
            <p className="text-gray-300 text-sm">
              {strengthCount > potentialChallenges
                ? (isKo ? '강점이 많아 긍정적인 관계 발전이 기대됩니다.' : 'More strengths suggest positive relationship growth.')
                : (isKo ? '도전 과제가 있지만 함께 극복하며 성장할 수 있습니다.' : 'Challenges exist but can be overcome together for growth.')}
            </p>
          </div>
        </InsightCard>

        <InsightCard emoji="🎯" title={isKo ? "관계 목표" : "Relationship Goals"} colorTheme="amber">
          <div className="space-y-3">
            ...
            <InsightContent colorTheme="amber">
              <div className="flex items-start gap-3">
                ...
                <p className="text-gray-200 text-sm">
                  {isKo ? '정기적인 소통과 감정 공유 시간 갖기' : 'Regular communication and emotional sharing time'}
                </p>
              </div>
            </InsightContent>
            <InsightContent colorTheme="amber">
              <div className="flex items-start gap-3">
                ...
                <p className="text-gray-200 text-sm">
                  {isKo ? '함께 성장할 수 있는 공동 프로젝트 만들기' : 'Create shared projects for mutual growth'}
                </p>
              </div>
            </InsightContent>
          </div>
        ...
        <div className="relative">
          ...
          <div className="flex items-center justify-between mb-6">
            {[
              ...
              { label: isKo ? '발전' : 'Growth', icon: '🌿', score: Math.min(overallScore + 10, 100) },
              { label: isKo ? '성숙' : 'Mature', icon: '🌳', score: Math.min(overallScore + 15, 100) },
              { label: isKo ? '완성' : 'Complete', icon: '🌲', score: Math.min(overallScore + 20, 100) },
            ].map((stage, idx) => (
              <div key={idx} className="text-center flex-1">
                ...
                <p className={`text-sm font-bold ${idx === 0 ? 'text-purple-300' : 'text-gray-500'}`}>
                  {stage.score}%
                </p>
              </div>
            ))}
      ...
      <InsightCard emoji="💡" title={isKo ? "미래를 위한 조언" : "Advice for the Future"} colorTheme="emerald">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {sajuAnalysis?.detailedInsights?.slice(0, 4).map((insight, idx) => (
            ...
            <>
              <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                <p className="text-gray-200 text-sm">
                  {isKo ? '서로의 장점을 인정하고 칭찬하세요' : 'Recognize and praise each other\'s strengths'}
                </p>
              </div>
              <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                <p className="text-gray-200 text-sm">
                  {isKo ? '갈등이 생기면 빠르게 대화로 해결하세요' : 'Resolve conflicts quickly through dialogue'}
                </p>
              </div>
              <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                <p className="text-gray-200 text-sm">
