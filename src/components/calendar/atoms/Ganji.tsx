...
import type { Ganji as GanjiData } from '@/types/calendar'
...
export function Ganji({ data, size = 30, en = true, accent = 'ember', className }: GanjiProps) {
  if (!data) return null
  ...
  return (
    <span className={[styles.ganji, className].filter(Boolean).join(' ')}>
      ...
      <span className={styles.kr} style={{ fontSize: Math.max(10, size * 0.32), marginTop: 5 }}>
        {data.kr}
      </span>
      {en && (
        <span className={styles.en} style={{ fontSize: Math.max(9, size * 0.28), marginTop: 2 }}>
          {data.en}
        </span>
      )}
    </span>
