...
/**
 * ShareDayButton — 오늘의 운세(1일 티어)를 SNS(인스타/카톡)용 1:1 이미지로
 * 만들어 공유/저장한다. 클릭 → 화면 밖 DayShareCard 를 잠깐 마운트 →
 * html-to-image 로 PNG 캡처 → 미리보기 모달에서 [공유하기](Web Share API) /
 * [이미지 저장].
 *
 * 서버/스토리지 없이 클라이언트에서 끝나므로 게스트도 사용 가능.
 * (타로 ShareTarotButton 의 캘린더 일(日)판 — 카드 이미지 프리로드 없이 로고만.)
 */
...
import { DayShareCard, SHARE_CARD_SIZE, type DayShareData } from './DayShareCard'

interface ShareDayButtonProps {
  /** 공유 카드에 그릴 데이터 — DayTier 에서 로케일 반영해 미리 만든다. */
  data: DayShareData
}
...
export function ShareDayButton({ data: shareData }: ShareDayButtonProps) {
  const isKo = shareData.isKo
  ...
  const capture = useCallback(async () => {
    ...
    try {
      ...
      logger.error('[ShareDay] capture failed', err instanceof Error ? err : undefined)
      setError(
        isKo
  ...
  const filename = `destinypal-day-${shareData.dateLabel.replace(/[^0-9a-zA-Z]+/g, '-')}.png`
  ...
  const handleNativeShare = async () => {
    ...
    const payload: ShareData = {
      files: [file],
      title: isKo ? 'DestinyPal 오늘의 운세' : 'My DestinyPal day',
      text: shareData.oneLine,
    }
    try {
      ...
      logger.error('[ShareDay] native share failed', err instanceof Error ? err : undefined)
    }
    // 공유 미지원/실패 → 저장으로 폴백.
    handleDownload()
  }
  ...
  return (
    <>
      ...
      {phase !== 'idle' && (
        ...
        >
          <DayShareCard ref={cardRef} data={shareData} />
        </div>
      )}
