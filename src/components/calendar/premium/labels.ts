...
const KO: LabelSet = {
  ...
  crossSignalSajuTitle: '사주가 자주 보낸 흐름',
  crossSignalAstroTitle: '점성이 자주 보낸 흐름',
  crossSignalFrequency: (n) => `— 이번 기간 ${n}일 등장`,
  crossFlowSummaryAligned: (start, end) => `${start} ~ ${end} 두 흐름이 함께 차오릅니다`,
  crossFlowSummaryOpposed: (start, end) => `${start} ~ ${end} 두 흐름이 엇갈립니다`,
  crossFlowSummaryPeak: (date) => `${date} 두 흐름이 동시에 차오르는 자리`,
  crossSpotAlignedHeader: '두 흐름이 동시에 차오름',
