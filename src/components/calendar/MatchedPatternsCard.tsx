...
export default function MatchedPatternsCard({ patterns }: Props) {
  ...
  return (
    <div className="bg-zinc-900/60 p-5 rounded-2xl border border-amber-500/30 shadow-lg shadow-amber-500/5">
      ...
      <div className="space-y-3">
        {patterns.map((p) => (
          <div
            key={p.id}
            className="bg-zinc-950/60 rounded-xl p-4 border border-amber-500/10"
          >
            {/* 헤드라인 (강조) */}
            {p.headline && (
              <div className="text-base font-bold text-amber-200 mb-2 flex items-center gap-1.5 leading-snug">
            ...
            {/* 근거 (작게, 명리 용어) */}
            {p.description && (
              <div className="text-xs text-zinc-500 flex items-center gap-1 leading-snug">
