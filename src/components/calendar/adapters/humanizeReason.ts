 ...
 *   - 음역 각 → 관계 평어 (어포지션→팽팽히 맞섬, 스퀘어→밀고 당기는 마찰)
...
const JOHU_RE =
  /^([子丑寅卯辰巳午未申酉戌亥])月\s*조후\s*[—\-]\s*([목화토금수])[이가]\s+\S+\s+균형에 필요$/
...
// '긴장각/대립각' 류 각(角) 조어도 여전히 용어투라(감사 갭 #4) 관계 묘사로 교체 —
// "수성 ↔ 타고난 화성 · 밀고 당기는 마찰" 처럼 꼬리 토큰 슬롯에서 자연스럽게 읽힌다.
