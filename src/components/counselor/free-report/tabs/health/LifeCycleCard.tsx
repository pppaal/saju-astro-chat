...
export default function LifeCycleCard({ lifeCycleStage, isKo }: LifeCycleCardProps) {
  const combinedNarrative = isKo
    ? `${lifeCycleStage.description.ko} ${lifeCycleStage.advice}`
    : `${lifeCycleStage.description.en} ${lifeCycleStage.advice}`
    ...
    <div className="rounded-2xl bg-gradient-to-br from-slate-900/80 to-cyan-900/20 border border-cyan-500/30 p-6">
      ...
      <div className="p-4 rounded-xl bg-cyan-500/10 border border-cyan-500/20">
        <div className="flex items-center justify-between mb-3">
          <span className="text-2xl font-bold text-cyan-400">{lifeCycleStage.stage}</span>
          <div className="flex items-center gap-1">
            {[...Array(10)].map((_, i) => (
              <div
                key={i}
                className={`w-2 h-4 rounded-sm ${i < lifeCycleStage.vitalityLevel ? 'bg-cyan-400' : 'bg-slate-600'}`}
              />
            ))}
          </div>
        </div>
        ...
        <p className="text-gray-300 text-sm leading-relaxed">
          {ensureMinSentenceText(combinedNarrative, isKo, 'health')}
        </p>
      </div>
    </div>
  )
