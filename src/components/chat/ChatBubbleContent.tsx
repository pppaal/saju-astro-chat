...
const ChatBubbleContent = React.memo(function ChatBubbleContent({
  ...
  // 점성/사주 기호 자동 치환은 assistant 답변에만 — 사용자가 직접 □ 등을
  // 입력한 경우는 보존 (디버그/테스트용). 답변 언어를 텍스트에서 감지해
  // 한국어/영어에 맞는 평어로 바꾼다 (theme 과 무관).
  const normalizedContent = isAssistant ? naturalizeAstroSymbols(stripped) : stripped
