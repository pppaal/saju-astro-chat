...
interface IljinData { year: number; month: number; day: number; heavenlyStem: string; earthlyBranch: string; sibsin: { cheon: string; ji: string; }; isCheoneulGwiin: boolean; }
...
export default function SajuAnalyzer() {
    ...
    return (
        <div style={{ width: '100%', maxWidth: '600px', margin: '0 auto' }}>
            <form onSubmit={handleSubmit} style={{ background: '#1e1e2f', padding: '2rem', borderRadius: '12px', border: '1px solid #4f4f7a', marginBottom: '2rem' }}>
                ...
                <div style={{ marginBottom: '2rem' }}>
                    <label htmlFor="cityName" style={labelStyle}>태어난 도시 (영문)</label>
                    <input id="cityName" name="cityName" type="text" value={formData.cityName} onChange={handleInputChange} placeholder="예: Seoul, London, Busan" style={inputStyle} />
                </div>
            ...
            {sajuResult && (
                <SajuResultDisplay result={sajuResult} />
            )}
        </div>
    );
}

const labelStyle: React.CSSProperties = { display: 'block', fontWeight: '500', marginBottom: '0.5rem', color: '#e0e0e0' };
const inputStyle: React.CSSProperties = { width: '100%', padding: '0.8rem', border: '1px solid #4f4f7a', borderRadius: '6px', fontSize: '1rem', backgroundColor: '#161625', color: '#ffffff' };
const selectStyle: React.CSSProperties = { ...inputStyle, appearance: 'none' };
const buttonStyle = (disabled: boolean): React.CSSProperties => ({ 
    width: '100%', padding: '1rem', cursor: disabled ? 'not-allowed' : 'pointer', 
    background: disabled ? '#2a2a3e' : '#3a6df0', 
    color: 'white', border: 'none', borderRadius: '6px', fontSize: '1.1rem', 
