...
export function IljinCalendar({ iljinData, year, month }: IljinCalendarProps) {
  const makeKstDateUTC = (y: number, m0: number, d: number) =>
    new Date(Date.UTC(y, m0, d, 15, 0, 0, 0));
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const sibsinCheon = iljin?.sibsin?.cheon
      ? typeof iljin.sibsin.cheon === 'string'
        ? iljin.sibsin.cheon
        : String(iljin.sibsin.cheon)
      : '';
    const sibsinJi = iljin?.sibsin?.ji
      ? typeof iljin.sibsin.ji === 'string'
        ? iljin.sibsin.ji
        : String(iljin.sibsin.ji)
      : '';
    ...
    calendarDays.push(
      ...
      >
        ...
        {!iljin && (
          <div style={{ position: 'absolute', bottom: 6, right: 8, fontSize: 11, color: '#666' }}>
            일진 없음
          </div>
        )}
        {iljin?.isCheoneulGwiin && (
          <span style={{ position: 'absolute', top: 5, right: 5, fontSize: '0.8rem' }}>⭐</span>
        )}
      </div>
    );
  }
  ...
  return (
    <div style={{ background: '#161625', padding: '1rem', borderRadius: 12 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)' }}>
        {headers.map((d, i) => (
          <div key={d} style={{ ...calendarHeaderStyle, color: i === 0 ? '#ff6b6b' : '#c0c0c0' }}>
            {d}
          </div>
        ))}
        {calendarDays}
