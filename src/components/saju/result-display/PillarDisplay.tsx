...
export function PillarBox({ title, heavenlyStem, earthlyBranch }: PillarBoxProps) {
  ...
  return (
    <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <div style={{ ...pillarCellStyle, backgroundColor: stemEl ? ELEMENT_COLORS[stemEl] : '#4a80e2' }}>
        {String(stemName)}
      </div>
      <div style={{ height: 8 }} />
      <div style={{ ...pillarCellStyle, backgroundColor: branchEl ? ELEMENT_COLORS[branchEl] : '#f3a73f' }}>
        {String(branchName)}
      </div>
