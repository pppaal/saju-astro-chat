...
export default function SajuResultDisplay({ result }: Props) {
  // 모든 Hooks를 컴포넌트 최상단으로 이동시킵니다.
  // 초기 상태는 result가 없을 경우를 대비해 null 또는 빈 배열로 설정합니다.
  ...
  // useEffect를 사용하여 result prop이 유효할 때 초기 상태를 설정합니다.
  useEffect(() => {
    if (result && result.daeun && result.daeun.cycles && result.daeun.cycles.length > 0) {
      ...
      setDisplayedIljin(result.iljin || []);
    }
  }, [result]); // result가 변경될 때 이 효과를 다시 실행합니다.

  // selectedDaeun이 설정된 이후에 연운을 계산하고 설정합니다.
  useEffect(() => {
    if (!selectedDaeun || !result) return;
  ...
  }, [selectedDaeun, result]);

  // selectedYeonun이 설정된 이후에 월운을 계산하고 설정합니다.
  useEffect(() => {
    if (!selectedYeonun || !result) return;
  ...
  }, [selectedYeonun, result]);

  // selectedWolun이 설정된 이후에 일진을 계산하고 설정합니다.
  useEffect(() => {
    if (!selectedWolun || !result) return;
    const newIljin = getIljinCalendar(selectedWolun.year, selectedWolun.month, result.dayMaster);
    setDisplayedIljin(newIljin);
  }, [selectedWolun, result]);

  // 렌더링 직전에 데이터 유효성을 검사합니다. Hooks가 모두 호출된 이후이므로 규칙에 위배되지 않습니다.
  if (!result || !result.daeun || !result.daeun.cycles || !selectedDaeun) {
    return <div style={{ color: 'white', textAlign: 'center', marginTop: '2rem' }}>결과를 표시할 수 없습니다.</div>;
  }

  // 이제 result가 유효하므로, 변수를 안전하게 구조분해합니다.
