...
function buildChatPrompt(lang: string, theme: string, snapshot: string, history: ChatMessage[]) {
  ...
  return [
    ...
    '## Direct Answer',
    '- One-line direct answer to the latest user question.',
    '## Evidence',
    '- Up to 3 concise evidence bullets grounded in snapshot factors.',
    '## Action Plan',
    '- 2-3 concrete and reversible actions for this week.',
    '## Avoid / Recheck',
    '- 2 caution points + 2 recheck checkpoints.',
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { computeDestinyMap } = await import('@/lib/counselor/astrology')
