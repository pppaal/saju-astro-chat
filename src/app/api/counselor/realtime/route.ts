...
export async function POST(req: NextRequest) {
  ...
  const ctxKey = `counselor:ctx:v3:${userId}:${birthFingerprint(body)}:${hourUnknown ? 'tU' : 'tK'}:${utcDateKey(new Date())}`
  let contextText: string | null = await cacheGet<string>(ctxKey)
  if (!contextText) {
    try {
      const { saju, astro, report, birthTimeUnknown } = await runFortuneWithRaw({
        birth: {
      ...
      const snapshot = serializeBirthSnapshot(saju, astro, { birthTimeUnknown })
