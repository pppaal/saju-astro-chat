  ...
  try {
    ...
    const compatibilityPromises = filteredProfiles.slice(0, limit * 2).map(async (profile) => {
      ...
      if (myProfile.personalityScores && profile.personalityScores) {
        try {
          const myScores = myProfile.personalityScores as { energy: number; cognition: number; decision: number; rhythm: number };
          const theirScores = profile.personalityScores as { energy: number; cognition: number; decision: number; rhythm: number };
