...
export const POST = withApiMiddleware(
  async (req: NextRequest, context: ApiContext) => {
    try {
      ...
      logger.error('[destiny-match/block] POST error:', { error })
      return NextResponse.json({ error: 'Failed to block user' }, { status: HTTP_STATUS.SERVER_ERROR })
    }
  },
...
export const DELETE = withApiMiddleware(
  async (req: NextRequest, context: ApiContext) => {
    try {
      ...
      logger.error('[destiny-match/block] DELETE error:', { error })
      return NextResponse.json({ error: 'Failed to unblock user' }, { status: HTTP_STATUS.SERVER_ERROR })
    }
  },
  createAuthenticatedGuard({
    route: '/api/destiny-match/block',
