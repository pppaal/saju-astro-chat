...
export const POST = withApiMiddleware(
  async (req: NextRequest, context: ApiContext) => {
    ...
    // Validate with Zod
    const validationResult = destinyMatchSwipeSchema.safeParse(rawBody)
    if (!validationResult.success) {
      logger.warn('[Destiny match swipe] validation failed', {
    ...
    const { targetProfileId, action, compatibilityScore } = validationResult.data
    ...
    // Super Like 일일 리셋 체크 (트랜잭션으로 원자성 보장)
    let updatedMyProfile = myProfile
    const today = new Date()
    today.setHours(0, 0, 0, 0)

    const needsReset = !myProfile.superLikeResetAt || new Date(myProfile.superLikeResetAt) < today

    if (needsReset) {
      const refreshed = await prisma.$transaction(async (tx) => {
        const resetResult = await tx.matchProfile.updateMany({
          where: {
            userId,
        ...
        if (resetResult.count > 0) {
          return tx.matchProfile.findUnique({
            where: { userId },
            include: {
              user: {
                select: {
...
export const DELETE = withApiMiddleware(
  async (req: NextRequest, context: ApiContext) => {
    ...
    // Validate with Zod
    const undoValidation = destinyMatchSwipeUndoSchema.safeParse(rawBody)
    if (!undoValidation.success) {
      return NextResponse.json(
        {
          error: 'validation_failed',
          details: undoValidation.error.issues.map((e) => ({
            path: e.path.join('.'),
            message: e.message,
          })),
        },
    ...
    const { swipeId } = undoValidation.data
