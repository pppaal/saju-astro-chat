...
import { analyzePatterns, getPatternStatistics } from '@/lib/saju/patternMatcher'
...
export function generateTier3Analysis(input: Tier3AnalysisInput): Tier3AnalysisResult {
  ...
  const parts: string[] = [
    ...
    lang === 'ko'
      ? '[🌙 고급 점성술 분석 - 진행법/택일/역행]'
      : '[🌙 ADVANCED ASTROLOGY - Progressions/Electional/Retrograde]',
    '═══════════════════════════════════════════════════════════════',
    '',
  ]
  ...
  try {
    ...
    if (astro && astro.planets) {
      ...
      if (sun && moon) {
        ...
        parts.push(
          lang === 'ko' ? `🌙 달 위상: ${moonPhaseName}` : `🌙 Moon Phase: ${moonPhaseName}`
        )
      ...
      const lilith = planets.find((p) => p.name === 'Lilith' || p.name === 'Black Moon Lilith') as
        | PlanetBase
        | undefined
    ...
    if (saju?.pillars) {
      const patternAnalysis = analyzePatterns(
        saju.pillars as unknown as import('@/lib/saju/types').SajuPillars
      )
