...
async function emailMap(userIds: string[]): Promise<Map<string, { email: string | null; name: string | null }>> {
...
export const GET = withApiMiddleware(
  async (req: NextRequest, context: ApiContext) => {
    try {
      ...
      if (!(await isAdminUser(context.userId))) {
        logger.warn('[admin/anomalies] unauthorized', { userId: context.userId })
      ...
      const ids = [...new Set([...topConsumersRaw.map((c) => c.userId), ...topGrantedRaw.map((g) => g.userId)])]
  ...
  createAuthenticatedGuard({ route: '/api/admin/anomalies', limit: 30, windowSeconds: 60 })
)
