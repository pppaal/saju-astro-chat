...
/**
 * 끊긴 타로 해석 복원 — 모바일에서 다른 앱 갔다 와 스트림이 끊겼을 때, 클라가
 * 이 엔드포인트를 turnId 로 폴링한다. 서버는 연결이 끊겨도 끝까지 생성해
 * 완성 리딩(raw JSON 문자열)을 캐시에 넣어두므로(interpret-stream route 의
 * persistIfRecoverable), 여기서 그 완성본을 돌려준다. 아직 생성 중이면 ready=false.
 *
 * 보안: 로그인 사용자만 조회 가능. 자기 userId 의 캐시 키로만 lookup
 * 하므로 turnId 를 알아도 다른 사용자 결과 못 본다. 게스트는 끊김 복구
 * 미지원 (interpret-stream route 가 게스트 결과는 캐시에 저장 안 함).
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const content = await cacheGet<string>(tarotTurnResultKey(userId, turnId))
