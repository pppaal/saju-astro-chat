...
export const DELETE = withApiMiddleware(
  async (req: NextRequest, context: ApiContext) => {
    ...
    try {
      ...
      // 개인정보성 로그(공유 링크)는 함께 삭제한다. 이 둘은 userId 를 FK 관계
      // 없이 들고 있어 user.delete() 의 cascade 대상이 아니다. 반면
      // BonusCreditPurchase·SecurityAuditLog 는 회계/보안 목적으로 보존한다
      // (userId 만 남는 고아 행). 나머지 연관 테이블은 모두 onDelete: Cascade
      // 로 user.delete() 한 번에 연쇄 삭제된다.
