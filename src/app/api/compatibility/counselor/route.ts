    ...
    const systemPrompt =
      counselorLang === 'ko'
        ? [
            ...
            '',
            '답변 분량 가이드 (열린 질문일 때): 본문 2~3 단락, 단락당 2~4 문장. 첫 단락은 *끌림/연결 메커니즘* (왜 서로에게 향하는지 사주·점성 근거로), 두 번째는 *부딪힘/긴장 지점* (어디서 결이 어긋나는지), 마지막은 *풀어가는 길/타이밍* (실용 조언 + 현재 대운/세운 cross 가 있으면 시기 단서). 짧은 사실 질문엔 단락 1개로 끝내도 좋음.',
            '',
            '규칙:',
            ...
            '- Composite 블록 안의 줄은 [C] 로 시작 — 이건 *관계 entity 자체* 의 내부 aspect 이지 A·B synastry cross 가 아니다. 절대 "A 의 달이 B 의 수성과" 같이 synastry 처럼 인용 X. 대신 "두 사람이 함께 만드는 분위기에서 달-수성 결" 식으로 표현.',
        ...
        : [
            ...
            '- Lines inside the Composite block start with [C] — these are *internal aspects of the relationship entity*, NOT A→B synastry crosses. Never cite as "A\'s Moon to B\'s Mercury" — instead frame as "in the atmosphere they create together, the Moon-Mercury grain…".',
