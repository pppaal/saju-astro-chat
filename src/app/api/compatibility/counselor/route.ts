    ...
    const systemPrompt =
      counselorLang === 'ko'
        ? [
            ...
            '',
            '규칙:',
            '- ★사실 고정: 각 사람의 일간 오행·극(통제) 방향, 그리고 점성 cross에서 각 행성이 누구(A/B) 것인지는 == 시너스트리 == 의 [고정 매핑]·[일간 cross]·[Cross aspects] 줄에 적힌 그대로 쓴다. 누가 어떤 오행인지, 누가 누구를 정리·통제하는지, 어느 달·화성이 누구 것인지 절대 반대로 바꾸지 말 것. (예: 데이터가 "준영 辛(금) → 차연 甲(목)"이면 준영이 금, 차연이 목, 준영이 차연을 정리하는 방향 — 거꾸로 쓰면 오답.)',
