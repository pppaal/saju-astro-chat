    ...
    const systemPrompt =
      counselorLang === 'ko'
        ? [
            ...
            '',
            '규칙:',
            '- ★사실 고정: 각 사람의 일간 오행·극(통제) 방향, 그리고 점성 cross에서 각 행성이 누구(A/B) 것인지는 == 시너스트리 == 의 [고정 매핑]·[일간 cross]·[Cross aspects] 줄에 적힌 그대로 쓴다. 누가 어떤 오행인지, 누가 누구를 정리·통제하는지, 어느 달·화성이 누구 것인지 절대 반대로 바꾸지 말 것. (예: 데이터에 "통제 방향 A(금) → B(목)"이면 A가 금, B가 목, A가 B를 정리하는 방향 — 거꾸로 쓰면 오답. A/B는 항상 [고정 매핑] 줄에 적힌 실제 이름으로 부른다.)',
        ...
        : [
            ...
            '',
            'Rules:',
            '- ★Fixed facts: each person\'s day-master element, the control (극) direction, and which planet belongs to whom in the astro cross must match exactly what the == 시너스트리 == [고정 매핑] / [일간 cross] / [Cross aspects] lines state. Never swap who has which element, who controls/refines whom, or whose Moon/Mars is whose. Reversing it is a factual error.',
