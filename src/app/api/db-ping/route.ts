...
/**
 * Authorize admin requests (timing-safe comparison)
 */
function authorize(req: NextRequest): boolean {
  ...
  return timingSafeCompare(provided, token)
}
...
export const GET = withApiMiddleware(
  async (req: NextRequest, _context: ApiContext) => {
    ...
    if (!dbUrl || !ca) {
      return apiError(ErrorCodes.SERVICE_UNAVAILABLE, 'Database not configured')
    }
