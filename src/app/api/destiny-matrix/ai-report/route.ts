...
function _buildFreeDigestReport(baseReport: {
...
function buildFreePreviewLead(params: {
  ...
  if (lang === 'ko') {
    ...
    if (theme === 'career') {
      return `${gradeDescription} 이번 무료 요약은 커리어에서 무엇을 더 벌릴지보다, 어디서 판단력을 보여줘야 하는지를 먼저 짚습니다.`
    }
    if (theme === 'wealth') {
...
function buildFreePreviewNextSteps(params: {
  ...
  if (lang === 'ko') {
    if (theme === 'love') {
      return [
        '결론을 서두르지 말고, 상대와 내 해석이 어디서 어긋나는지 한 줄로 적어보세요.',
...
function buildRichFreeDigestReport(baseReport: {
  overallScore: { total: number; grade: string; gradeDescription: string }
  topInsights: Array<{
    title: string
    description: string
  ...
  const summary =
    baseReport.lang === 'ko'
      ? preferredDomain
        ? `${baseReport.overallScore.gradeDescription} ${preferredDomain} 중심 핵심 신호를 빠르게 요약했습니다.`
        : `${baseReport.overallScore.gradeDescription} 핵심 신호를 빠르게 요약했습니다.`
      : preferredDomain
        ? `A quick ${preferredDomain}-focused summary of key signals. ${baseReport.overallScore.gradeDescription}`
        : `A quick summary of key signals. ${baseReport.overallScore.gradeDescription}`
...
export const POST = withApiMiddleware(
  async (req: NextRequest, context) => {
    try {
      ...
      if (isFreeTier) {
        const freeReport = buildRichFreeDigestReport({
          overallScore: {
