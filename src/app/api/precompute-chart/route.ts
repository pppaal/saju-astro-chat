          ...
          try {
            ...
            try {
              ...
              ;(advancedSajuData as Record<string, unknown>).deukryeong =
                sajuModule.calculateDeukryeong(
                  dayMasterForAnalysis.name,
                  pillarsSimple.month.branch
                )
            } catch (err) {
              logger.debug('[precompute] Partial calculation failed', { error: err })
            }
            ...
            ;(advancedAstroData as Record<string, unknown>).currentTransits = {
              ...
              summary: {
                ...
                applyingCount: transitAspects.filter((a: { isApplying: boolean }) => a.isApplying)
                  .length,
                separatingCount: transitAspects.filter(
                  (a: { isApplying: boolean }) => !a.isApplying
                ).length,
              },
            }
          }
