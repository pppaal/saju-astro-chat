  ...
  async (request: NextRequest) => {
    try {
      ...
      if (!validationResult.success) {
        logger.warn('[Life prediction] validation failed', {
      ...
      switch (body.type) {
        ...
        case 'past-analysis': {
          ...
          if (targetDate) {
            ...
            const worstDay = retrospectives.reduce((worst, r) =>
          ...
          return createSuccessResponse(
            {
              ...
              nextBestWindow: result.nextBestWindow
                ? {
                    ...
                    specificDays: result.nextBestWindow.specificDays?.map((d) =>
                      formatDateToISO(d)
                    ),
                  }
                : null,
          ...
          return createSuccessResponse(
            {
              ...
              worstWeek: weeklyResult.worstWeek
                ? {
                    ...
                    bestDays: (weeklyResult.worstWeek.bestDays ?? []).map((d) =>
                      formatDateToISO(d)
                    ),
                  }
                : null,
