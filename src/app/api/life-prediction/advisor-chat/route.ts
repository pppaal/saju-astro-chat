...
function buildFallbackContext(
  sipsin?: string,
  eventType?: string
): { context: string; insights: string[] } {
  const tips: string[] = []
  ...
  if (eventType) {
    ...
    if (eventTips[eventType]) {
      tips.push(eventTips[eventType])
    }
  }
...
// ============================================================
async function fetchTherapeuticQuestions(theme: string, userMessage: string): Promise<string[]> {
  try {
    const response = await apiClient.post(
      '/api/counseling/therapeutic-questions',
      {
        theme,
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    // ============================================================
    try {
      const counselingResponse = await apiClient.post(
        '/api/counseling/chat',
        {
          message,
      ...
      if (
        counselingResponse.ok &&
        (counselingResponse.data as CounselingResponse)?.status === 'success'
      ) {
      ...
      logger.warn(
        '[Advisor Chat] Backend counseling engine unavailable, falling back to GPT:',
        backendError
      )
    ...
    // RAG 컨텍스트 + 치료적 질문 병렬로 가져오기 (40% 속도 개선)
    const [{ context: ragContext }, therapeuticQuestions] = await Promise.all([
      fetchRagContext(context.sipsin, context.eventType, message),
      fetchTherapeuticQuestions(context.eventType, message),
    ])
