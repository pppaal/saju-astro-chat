...
export function CompatRadarOverlay({
  ...
  const poly = (c: Counts) =>
    AXES.map((x, i) => {
      const p = pt(R * Math.max(0.04, c[x.key] / max), i)
      return `${p.x},${p.y}`
    }).join(' ')
    ...
    >
      <div className="mb-1 flex items-center justify-center gap-4 text-[11px] font-semibold">
        <span className="flex items-center gap-1.5" style={{ color: rose }}>
          <span className="inline-block h-2 w-2 rounded-full" style={{ backgroundColor: rose }} />
          {nameA}
        </span>
        <span className="flex items-center gap-1.5" style={{ color: sky }}>
          <span className="inline-block h-2 w-2 rounded-full" style={{ backgroundColor: sky }} />
          {nameB}
        </span>
      </div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full">
        ...
        <g className="chart-grow-in">
          {sumA > 0 && (
            <polygon points={poly(a)} fill="rgba(251,113,133,0.28)" stroke={rose} strokeWidth="2" />
          )}
          {sumB > 0 && (
            <polygon points={poly(b)} fill="rgba(56,189,248,0.24)" stroke={sky} strokeWidth="2" />
          )}
        </g>
