...
export default function CompatibilityError({
  ...
  useEffect(() => {
    logger.error("Compatibility error:", { message: error.message, digest: error.digest, stack: error.stack });
  }, [error]);
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#f5f4f1] p-4">
      <div className="max-w-md text-center" role="alert">
        ...
        {error.digest && (
          <p className="mt-4 text-xs text-[#a8a29e]">Error ID: {error.digest}</p>
        )}
      </div>
    </div>
  );
